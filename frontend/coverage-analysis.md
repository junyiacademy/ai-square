...
### 🔵 **第四優先：備份與舊版本** (影響 <10% 用戶)

這些檔案可能是舊版本或備份，考慮移除：

1. **備份路由** (0%, 總計 764行)
   - `route-gcs-backup.ts` (337行) - 登入備份
   - `route-optimized.ts` (270行) - PBL 優化版
   - `route-hybrid.ts` (157行) - Assessment 混合版
   - `route-v2.ts` (74行) - Assessment v2
   - **建議**: 確認後刪除這些檔案
...
**建議執行順序**：
1. Week 1: Discovery APIs + 刪除備份 → 76%
2. Week 2: 部分覆蓋優化 → 79%
