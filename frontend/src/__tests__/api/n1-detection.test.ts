...
mockOptimizationUtils.getPaginationParams = jest.fn(() => ({
  page: 1,
  limit: 10,
}));
mockOptimizationUtils.createPaginatedResponse = jest.fn(
  (data, total, params) => ({ data, total, ...params }),
);
...
describe("N+1 Query Detection Tests", () => {
  ...
  describe("PBL User Programs API - N+1 Prevention", () => {
    it("should use batch loading for tasks and evaluations to prevent N+1 queries", async () => {
      ...
      expect(taskRepo.findByProgramIds).toHaveBeenCalledWith([
        "prog-1",
        "prog-2",
        "prog-3",
      ]);
      ...
      expect(evaluationRepo.findByProgramIds).toHaveBeenCalledWith([
        "prog-1",
        "prog-2",
        "prog-3",
      ]);
  ...
  describe("Discovery My Programs API - N+1 Prevention", () => {
    it("should use batch loading for tasks to prevent N+1 queries in discovery scenarios", async () => {
      ...
      // Act
      const request = createAuthenticatedRequest(
        "http://localhost:3000/api/discovery/my-programs",
      );
  ...
  describe("Query Efficiency Validation", () => {
    it("should demonstrate the efficiency improvement with realistic data volumes", async () => {
      ...
      const mockTasks = Array.from(
        { length: programCount * tasksPerProgram },
        (_, i) =>
          createMockTask({
            id: `task-${i}`,
      ...
      const mockEvaluations = Array.from(
        { length: programCount * evaluationsPerProgram },
        (_, i) =>
          createMockEvaluation({
            id: `eval-${i}`,
