...
describe("ErrorTracker", () => {
  ...
  describe("captureApiError", () => {
    ...
    it("should handle non-string response", () => {
      ...
      const metrics = errorTracker.getMetrics();
      expect(metrics.recentErrors[0].context.response).toBe(
  ...
  describe("getAllErrors", () => {
    it("should combine memory and localStorage errors", () => {
      ...
      localStorageMock.store["error_tracker_reports"] = JSON.stringify([
        storedError,
      ]);
  ...
  describe("error reporting", () => {
    ...
    it("should log errors in development mode", () => {
      const errorTracker = getErrorTracker();
      const errorId = errorTracker.captureError("Development error", {
        component: "TestComp",
      });
