...
describe("Storage Errors", () => {
  ...
  describe("Error handling patterns", () => {
    ...
    it("handles different error types correctly", async () => {
      await expect(
        performStorageOperation(true, true, false, true, true),
      ).rejects.toThrow(StorageConnectionError);

      await expect(
        performStorageOperation(true, true, true, false, true),
      ).rejects.toThrow(StoragePermissionError);

      await expect(
        performStorageOperation(true, true, true, true, false),
      ).rejects.toThrow(StorageValidationError);

      await expect(
        performStorageOperation(false, true, true, true, true),
      ).rejects.toThrow(StorageNotFoundError);

      await expect(
        performStorageOperation(true, false, true, true, true),
      ).rejects.toThrow(StorageQuotaExceededError);

      await expect(
        performStorageOperation(true, true, true, true, true),
