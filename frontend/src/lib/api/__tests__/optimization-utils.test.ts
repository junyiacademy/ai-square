...
jest.mock("@/lib/cache/distributed-cache-service", () => ({
  distributedCacheService: {
    getWithRevalidation: jest
      .fn()
      .mockImplementation(async (_key, handler) => ({ data: await handler() })),
    get: jest.fn().mockResolvedValue(undefined),
    set: jest.fn().mockResolvedValue(undefined),
  },
}));
...
describe("optimization-utils", () => {
  ...
  it("createPaginatedResponse computes totals and flags", () => {
    const res = createPaginatedResponse([1, 2, 3], 50, {
      page: 2,
      limit: 10,
      offset: 10,
    });
  ...
  it("memoize caches results within maxAge window", () => {
    const fn = ((x: number) => x * 2) as unknown as (
      ...args: unknown[]
    ) => unknown;
...
const mockDistributedCacheService = distributedCacheService as jest.Mocked<
  typeof distributedCacheService
>;
...
describe("optimization-utils", () => {
  ...
  describe("cachedGET", () => {
    ...
    it("returns cached data when available in distributed cache", async () => {
      ...
      expect(mockDistributedCacheService.get).toHaveBeenCalledWith(
        "api:/api/test:?param=value",
      );
    ...
    it("uses stale-while-revalidate when specified", async () => {
      mockDistributedCacheService.getWithRevalidation.mockResolvedValue(
        mockData,
      );
      ...
      expect(
        mockDistributedCacheService.getWithRevalidation,
      ).toHaveBeenCalledWith("api:/api/test:", mockHandler, {
        ttl: 300000,
    ...
    it("handles cache errors gracefully", async () => {
      ...
      mockDistributedCacheService.get.mockRejectedValue(
      ...
      await expect(cachedGET(request, mockHandler)).rejects.toThrow(
        "Cache error",
      );
    });
  });
  ...
  describe("getPaginationParams", () => {
    it("extracts pagination parameters from request", () => {
      const request = new NextRequest(
        "http://localhost:3000/api/test?page=2&limit=50",
      );
    ...
    it("calculates offset from page and limit", () => {
      const request = new NextRequest(
        "http://localhost:3000/api/test?page=3&limit=25",
      );
      const params = getPaginationParams(request);
  ...
  describe("createPaginatedResponse", () => {
    ...
    it("creates paginated response with correct structure", () => {
      const result = createPaginatedResponse(testData.slice(0, 20), 50, {
        page: 1,
        limit: 20,
      });
    ...
    it("handles last page correctly", () => {
      const result = createPaginatedResponse(testData.slice(40), 50, {
        page: 3,
        limit: 20,
      });
