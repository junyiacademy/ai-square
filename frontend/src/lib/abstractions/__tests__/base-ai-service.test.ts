...
describe("BaseAIService", () => {
  ...
  describe("generateContent()", () => {
    ...
    it("should throw errors when configured to fail", async () => {
      ...
      await expect(failingAIService.generateContent(request)).rejects.toThrow(
    ...
    it("should handle mock errors", async () => {
      ...
      await expect(mockAIService.generateContent(request)).rejects.toThrow(
        "Custom test error",
      );
    });
  });
  ...
  describe("generateChat()", () => {
    ...
    it("should handle single message", async () => {
      const messages = [{ role: "user", content: "Single message test" }];

      const response = await mockAIService.generateChat(messages);

      expect(response).toBeDefined();
      expect(response.content).toBe(
        "Mock chat response to: Single message test",
      );
    });
    ...
    it("should throw errors when configured to fail", async () => {
      ...
      await expect(failingAIService.generateChat(messages)).rejects.toThrow(
  ...
  describe("evaluateResponse()", () => {
    it("should evaluate responses with criteria", async () => {
      ...
      const score = await mockAIService.evaluateResponse(
        prompt,
        response,
        criteria,
      );
    ...
    it("should handle multiple evaluation criteria", async () => {
      ...
      const response =
        "The answer is 42 because of the following calculation steps...";
      const criteria = [
    ...
    it("should throw errors when configured to fail", async () => {
      ...
      await expect(
        failingAIService.evaluateResponse(prompt, response, criteria),
  ...
  describe("Interface Compliance", () => {
    it("should implement all required methods", () => {
      ...
      methods.forEach((method) => {
        expect(typeof mockAIService[method as keyof MockAIService]).toBe(
          "function",
        );
      });
    });
  ...
  describe("Performance and Reliability", () => {
    ...
    it("should handle high-performance implementation efficiently", async () => {
      ...
      const requests = Array(100)
        .fill(null)
        .map((_, i) =>
          highPerfAIService.generateContent({
      ...
      const firstResponse = await highPerfAIService.generateContent({
        prompt: "Cache test",
      });
      const secondResponse = await highPerfAIService.generateContent({
        prompt: "Cache test",
      });
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should handle partial service failures gracefully", async () => {
      ...
      await expect(
        mockAIService.generateContent({ prompt: "test" }),
      ).rejects.toThrow("Content generation failed");

      // But other methods should still work
      const chatResponse = await mockAIService.generateChat([
        { role: "user", content: "test" },
      ]);
      expect(chatResponse).toBeDefined();

      const evalScore = await mockAIService.evaluateResponse("test", "test", [
        "test",
      ]);
    ...
    it("should handle network timeout scenarios", async () => {
      ...
      await expect(
        timeoutService.generateContent({ prompt: "test" }),
      ).rejects.toThrow("Request timeout");

      await expect(timeoutService.generateChat([])).rejects.toThrow(
        "Chat timeout",
      );

      await expect(timeoutService.evaluateResponse("", "", [])).rejects.toThrow(
        "Evaluation timeout",
      );
    });
  });
  ...
  describe("Mock Utility Functions", () => {
    it("should allow setting and clearing mock responses", () => {
      ...
      expect(mockAIService["mockResponses"].get("generateContent")).toEqual({
        content: "Different response",
        metadata: { different: true },
      });
    });
    ...
    it("should allow setting and clearing mock errors", () => {
      ...
      expect(mockAIService["mockErrors"].get("generateContent")).toBe(
        testError,
      );
    });
