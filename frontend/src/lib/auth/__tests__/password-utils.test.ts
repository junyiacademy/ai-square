...
describe("Password Utils", () => {
    ...
    it("should update user password hash with default role", async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringMatching(
          /UPDATE users[\s\S]*SET password_hash = \$1, role = \$2, updated_at = CURRENT_TIMESTAMP[\s\S]*WHERE id = \$3/,
        ),
  ...
  describe("updateUserEmailVerified", () => {
    it("should update user email verification status", async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringMatching(
          /UPDATE users[\s\S]*SET email_verified = true, email_verified_at = CURRENT_TIMESTAMP[\s\S]*WHERE id = \$1/,
        ),
        [userId],
      );
    });
    ...
    it("should handle null userId for email verification", async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(expect.any(String), [null]);
    });
  ...
  describe("getUserWithPassword", () => {
    it("should retrieve user with password hash by email", async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        expect.stringMatching(
          /SELECT[\s\S]*FROM users[\s\S]*WHERE LOWER\(email\) = LOWER\(\$1\)/,
        ),
        ["test@example.com"],
      );
    });
    ...
    it("should return null when user not found", async () => {
      ...
      const result = await getUserWithPassword(
        mockPool,
    ...
    it("should handle concurrent database operations", async () => {
      mockPool.query = jest
        ...
        .mockImplementation(
          () =>
            new Promise((resolve) =>
              setTimeout(() => resolve({ rows: [], rowCount: 1 }), 10),
            ),
        );
