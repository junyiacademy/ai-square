...
(mockYamlLoader.load as jest.MockedFunction<any>).mockResolvedValue(mockContent);
...
(mockYamlLoader.load as jest.MockedFunction<any>).mockResolvedValue(mockContent);
    ...
    it('should load other content types from repo', async () => {
      ...
      (mockFs.readFile as jest.MockedFunction<any>).mockResolvedValue('title: Question Bank');
    ...
    it('should return GCS override when available', async () => {
      ...
      expect(mockBucket.file).toHaveBeenCalledWith(`cms/overrides/${testType}/${testFileName}`);
      expect(result).toEqual({
        ...baseContent,
        ...overrideContent,
    ...
    it('should handle GCS override read failure gracefully', async () => {
      const baseContent = { title: 'Base Content' };
      (mockYamlLoader.load as jest.MockedFunction<any>).mockResolvedValue(baseContent);
      (mockFile.download as jest.MockedFunction<any>).mockRejectedValue(new Error('File not found'));
    ...
    it('should handle non-object override gracefully', async () => {
      const baseContent = { title: 'Base Content' };
      (mockYamlLoader.load as jest.MockedFunction<any>).mockResolvedValue(baseContent);
      (mockFile.download as jest.MockedFunction<any>).mockResolvedValue([Buffer.from('simple string')]);
      (mockYaml.load as jest.MockedFunction<any>).mockReturnValue('simple string');
  ...
  describe('listContent', () => {
    ...
    it('should handle directory read error gracefully', async () => {
      ...
      (mockFs.readdir as jest.MockedFunction<any>).mockRejectedValue(new Error('Directory not found'));

      const result = await contentService.listContent(testType);

      expect(result).toEqual([]);
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error listing content:', expect.any(Error));
    });
    ...
    it('should include GCS-only draft items', async () => {
      ...
      (mockFile.download as jest.MockedFunction<any>).mockResolvedValue([Buffer.from('title: Draft Content')]);
      (mockYaml.load as jest.MockedFunction<any>).mockReturnValue({ title: 'Draft Content' });
    ...
    it('should handle GCS listing error gracefully', async () => {
      const consoleErrorSpy = jest.spyOn(console, 'error').mockImplementation();
      mockFs.readdir.mockResolvedValue([]);
      (mockBucket.getFiles as jest.MockedFunction<any>).mockRejectedValue(new Error('GCS error'));

      const result = await contentService.listContent(testType);

      expect(result).toEqual([]);
      expect(consoleErrorSpy).toHaveBeenCalledWith('Error listing GCS drafts:', expect.any(Error));
    });
    ...
    it('should handle different working directory structures', async () => {
      ...
      expect(mockFs.readdir as jest.MockedFunction<any>).toHaveBeenCalledWith(expect.stringContaining('/app/frontend/public'));
    });
  });
  ...
  describe('saveContent', () => {
    ...
    it('should save draft content to GCS', async () => {
      ...
      (mockFile.download as jest.MockedFunction<any>).mockResolvedValue([Buffer.from('{}')]); // For metadata

      await contentService.saveContent(testType, testFileName, testContent, 'draft', testUser);

      expect(mockBucket.file).toHaveBeenCalledWith(`cms/drafts/${testType}/${testFileName}`);
    ...
    it('should save published content to overrides', async () => {
      ...
      await contentService.saveContent(testType, testFileName, testContent, 'published', testUser);
    ...
    it('should save content to history', async () => {
      (mockYaml.dump as jest.MockedFunction<any>).mockReturnValue('title: Test Content');
      (mockFile.download as jest.MockedFunction<any>).mockRejectedValue(new Error('Not found')); // No existing metadata
    ...
    it('should throw error when GCS is not configured', async () => {
      ...
      await expect(
        serviceWithoutGCS.saveContent(testType, testFileName, testContent, 'draft', testUser)
  ...
  describe('deleteOverride', () => {
    ...
    it('should ignore deletion errors silently', async () => {
      (mockFile.delete as jest.MockedFunction<any>).mockRejectedValue(new Error('File not found'));

      await expect(contentService.deleteOverride(testType, testFileName)).resolves.not.toThrow();
    });
    ...
    it('should handle missing GCS bucket gracefully', async () => {
      ...
      await expect(serviceWithoutGCS.deleteOverride(testType, testFileName)).resolves.not.toThrow();
    });
  });
  ...
  describe('publish', () => {
    ...
    it('should copy draft to override location', async () => {
      ...
      (mockFile.download as jest.MockedFunction<any>).mockResolvedValue([draftContent]);
  ...
  describe('private methods', () => {
    describe('readFromRepo', () => {
      it('should handle file read errors gracefully', async () => {
        ...
        const result = await (contentService as any).readFromRepo('domain', 'nonexistent.yaml');
    ...
    describe('readFromGCS', () => {
      it('should return null when bucket is not configured', async () => {
        ...
        const result = await (serviceWithoutGCS as any).readFromGCS('test/path');
        expect(result).toBeNull();
      });

      it('should handle GCS read errors gracefully', async () => {
        (mockFile.download as jest.MockedFunction<any>).mockRejectedValue(new Error('GCS error'));
    ...
    describe('getRepoPath', () => {
      ...
      it('should return default path for unknown content type', () => {
        const result = (contentService as any).getRepoPath('unknown' as ContentType);
  ...
  describe('error handling and edge cases', () => {
    it('should handle YAML parsing errors', async () => {
      ...
      const result = await (contentService as any).readFromRepo('domain', 'invalid.yaml');
      expect(result).toBeNull();
    });
    ...
    it('should handle concurrent cache access safely', async () => {
      ...
      const promise1 = contentService.getContent('domain', 'concurrent-test.yaml');
      const promise2 = contentService.getContent('domain', 'concurrent-test.yaml');
    ...
    it('should handle malformed metadata gracefully', async () => {
      (mockFile.download as jest.MockedFunction<any>).mockResolvedValue([Buffer.from('invalid json')]);

      const result = await (contentService as any).getMetadata('domain', 'test.yaml');
      expect(result).toBeNull();
    });
    ...
    it('should handle empty file content', async () => {
      ...
      const result = await (contentService as any).readFromRepo('domain', 'empty.yaml');
      expect(result).toBeNull();
    });
  });
