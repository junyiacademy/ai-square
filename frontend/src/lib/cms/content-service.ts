...
export class ContentService {
  ...
  async getContent(type: ContentType, fileName: string): Promise<unknown> {
    ...
    if (this.bucket) {
      const override = await this.readFromGCS(
        `${GCS_PATHS.overrides}${type}/${fileName}`,
      );
    ...
    if (this.bucket) {
      try {
        const [files] = await this.bucket.getFiles({
          prefix: `${GCS_PATHS.drafts}${type}/`,
        });

        for (const file of files) {
          const fileName = path.basename(file.name);
          const exists = items.find(
            (item) => item.file_path === `${type}/${fileName}`,
          );
  ...
  async saveContent(
    ...
    const basePath =
      status === "draft" ? GCS_PATHS.drafts : GCS_PATHS.overrides;
    ...
    // Update metadata
    const metadata: ContentMetadata = (await this.getMetadata(
      type,
      fileName,
    )) || {
      created_at: new Date(),
  ...
  private async readFromRepo(
    ...
    const filePath = path.join(
      baseDir,
      "public",
      this.getRepoPath(type),
      fileName,
    );
    console.log(
      `[ContentService] Reading from repo: ${filePath}, cwd: ${process.cwd()}`,
    );
