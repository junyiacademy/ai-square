...
describe("CacheService", () => {
  ...
  describe("Memory Cache Operations", () => {
    ...
    it("should enforce memory cache size limits", async () => {
      ...
      expect(await cacheService.get("key-109", "memory")).toEqual({
        value: 109,
      });
      expect(await cacheService.get("key-105", "memory")).toEqual({
        value: 105,
      });
    });
  });
  ...
  describe("LocalStorage Cache Operations", () => {
    ...
    it("should handle localStorage TTL expiration", async () => {
      ...
      await cacheService.set("ls-ttl-test", testData, {
        storage: "localStorage",
        ttl,
      });

      // Should be available
      expect(await cacheService.get("ls-ttl-test", "localStorage")).toEqual(
        testData,
      );
  ...
  describe("Both Storage Mode", () => {
    it("should store data in both memory and localStorage", async () => {
      ...
      expect(await cacheService.get("both-test", "localStorage")).toEqual(
        testData,
      );
    });
    ...
    it("should sync data from localStorage to memory when using both mode", async () => {
      ...
      await cacheService.set("sync-test", testData, {
        storage: "localStorage",
      });
    ...
    it("should prioritize memory cache over localStorage", async () => {
      ...
      await cacheService.set("priority-test", localStorageData, {
        storage: "localStorage",
      });
      await cacheService.set("priority-test", memoryData, {
        storage: "memory",
      });
  ...
  describe("Fetch with Cache", () => {
    ...
    it("should generate different cache keys for different requests", async () => {
      ...
      const result1 = await cacheService.fetchWithCache(
        "https://api.example.com/data1",
      );
      const result2 = await cacheService.fetchWithCache(
    ...
    it("should generate different cache keys for different HTTP methods", async () => {
      ...
      const getResult = await cacheService.fetchWithCache(
        "https://api.example.com/data",
      );
      const postResult = await cacheService.fetchWithCache(
        "https://api.example.com/data",
        { method: "POST" },
      );
    ...
    it("should respect cache options for fetch", async () => {
      ...
      await cacheService.fetchWithCache("https://api.example.com/ttl-test", {
        ttl: shortTtl,
      });
      ...
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () => Promise.resolve({ result: "refreshed" }),
      });
      const refreshedResult = await cacheService.fetchWithCache(
        "https://api.example.com/ttl-test",
        { ttl: shortTtl },
      );
  ...
  describe("LocalStorage Cleanup", () => {
    it("should cleanup expired localStorage entries", async () => {
      ...
      await cacheService.set(
        "expired-1",
        { data: "expired1" },
        { ttl: 1000, storage: "localStorage" },
      );
      mockDateNow.mockReturnValue(1640995200000 + 500);
      await cacheService.set(
        "valid-1",
        { data: "valid1" },
        { ttl: 2000, storage: "localStorage" },
      );
      ...
      expect(await cacheService.get("valid-1", "localStorage")).toEqual({
        data: "valid1",
      });
