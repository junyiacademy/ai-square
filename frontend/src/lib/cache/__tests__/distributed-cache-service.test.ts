...
const loadSvc = () => {
  ...
  const svc = mod.distributedCacheService as unknown as {
    [k: string]: unknown;
  };
  const redisMod = require("../redis-cache-service");
  const redis = redisMod.redisCacheService as unknown as {
    [k: string]: unknown;
  };
...
describe("distributed-cache-service", () => {
  it("get/set falls back across layers and local cache", async () => {
    ...
    await (svc as any).set(
      "k",
      { hello: "world" },
      { ttl: 500, useRedis: false },
    );
  ...
  it("getWithRevalidation returns fresh, serves stale then revalidates", async () => {
    ...
    const v1 = await (svc as any).getWithRevalidation("rv", fetcher, {
      ttl: 100,
    });
    ...
    const v2 = await (svc as any).getWithRevalidation("rv", fetcher, {
      ttl: 100,
    ...
    const v3 = await (svc as any).getWithRevalidation("rv", fetcher, {
      ttl: 100,
    });
  ...
  it("returns stale data on getWithRevalidation error path if available", async () => {
    ...
    const val = await svc.getWithRevalidation("e", fetcher, {
      ttl: 50,
      staleWhileRevalidate: 500,
    });
