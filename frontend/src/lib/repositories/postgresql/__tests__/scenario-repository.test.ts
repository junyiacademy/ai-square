...
describe("PostgreSQLScenarioRepository", () => {
  ...
  const mockDBScenario: DBScenario = {
    ...
    description: {
      en: "Learn AI ethics through problem solving",
      zh: "透過問題解決學習AI倫理",
    },
    ...
    resources: [{ type: "document", url: "/docs/ethics.pdf" }],
  ...
  describe("create", () => {
    it("should create a new scenario", async () => {
      ...
      expect(mockPool.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining(["assessment", "draft", "1.0.0", "manual"]),
      );
    ...
    it("should handle create errors", async () => {
      ...
      await expect(repository.create(scenario)).rejects.toThrow(
        "Insert failed",
      );
    });
  });
  ...
  describe("update", () => {
    ...
    it("should update mode-specific data", async () => {
      const updates = {
        pblData: {
          ksaMapping: { knowledge: ["K3"], skills: ["S2"], attitudes: [] },
        },
    ...
    it("should handle empty updates", async () => {
      ...
      await expect(repository.update("scenario-123", updates)).rejects.toThrow(
        "No fields to update",
      );
    });
  ...
  describe("checkPrerequisites", () => {
    it("should return true for scenario with no prerequisites", async () => {
      ...
      const result = await repository.checkPrerequisites(
        "scenario-123",
        "user-456",
      );
  ...
  describe("findAll", () => {
    it("should find all scenarios without pagination", async () => {
      const scenarios = [
        mockDBScenario,
        { ...mockDBScenario, id: "scenario-456" },
      ];
  ...
  describe("findBySourcePath", () => {
    it("should find scenario by source path", async () => {
      ...
      const result = await repository.findBySourcePath(
  ...
  describe("Data conversion", () => {
    ...
    it("should convert task templates correctly", async () => {
      ...
      expect(result!.taskTemplates[0].title).toEqual({
        en: "Task 1",
        zh: "任務1",
      });
  ...
  describe("Complex queries and edge cases", () => {
    it("should handle scenarios with different modes and data", async () => {
      ...
      expect(result[1].assessmentData).toEqual({
