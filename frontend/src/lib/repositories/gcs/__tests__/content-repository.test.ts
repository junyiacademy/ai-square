...
describe("GCSContentRepository", () => {
  ...
  describe("getYamlContent", () => {
    ...
    it("should successfully read and parse YAML content", async () => {
      ...
      mockFile.download.mockResolvedValueOnce([
        Buffer.from("title: Test Scenario"),
      ]);
    ...
    it("should throw error when file does not exist", async () => {
      ...
      await expect(
        repository.getYamlContent("nonexistent.yaml"),
    ...
    it("should handle download errors", async () => {
      ...
      await expect(repository.getYamlContent("error.yaml")).rejects.toThrow(
        "Download failed",
      );
    });
    ...
    it("should handle YAML parsing errors", async () => {
      ...
      await expect(repository.getYamlContent("invalid.yaml")).rejects.toThrow(
        "Invalid YAML",
      );
    });
  });
  ...
  describe("listYamlFiles", () => {
    ...
    it("should handle GCS errors", async () => {
      ...
      await expect(repository.listYamlFiles("error/")).rejects.toThrow(
        "GCS error",
      );
    });
  });
  ...
  describe("getScenarioContent", () => {
    ...
    beforeEach(() => {
      jest
        .spyOn(repository, "getYamlContent")
        .mockImplementation(async (path: string) => {
          if (path.includes("content_zh.yaml")) {
            return { ...mockScenarioData, title: "測試情境" };
          }
          if (
            path.includes("content.yaml") ||
            path.includes("_scenario.yaml")
          ) {
      ...
      jest
        .spyOn(repository as any, "transformScenarioContent")
    ...
    it("should get scenario content with language preference", async () => {
      const result: any = await repository.getScenarioContent(
        "test-scenario",
        "zh",
      );

      expect(repository.getYamlContent).toHaveBeenCalledWith(
        "scenarios/test-scenario/content_zh.yaml",
      );
    ...
    it("should fallback to default language when specific language not found", async () => {
      ...
      expect(repository.getYamlContent).toHaveBeenCalledWith(
        "scenarios/test-scenario/content_fr.yaml",
      );
      expect(repository.getYamlContent).toHaveBeenCalledWith(
        "scenarios/test-scenario/content.yaml",
      );
    ...
    it("should try different path patterns", async () => {
      ...
      expect(repository.getYamlContent).toHaveBeenCalledWith(
        "scenarios/test-scenario/content_en.yaml",
      );
      expect(repository.getYamlContent).toHaveBeenCalledWith(
        "scenarios/test-scenario/content.yaml",
      );
      expect(repository.getYamlContent).toHaveBeenCalledWith(
        "pbl_data/test-scenario_scenario.yaml",
      );
      expect(result.type).toBe("pbl");
    });
    ...
    it("should throw error when no content found", async () => {
      ...
      await expect(
        repository.getScenarioContent("nonexistent"),
  ...
  describe("getAllScenarios", () => {
    beforeEach(() => {
      jest
        .spyOn(repository, "listYamlFiles")
    ...
    it("should get scenarios filtered by type", async () => {
      ...
      expect(repository.listYamlFiles).toHaveBeenCalledWith("pbl_data/");
      expect(repository.listYamlFiles).not.toHaveBeenCalledWith(
        "assessment_data/",
      );
  ...
  describe("getKSAMappings", () => {
    const mockKSAData = {
      knowledge: {
        "K1.1": {
          domain: "AI Concepts",
          description: "Basic AI understanding",
        },
      ...
      skills: {
        ...
        "S2.1": {
          ...
          description: "AI critical thinking",
        },
      },
      attitudes: {
    ...
    beforeEach(() => {
      ...
      (repository as any).extractMultilingualField = jest
        .fn()
        .mockReturnValue({ en: "Test description" });
    });
  ...
  describe("getAILiteracyDomains", () => {
    ...
    beforeEach(() => {
      jest
        .spyOn(repository, "getYamlContent")
        .mockResolvedValue(mockDomainsData);
      (repository as any).extractMultilingualField = jest
        .fn()
        .mockReturnValue({ en: "Test field" });
    });
  ...
  describe("transformScenarioContent", () => {
    beforeEach(() => {
      (repository as any).extractMultilingualField = jest
        .fn()
        .mockImplementation((obj: any, field: string) => ({
          en: obj[field] || "Default value",
        }));
    });
    ...
    it("should transform raw content to ScenarioContent format", () => {
      ...
      const result = (repository as any).transformScenarioContent(
        rawContent,
        "test-id",
      );
  ...
  describe("error handling and edge cases", () => {
    it("should handle bucket initialization errors", () => {
      ...
      expect(
        () => new GCSContentRepository(mockStorage, "invalid-bucket"),
    ...
    it("should handle network timeouts", async () => {
      ...
      await expect(repository.getYamlContent("timeout.yaml")).rejects.toThrow(
        "Timeout",
      );
    });
