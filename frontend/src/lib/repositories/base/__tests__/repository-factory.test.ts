...
describe.skip("RepositoryFactory", () => {
  ...
  it("healthCheck returns success when providers respond", async () => {
    ...
    expect((result.details.postgresql as Record<string, unknown>).status).toBe(
      "connected",
    );
    expect((result.details.gcs as Record<string, unknown>).status).toBe(
      "connected",
    );
  });
