...
export class HybridTranslationService {
  ...
  async getProgram(
    ...
    return this.storageService.getProgram(
      scenarioId,
      programId,
  ...
  async getTask(
    ...
    return this.storageService.getTask(
      scenarioId,
      programId,
      taskId,
    ) as unknown as ITask;
  ...
  private async applyTranslations(
    ...
    try {
      ...
      console.warn(
        `Failed to load translations for ${scenarioId} in ${language}:`,
        error,
      );
  ...
  private mergeTranslations(
    ...
    if (yamlData.scenario_info) {
      translated.title =
        getTranslated(
          yamlData.scenario_info as Record<string, unknown>,
          "title",
        ) || translated.title;
    ...
    if ((translated as LegacyScenarioData).stages && yamlData.stages) {
      (translated as LegacyScenarioData).stages = (
        translated as LegacyScenarioData
      ).stages!.map((stage) => {
        ...
        const translatedStage = {
          ...
          description:
            getTranslated(yamlStage, "description") || stage.description,
        };
        ...
        if (stage.tasks && Array.isArray(stage.tasks)) {
          if (typeof stage.tasks[0] === "object" && yamlStage.tasks) {
            translatedStage.tasks = stage.tasks.map((task: Task | string) => {
              ...
              const yamlTask = (yamlStage.tasks as Task[]).find(
                (t) => t.id === task.id,
              );
              ...
              return {
                ...
                title:
                  getTranslated(
                    yamlTask as unknown as Record<string, unknown>,
                    "title",
                  ) || task.title,
                description:
                  getTranslated(
      ...
      yamlData.stages.forEach((yamlStage) => {
        if (yamlStage.tasks) {
          (yamlStage.tasks as Task[]).forEach((yamlTask) => {
            const task = (translated as LegacyScenarioData).tasks?.find(
              (t) => t.id === yamlTask.id,
            );
            if (task) {
              task.title =
              ...
              (task as unknown as Record<string, unknown>).instructions =
                getTranslated(
                  yamlTask as unknown as Record<string, unknown>,
                  "instructions",
                ) || (task as unknown as Record<string, unknown>).instructions;
              (task as unknown as Record<string, unknown>).expected_outcome =
                getTranslated(
                  yamlTask as unknown as Record<string, unknown>,
                  "expected_outcome",
                ) ||
  ...
  private async loadFromYaml(
    ...
    try {
      ...
      const scenario: Record<string, unknown> = {
        ...
        title:
          yamlData.scenario_info?.[`title_${language}`] ||
          yamlData.scenario_info?.title,
        description:
          yamlData.scenario_info?.[`description_${language}`] ||
          yamlData.scenario_info?.description,
      ...
      if (yamlData.stages) {
        (scenario as LegacyScenarioData).stages = yamlData.stages.map(
          (stage) => ({
            id: stage.id,
            title:
              (stage[`title_${language}` as keyof LegacyStage] as string) ||
              stage.title,
            description:
              (stage[
                `description_${language}` as keyof LegacyStage
              ] as string) || stage.description,
            tasks: stage.tasks || [],
          }),
        );
      }
      ...
      throw new Error(
        `Failed to load scenario ${scenarioId} from YAML: ${error}`,
      );
    }
  }
