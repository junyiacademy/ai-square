...
describe('PBLInitializationService', () => {
  ...
  describe('scanScenarioDirectories', () => {
    it('should scan and return scenario directories', async () => {
      ...
      fs.readdir.mockResolvedValueOnce(['scenario1_en.yaml', 'scenario1_zh.yaml']);
  ...
  describe('buildMultilingualScenario', () => {
    it('should build scenario with multilingual content', async () => {
      ...
      const result = await service.buildMultilingualScenario('test-dir', languageFiles);
  ...
  describe('createOrUpdateScenario', () => {
    it('should create new scenario if not exists', async () => {
      ...
      (mockScenarioRepo.findByMode as jest.Mock).mockResolvedValue([]);
      (mockScenarioRepo.create as jest.Mock).mockResolvedValue({ id: 'new-id', ...scenarioData });
    ...
    it('should update existing scenario if force is true', async () => {
      ...
      (mockScenarioRepo.findByMode as jest.Mock).mockResolvedValue([existingScenario]);
      (mockScenarioRepo.update as jest.Mock).mockResolvedValue({ id: 'existing-id', ...scenarioData });

      const result = await service.createOrUpdateScenario(scenarioData, true);

      expect(mockScenarioRepo.update).toHaveBeenCalledWith('existing-id', scenarioData);
  ...
  describe('clearPBLCaches', () => {
    it('should clear all PBL-related caches', async () => {
      ...
      (distributedCacheService.getAllKeys as jest.Mock).mockResolvedValue(mockKeys);
      (distributedCacheService.delete as jest.Mock).mockResolvedValue(undefined);

      const result = await service.clearPBLCaches();

      expect(distributedCacheService.delete).toHaveBeenCalledWith('scenarios:by-mode:pbl');
      expect(distributedCacheService.delete).toHaveBeenCalledWith('pbl:scenarios:*');

      // Should delete PBL-related keys
      expect(distributedCacheService.delete).toHaveBeenCalledWith('scenarios:by-mode:pbl');
      expect(distributedCacheService.delete).toHaveBeenCalledWith('pbl:scenarios:1');
      expect(distributedCacheService.delete).toHaveBeenCalledWith('pbl:scenarios:2');
      expect(distributedCacheService.delete).toHaveBeenCalledWith('scenarios:all');

      // Should not delete unrelated keys
      const deleteCalls = (distributedCacheService.delete as jest.Mock).mock.calls;
    ...
    it('should handle cache clearing errors gracefully', async () => {
      (distributedCacheService.getAllKeys as jest.Mock).mockRejectedValue(new Error('Cache error'));
  ...
  describe('initializePBLScenarios', () => {
    it('should initialize scenarios from YAML files', async () => {
      ...
      // Mock repository
      (mockScenarioRepo.findByMode as jest.Mock).mockResolvedValue([]);
      (mockScenarioRepo.create as jest.Mock).mockResolvedValue({ id: 'new-id', ...yamlData });

      // Mock cache service
      (distributedCacheService.getAllKeys as jest.Mock).mockResolvedValue([]);
      (distributedCacheService.delete as jest.Mock).mockResolvedValue(undefined);

      const result = await service.initializePBLScenarios({ force: false, clean: false });
    ...
    it('should clean scenarios before initialization if clean flag is true', async () => {
      ...
      const result = await service.initializePBLScenarios({ force: false, clean: true });
