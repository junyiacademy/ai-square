...
describe('PBLScenarioLoaderService', () => {
  ...
  describe('loadScenarios', () => {
      ...
      const mockScenarios: Partial<IScenario>[] = [
        {
          ...
          description: { en: 'Learn AI for job search', zhTW: '學習使用 AI 尋找工作' },
      ...
      const mockScenarios: Partial<IScenario>[] = [
        {
          ...
          description: 'Simple Description' as unknown as Record<string, string>,
          metadata: {}
        }
      ];
    ...
    it('should extract targetDomains from multiple sources', async () => {
      ...
      expect(result[0].targetDomains).toEqual(['engaging_with_ai', 'creating_with_ai']);
      expect(result[0].targetDomain).toEqual(['engaging_with_ai', 'creating_with_ai']);
      expect(result[0].domains).toEqual(['engaging_with_ai', 'creating_with_ai']);
    });
    ...
    it('should build scenario index after loading', async () => {
      ...
      expect(scenarioIndexService.buildIndex).toHaveBeenCalledWith(mockScenarios);
    });
  });
