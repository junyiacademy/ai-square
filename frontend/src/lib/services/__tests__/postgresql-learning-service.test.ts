...
import type { IProgram, ITask, IEvaluation, IScenario, ITaskTemplate } from '@/types/unified-learning';
...
describe('PostgreSQLLearningService', () => {
  ...
  describe('createLearningProgram', () => {
    it('should create a program and log PostgreSQL-specific message', async () => {
      const result = await service.createLearningProgram(
        'scenario-123',
        'user-123',
        { userEmail: 'test@example.com', userName: 'Test User', language: 'en' }
      );
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('[PostgreSQLLearningService] Program created: program-123')
      );
  ...
  describe('completeTask', () => {
    it('should complete task and log PostgreSQL-specific message', async () => {
      ...
      // Check PostgreSQL-specific logging
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('[PostgreSQLLearningService] Task completed: task-123, evaluation: eval-123')
      );
  ...
  describe('completeProgram', () => {
    it('should complete program and log PostgreSQL-specific message', async () => {
      ...
      const result = await service.completeProgram(
        'program-123',
        'user-123',
        { score: 85, feedbackData: { text: 'Program completed!' } }
      );
      ...
      // Check PostgreSQL-specific logging
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('[PostgreSQLLearningService] Program completed: program-123, evaluation: eval-123')
      );
  ...
  describe('PostgreSQL-specific hooks', () => {
    it('should log after program creation', async () => {
      await service.createLearningProgram(
        'scenario-123',
        'user-123',
        { userEmail: 'test@example.com', userName: 'Test User', language: 'en' }
      );
    ...
    it('should log after program completion', async () => {
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('[PostgreSQLLearningService] Program completed:')
      );
    });
  });
  ...
  describe('error handling', () => {
    it('should handle repository errors gracefully', async () => {
      mockScenarioRepo.findById.mockRejectedValueOnce(new Error('DB connection failed'));
  ...
  describe('batch operations', () => {
    it('should handle creating multiple tasks', async () => {
      const scenarioWithMultipleTasks = {
        ...mockScenario,
        taskTemplates: [
          { id: 't1', title: { en: 'Task 1' }, type: 'question' } as ITaskTemplate,
          { id: 't2', title: { en: 'Task 2' }, type: 'chat' } as ITaskTemplate,
          { id: 't3', title: { en: 'Task 3' }, type: 'creation' } as ITaskTemplate
        ]
      };

      mockScenarioRepo.findById.mockResolvedValueOnce(scenarioWithMultipleTasks);
