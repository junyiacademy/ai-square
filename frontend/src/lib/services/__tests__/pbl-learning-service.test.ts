...
describe("PBLLearningService", () => {
  ...
  describe("startLearning", () => {
    ...
    it("should throw error if scenario not found", async () => {
      ...
      await expect(
        service.startLearning("user-123", "scenario-123"),
  ...
  describe("getProgress", () => {
    ...
    it("should handle program not found", async () => {
      ...
      await expect(service.getProgress("program-123")).rejects.toThrow(
        "Program not found",
      );
    });
  ...
  describe("submitResponse", () => {
    ...
    it("should complete task when response indicates completion", async () => {
      ...
      expect(mockProgramRepo.updateProgress).toHaveBeenCalledWith(
        "program-123",
        1,
      );
      expect(mockTaskRepo.updateStatus).toHaveBeenCalledWith(
    ...
    it("should handle task not found", async () => {
      ...
      await expect(
        service.submitResponse("program-123", "task-123", {}),
  ...
  describe("completeLearning", () => {
    ...
    it("should handle program not found", async () => {
      ...
      await expect(service.completeLearning("program-123")).rejects.toThrow(
        "Program not found",
      );
    });
  ...
  describe("evaluateTask", () => {
    it("should evaluate task completion", async () => {
      ...
      expect(mockEvaluationRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          feedbackText:
            "Excellent work! You demonstrated deep understanding and creative problem-solving.",
        }),
      );
    ...
    it("should handle task not found", async () => {
      ...
      await expect(service.evaluateTask("task-123")).rejects.toThrow(
        "Task not found",
      );
    });
    ...
    it("should give lower score for short time", async () => {
      ...
      expect(mockEvaluationRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          feedbackText:
            "Good effort! You showed solid understanding of the concepts.",
        }),
      );
    });
    ...
    it("should give low score for incomplete task", async () => {
      ...
      expect(mockEvaluationRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          feedbackText:
            "Keep working on it! Consider exploring the problem from different angles.",
        }),
      );
    });
  });
  ...
  describe("generateFeedback", () => {
    it("should generate feedback in requested language", async () => {
      ...
      expect(result).toBe(
        "做得好！你在這個任務中獲得了 85% 的分數。Well done!",
      );
    });
    ...
    it("should handle evaluation not found", async () => {
      ...
      await expect(service.generateFeedback("eval-123", "en")).rejects.toThrow(
    ...
    it("should generate Spanish feedback", async () => {
      ...
      expect(result).toBe(
        "¡Buen trabajo! Obtuviste 90% en esta tarea. Excelente trabajo!",
      );
    });
  });
  ...
  describe("edge cases and full coverage", () => {
    ...
    it("should handle missing aiResponse feedback", async () => {
      ...
      expect(result.feedback).toBe(
        "You're showing good understanding of the problem.",
      );
    });
    ...
    it("should calculate completion feedback for high score", async () => {
      ...
      expect(mockEvaluationRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
          feedbackText: expect.stringContaining(
            "Congratulations on completing the PBL scenario",
          ),
        }),
      );
    });
