import { getScenarioStorageService, type IScenarioStorageService } from '../scenario-storage-service';
...
describe('MockScenarioStorageService', () => {
  ...
  describe('getTask', () => {
    ...
    it('includes task and program IDs', async () => {
      const task = await service.getTask('scenario-123', 'program-456', 'task-789');
  ...
  describe('getEvaluation', () => {
    it('returns evaluation data with correct structure', async () => {
      ...
      const evaluation = await service.getEvaluation(scenarioId, programId, evaluationId);
    ...
    it('includes evaluation and program IDs', async () => {
      const evaluation = await service.getEvaluation('scenario-123', 'program-456', 'eval-789');
  ...
  describe('interface compliance', () => {
    ...
    it('all methods return promises', async () => {
      ...
      expect(service.getEvaluation('sid', 'pid', 'eid')).toBeInstanceOf(Promise);
    });
  });
});
