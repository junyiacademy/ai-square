...
let assessmentYAMLLoader: { scanAssessments: jest.Mock; loadAssessment: jest.Mock; getAvailableLanguages: jest.Mock; getTranslatedField: jest.Mock };
...
function loadService() {
  ...
  jest.doMock('../assessment-yaml-loader', () => ({
    assessmentYAMLLoader: {
      ...
      getTranslatedField: jest.fn((cfg: Record<string, unknown>, field: string, lang: string) => {
  ...
  jest.isolateModules(() => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    ServiceCtor = require('../scenario-initialization-service').ScenarioInitializationService;
  });
...
describe('ScenarioInitializationService', () => {
  ...
  it('initializes PBL scenarios (create path)', async () => {
    ...
    pblYAMLLoader.loadScenario.mockResolvedValue({
      scenario_info: { title: 'AI Job Search', description: 'desc', difficulty: 'intermediate', estimated_duration: '45 minutes', target_domains: ['engage'] },
      programs: [{ id: 'p1', tasks: [{ id: 't1', title: 'Task' }] }]
    });

    mockScenarioRepo.findByMode.mockResolvedValue([]);
    mockScenarioRepo.create.mockImplementation(async (dto: Omit<IScenario, 'id'>) => existingScenario({ id: 'new1', ...dto }));
  ...
  it('skips when scenario exists and forceUpdate=false', async () => {
    ...
    mockScenarioRepo.findByMode.mockResolvedValue([ existingScenario({ sourcePath: yamlPath }) ]);
  ...
  it('dryRun should not write, but count created', async () => {
    ...
    pblYAMLLoader.scanScenarios.mockResolvedValue(['ai_job_search']);
    pblYAMLLoader.loadScenario.mockResolvedValue({ scenario_info: { title: 'A', description: '' }, programs: [] });
    mockScenarioRepo.findByMode.mockResolvedValue([]);
  ...
  it('initializes Discovery scenarios via scanPaths/loadPath', async () => {
    ...
    discoveryYAMLLoader.loadPath.mockResolvedValue({
      metadata: { title: 'App Dev', long_description: 'ldesc', estimated_hours: 2, skill_focus: ['js'] },
      category: 'tech', difficulty_range: 'beginner-advanced',
      world_setting: {}, skill_tree: { core_skills: [], advanced_skills: [] }, milestone_quests: []
    });
    ...
    mockScenarioRepo.findByMode.mockResolvedValue([]);
    mockScenarioRepo.create.mockImplementation(async (dto: Omit<IScenario, 'id'>) => existingScenario({ id: 'd1', ...dto }));
  ...
  it('initializes Assessment scenarios combining languages', async () => {
    ...
    assessmentYAMLLoader.getAvailableLanguages.mockResolvedValue(['en', 'zhTW']);

    assessmentYAMLLoader.loadAssessment
      .mockResolvedValueOnce({
        config: { title: 'AI Literacy Test', description: 'desc', total_questions: 20, time_limit_minutes: 30, passing_score: 75, domains: [] },
        questions: [{ id: 'q1', question: 'Q1', options: ['a'], correct_answer: 'a' }]
      })
      .mockResolvedValueOnce({
        config: { title_zhTW: 'AI 素養測驗', description_zhTW: '描述' },
        questions: [{ id: 'q1', question_zhTW: 'Q1-中', options: ['a'], correct_answer: 'a' }]
      });

    mockScenarioRepo.findByMode.mockResolvedValue([]);
    mockScenarioRepo.create.mockImplementation(async (dto: Omit<IScenario, 'id'>) => existingScenario({ id: 'a1', ...dto }));

    const result = await service.initializeAssessmentScenarios();
    expect(assessmentYAMLLoader.scanAssessments).toHaveBeenCalled();
    expect(assessmentYAMLLoader.getAvailableLanguages).toHaveBeenCalledWith('ai_literacy');
    expect(mockScenarioRepo.create).toHaveBeenCalledTimes(1);
    expect(result.created).toBe(1);
  });
