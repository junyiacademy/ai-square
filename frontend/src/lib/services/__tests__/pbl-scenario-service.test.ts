...
describe("PBLScenarioService", () => {
  ...
  describe("createScenarioFromYAML", () => {
    it("should create scenario from YAML with English content", async () => {
      ...
      const scenario = await service.createScenarioFromYAML(
        "ai-education-design",
        "en",
      );
    ...
    it("should handle YAML parse errors", async () => {
      ...
      await expect(
        service.createScenarioFromYAML("invalid", "en"),
  ...
  describe("findOrCreateScenarioByYAMLId", () => {
    it("should return existing scenario if found", async () => {
      ...
      const result = await service.findOrCreateScenarioByYAMLId(
        "ai-education-design",
        "en",
      );
    ...
    it("should handle multiple scenarios and find correct one", async () => {
      const scenarios = [
        {
          ...mockCreatedScenario,
          sourceMetadata: { yamlId: "other-scenario" },
        },
        {
          ...mockCreatedScenario,
          sourceMetadata: { yamlId: "ai-education-design" },
        ...
        {
          ...mockCreatedScenario,
          sourceMetadata: { yamlId: "another-scenario" },
        },
      ];
  ...
  describe("listAvailableYAMLIds", () => {
    ...
    it("should handle directory read errors", async () => {
      ...
      await expect(service.listAvailableYAMLIds()).rejects.toThrow(
        "Permission denied",
      );
    });
  });
  ...
  describe("private methods through public API", () => {
    ...
    it("should preserve original task data in metadata", async () => {
      ...
      expect(createCall.taskTemplates[0].metadata.originalTaskData).toEqual(
        mockPBLYAMLData.tasks[0],
      );
    });
  });
  ...
  describe("edge cases", () => {
    it("should handle concurrent calls gracefully", async () => {
      ...
      const promises = Array(5)
        .fill(null)
        .map((_, i) => service.createScenarioFromYAML(`scenario-${i}`, "en"));
