...
describe("ScenarioTranslationService", () => {
    ...
    it("should handle process.cwd() variations", async () => {
      ...
      expect(fs.readFile).toHaveBeenCalledWith(
        expect.stringContaining(
          "/path/to/project/frontend/public/assessment_data",
        ),
        "utf-8",
      );
  ...
  describe("translateScenario", () => {
    ...
    it("should preserve existing metadata", async () => {
      ...
      const result = await service.translateScenario(
  ...
  describe("clearExpiredCache", () => {
    ...
    it("should not clear non-expired entries", () => {
      ...
      service["cache"].set("key1", {
        content: {},
        timestamp: mockTime - 5 * 60 * 1000,
      });
      service["cache"].set("key2", {
        content: {},
        timestamp: mockTime - 15 * 60 * 1000,
      });
  ...
  describe("edge cases", () => {
    it("should handle undefined sourceMetadata", async () => {
      ...
      const result = await service.loadTranslation(
    ...
    it("should handle concurrent requests for same translation", async () => {
      ...
      jest.clearAllMocks();
      const cachedResult = await concurrentService.loadTranslation(
        mockScenario,
        "zh",
      );
      expect(fs.readFile).not.toHaveBeenCalled();
