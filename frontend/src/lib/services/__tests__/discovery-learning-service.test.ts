...
import type {
  ...
  DiscoveryProgress,
} from "../discovery-learning-service";
...
describe("DiscoveryLearningService", () => {
  ...
  describe("submitResponse", () => {
    ...
    it("should handle level up", async () => {
      ...
      await service.submitResponse("program-123", "task-123", {
        completed: true,
      });
  ...
  describe("edge cases and coverage", () => {
    ...
    it("should award achievement for advanced creation tasks", async () => {
      ...
      expect(result.metadata?.newAchievements).toContain(
        "Advanced Challenge Master",
      );
    });
      ...
      const tasksWithSkills = [
        {
          ...mockTask,
          status: "completed",
          discoveryData: { skills: ["writing", "design"] },
        },
        {
          ...mockTask,
          id: "task-2",
          status: "completed",
          discoveryData: { skills: ["writing"] },
        },
        {
          ...mockTask,
          id: "task-3",
          status: "completed",
          discoveryData: { skills: ["writing", "marketing"] },
        },
      ];
      mockProgramRepo.findById.mockResolvedValue(mockProgram);
    ...
    it("should unlock skills for specific levels", async () => {
      ...
      expect(mockProgramRepo.update).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          discoveryData: expect.objectContaining({
            level: 2,
            unlockedSkills: expect.arrayContaining([
      ...
      expect(mockTaskRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({
          title: expect.objectContaining({
            en: expect.stringContaining("Master"),
          }),
          type: "creation",
          status: "pending",
        }),
