...
export class UnifiedEvaluationSystem implements IEvaluationSystem {
  ...
  async evaluateProgram(
    ...
    const dimensionMap = new Map<
      string,
      { total: number; count: number; maxTotal: number }
    >();
    ...
    const aggregatedDimensions: {
      ...
      feedback: string;
      maxScore?: number;
    }[] = Array.from(dimensionMap.entries()).map(([dimension, data]) => ({
      dimension,
    ...
    return {
      ...
      score: averageScore,
      maxScore: 100,
      feedbackText: await this.generateProgramFeedback(
        program,
        taskEvaluations,
      ),
      feedbackData: {},
  ...
  async generateFeedback(
    ...
    try {
      ...
      return (
        response.content || evaluation.feedbackText || "No feedback available"
      );
    } catch (error) {
  ...
  private async evaluatePBLTask(
    ...
    const overallScore =
      (qualityScore.knowledge + qualityScore.skills + qualityScore.attitudes) /
      3;
    ...
    return {
      ...
      metadata: {
        ...
        ksaCodes:
          ((task.content.context as Record<string, unknown>)
            ?.ksaCodes as string[]) || [],
        sourceType: "pbl",
        targetType: "task",
      },
   ...
   * Assessment Task 評估
   */
  private async evaluateAssessmentTask(
    task: ITask,
    context: IEvaluationContext,
  ): Promise<IEvaluation> {
    const interactions = task.interactions;
    ...
    // 計算正確率
    const correctAnswers = interactions.filter(
      (i) => i.type === "user_input" && i.metadata?.isCorrect === true,
    ).length;
    ...
    const domainScores = this.calculateDomainScores(
      interactions,
      questions as { [key: string]: unknown; domain?: string }[],
    );
   ...
   * Discovery Task 評估
   */
  private async evaluateDiscoveryTask(
    task: ITask,
    context: IEvaluationContext,
  ): Promise<IEvaluation> {
    const interactions = task.interactions;
    ...
    return {
      ...
      discoveryData: {
        ...
        skillsImproved:
          ((
            (task.content as Record<string, unknown>).context as Record<
              string,
              unknown
            >
   ...
   * 通用 Task 評估
   */
  private async evaluateGenericTask(
    task: ITask,
    context: IEvaluationContext,
  ): Promise<IEvaluation> {
    const interactions = task.interactions;
   ...
   * 分析 PBL 互動品質
   */
  private analyzePBLInteractionQuality(interactions: IInteraction[]): {
    knowledge: number;
    skills: number;
    attitudes: number;
  } {
  ...
  private calculateDomainScores(
    ...
    questions: { domain?: string; [key: string]: unknown }[],
  ...
  private analyzeExplorationQuality(interactions: IInteraction[]): number {
    ...
    const userActions = interactions.filter(
      (i) => i.type === "user_input",
    ).length;
   ...
   * 產生 Program 級別回饋
   */
  private async generateProgramFeedback(
    program: IProgram,
    taskEvaluations: IEvaluation[],
  ): Promise<string> {
    ...
    return (
      `You have completed ${completedTasks} tasks with an average score of ${averageScore.toFixed(1)}%. ` +
  ...
  private buildFeedbackPrompt(
    ...
    ${
      evaluation.domainScores
        ? `
    Dimension Scores:
    ${Object.entries(evaluation.domainScores)
      .map(([d, score]: [string, number]) => `- ${d}: ${score}/100`)
      .join("\n")}
    `
        : ""
    }
