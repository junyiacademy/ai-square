...
describe("EvaluationStrategies", () => {
    ...
    interactions: [
      {
        type: "user_input",
        content:
          "This is my detailed solution to the problem with comprehensive analysis",
  ...
  describe("PBLEvaluationStrategy", () => {
    ...
    it("should evaluate PBL task with quality metrics", async () => {
      const result = await strategy.evaluateTask(
        mockTask,
    ...
    it("should calculate quality metrics correctly", async () => {
      ...
      const qualityMetrics = (result.pblData as Record<string, unknown>)
    ...
    it("should evaluate program with task evaluations", async () => {
      ...
      const result = await strategy.evaluateProgram(
        mockProgram,
        taskEvaluations,
      );
  ...
  describe("AssessmentEvaluationStrategy", () => {
    ...
    it("should provide domain-specific scores", async () => {
      ...
      const domainScores = (result.assessmentData as Record<string, unknown>)
        .domainScores as Record<string, unknown>;
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle null interactions gracefully", async () => {
      ...
      const result = await pblStrategy.evaluateTask(
        task,
        mockEvaluationContext,
      );
      expect(result.score).toBe(0);
    ...
    it("should handle missing program context", async () => {
      ...
      await expect(
        pblStrategy.evaluateTask(mockTask, contextWithoutProgram),
      ).rejects.toThrow();
    });
  });
});
