...
export class PBLScenarioService {
  ...
  async createScenarioFromYAML(
    ...
    const scenario: Omit<IScenario, "id"> = {
      ...
      title: {
        [language]: this.getLocalizedField(
      ...
      objectives: this.getLocalizedArrayField(
        yamlData.scenario_info,
        "learning_objectives",
        language,
      ),
      ...
      taskTemplates: this.convertTasksToTemplates(
        yamlData.tasks || [],
        language,
      ),
   ...
   */
  async findOrCreateScenarioByYAMLId(
    yamlId: string,
    language: string = "en",
  ): Promise<IScenario> {
    const { repositoryFactory } =
    ...
    const existingScenario = existingScenarios.find(
      (s: IScenario) => s.sourceMetadata?.yamlId === yamlId,
    );
  ...
  private getLocalizedArrayField(
    obj: Record<string, unknown>,
    fieldName: string,
    language: string,
  ): string[] {
  ...
  private convertTasksToTemplates(
    ...
    return tasks.map((task) => ({
      ...
      description: {
        [language]: this.getLocalizedField(task, "description", language),
      },
      metadata: {
        category: task.category,
        instructions: this.getLocalizedArrayField(
          task,
          "instructions",
          language,
        ),
        expectedOutcome: this.getLocalizedField(
          task,
          "expected_outcome",
          language,
        ),
