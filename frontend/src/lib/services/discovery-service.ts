...
export class DiscoveryService implements IDiscoveryService {
  ...
  async getPersonalizedRecommendations(
    ...
    const recommendations =
      await this.discoveryRepo.getCareerRecommendations(userId);
  ...
  async analyzeSkillGaps(
    ...
    // 分析差距
    const skillGaps: ISkillGap[] = career.discoveryData.requiredSkills.map(
      (skill) => {
        const currentLevel = userSkills.get(skill) || 0;
        const requiredLevel = this.getRequiredSkillLevel(
          career.discoveryData.careerLevel,
        );
  ...
  async calculateCareerReadiness(
    ...
    // 加權平均
    const totalWeight = readinessScores.reduce(
      (sum, item) => sum + item.weight,
      0,
    );
    const weightedSum = readinessScores.reduce(
  ...
  async generateCareerInsights(
    ...
    const career = await this.discoveryRepo.findCareerPathById(careerId);
    const userProgress =
      await this.discoveryRepo.getUserDiscoveryProgress(userId);
    const skillGaps = await this.analyzeSkillGaps(userId, careerId);
    ...
    const prompt = `
      ...
      - Key skill gaps: ${skillGaps
        .slice(0, 3)
        .map((g) => `${g.skill} (${g.currentLevel}/${g.requiredLevel})`)
        .join(", ")}
