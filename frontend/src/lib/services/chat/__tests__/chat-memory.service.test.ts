  ...
  describe("updateShortTermMemory", () => {
    it("should add new topic to recent topics", async () => {
      ...
      mockFile.download.mockResolvedValueOnce([
        Buffer.from(JSON.stringify(existingMemory)),
      ]);

      mockFile.exists.mockResolvedValueOnce([false]); // long_term.json (for getUserMemory)
      mockFile.save.mockResolvedValue(undefined);

      await service.updateShortTermMemory(
        "user@test.com",
        "New AI topic discussion",
      );
  ...
  describe("getUserMemory", () => {
    it("should retrieve both short-term and long-term memory", async () => {
      ...
      const shortTermFile = {
        ...
        download: jest
          .fn()
          .mockResolvedValue([Buffer.from(JSON.stringify(shortTerm))]),
      };
      ...
      const longTermFile = {
        ...
        download: jest
          .fn()
          .mockResolvedValue([Buffer.from(JSON.stringify(longTerm))]),
      };
