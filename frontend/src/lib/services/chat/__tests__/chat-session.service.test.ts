...
import { ChatSessionService, type ChatSession, type ChatMessage } from '../chat-session.service';
...
describe('ChatSessionService', () => {
  ...
  describe('createSession', () => {
    it('should create new session with first message', async () => {
      ...
      const result = await service.createSession('user@test.com', 'session1', message);
    ...
    it('should handle sanitized email format', async () => {
      ...
      expect(mockBucket.file).toHaveBeenCalledWith('user/user_at_test_com/chat/sessions/session1.json');
    });
  });
  ...
  describe('getSession', () => {
    it('should retrieve existing session', async () => {
      const existingSession: ChatSession = {
        ...
        messages: [
          { id: 'msg1', role: 'user', content: 'Hello', timestamp: '2024-01-01T00:00:00Z' }
        ],
        last_message: 'Hello',
        message_count: 1,
        tags: []
      ...
      mockFile.download.mockResolvedValue([Buffer.from(JSON.stringify(existingSession))]);
  ...
  describe('addMessage', () => {
    it('should add message to existing session', async () => {
      ...
      const result = await service.addMessage('user@test.com', 'session1', newMessage);
    ...
    it('should create new session if not exists', async () => {
      ...
      const result = await service.addMessage('user@test.com', 'session1', message);
  ...
  describe('updateSessionTitle', () => {
    it('should update session title', async () => {
      ...
      const result = await service.updateSessionTitle('user@test.com', 'session1', 'New Title');
  ...
  describe('updateChatIndex', () => {
    ...
    it('should update existing index', async () => {
      ...
      mockFile.exists.mockResolvedValue([true]);
      mockFile.download.mockResolvedValue([Buffer.from(JSON.stringify(existingIndex))]);
      mockFile.save.mockResolvedValue(undefined);
      ...
      await service.updateChatIndex('user@test.com', 'session1', updatedSession);
  ...
  describe('sanitizeEmail', () => {
    it('should sanitize email correctly', () => {
      const result = (service as unknown as { sanitizeEmail: (email: string) => string }).sanitizeEmail('user@test.com');
      expect(result).toBe('user_at_test_com');
    });

    it('should handle multiple dots', () => {
      const result = (service as unknown as { sanitizeEmail: (email: string) => string }).sanitizeEmail('user.name@test.co.uk');
