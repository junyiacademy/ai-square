...
import {
  ...
  type UserContext,
} from "../chat-context-builder.service";
  ...
  describe("buildContext", () => {
    it("should build complete user context", async () => {
      ...
      const memory = {
        shortTerm: {
          recentActivities: [
            { type: "assessment", timestamp: "2024-01-01T00:00:00Z" },
          ],
      ...
      const userDataFile = {
        ...
        download: jest
          .fn()
          .mockResolvedValue([Buffer.from(JSON.stringify(userData))]),
      };
      ...
      const shortTermFile = {
        ...
        download: jest
          .fn()
          .mockResolvedValue([Buffer.from(JSON.stringify(memory.shortTerm))]),
      };
      ...
      const longTermFile = {
        ...
        download: jest
          .fn()
          .mockResolvedValue([Buffer.from(JSON.stringify(memory.longTerm))]),
      };
    ...
    it("should handle partial memory data", async () => {
      ...
      mockFile.download.mockResolvedValueOnce([
        Buffer.from(JSON.stringify(partialMemory.shortTerm)),
      ]);
