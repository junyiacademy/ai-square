...
export class AssessmentLearningService implements BaseLearningService {
    ...
    await this.taskRepo.create({
      ...
      content: {
        ...
        context: {
          questions,
          timeLimit:
            (scenario.assessmentData as Record<string, unknown>)
              ?.timeLimitMinutes || 15,
          language,
        },
  ...
  async completeAssessment(
    ...
    const evaluation = await this.evaluationRepo.create({
      ...
      feedbackText: this.generateSimpleFeedback(
  ...
  async getProgress(programId: string): Promise<LearningProgress> {
    ...
    const answeredQuestions = task.interactions.filter(
      (i) =>
        i.type === "user_input" &&
        (i.content as Record<string, unknown>)?.questionId,
    ).length;

    const assessmentData =
      (program.assessmentData as Record<string, unknown>) || {};
    const selectedQuestions =
      (assessmentData.selectedQuestions as string[]) || [];
    const totalQuestions = selectedQuestions.length;
    const timeStartedValue =
      (assessmentData.timeStarted as string) || program.startedAt || "";
  ...
  private selectQuestions(
    ...
    console.log(
      "selectQuestions - assessmentData keys:",
      Object.keys(assessmentData),
    );
    console.log(
      "selectQuestions - assessmentData.questionBank type:",
      typeof assessmentData.questionBank,
    );
    ...
    const questionBankByLanguage =
      assessmentData.questionBankByLanguage as Record<
        string,
        Array<Record<string, unknown>>
      >;
    ...
    if (
      ...
      (questionBankByLanguage[language] || questionBankByLanguage["en"])
    ) {
      ...
      if (Array.isArray(questionBank)) {
        ...
        if (keys.every((k) => !isNaN(Number(k)))) {
          ...
          bankArray = Object.values(questionBank) as Array<
      ...
      console.log(
        `questionBank converted to array with ${bankArray.length} domains`,
      );

      // Extract questions from all domains
      allQuestions = bankArray.flatMap((domain: Record<string, unknown>) => {
        const domainQuestions =
          (domain.questions as Array<Record<string, unknown>>) || [];
    ...
    if (allQuestions.length === 0) {
      console.error(
        "No questions found. AssessmentData:",
    ...
    const selected = shuffled.slice(
      0,
      Math.min(totalQuestions, shuffled.length),
    );
  ...
  private calculateScore(task: ITask): {
    ...
    const interactions = task.interactions.filter(
      (i) => i.type === "user_input",
    );
    const correctAnswers = interactions.filter(
      (i) => (i.content as Record<string, unknown>)?.isCorrect,
    ).length;
    ...
    return {
      ...
      correctAnswers,
      totalQuestions,
      questionResults: interactions.map((i) => i.content) as Record<
        string,
        unknown
      >[],
    };
