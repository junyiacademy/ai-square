...
import { fromIInteraction, isAssessmentInteraction } from '@/types/assessment-types';
import type { ITaskRepository, IProgramRepository, IEvaluationRepository, IUserRepository } from '@/lib/repositories/interfaces';
...
export class AssessmentCompletionService {
   ...
   */
  async validateCompletion(program: IProgram): Promise<CompletionValidationResult | null> {
    // Check if program is already completed with evaluation
    if (program.status === 'completed' && program.metadata?.evaluationId) {
      const existingEvaluation = await this.evaluationRepo.findById?.(program.metadata.evaluationId as string);
    ...
    const existingEvaluations = await this.evaluationRepo.findByProgram(program.id);
   ...
   */
  async checkAssessmentCompletion(tasks: ITask[]): Promise<AssessmentCompletionStatus> {
   ...
   */
  async collectQuestionsAndAnswers(tasks: ITask[]): Promise<QuestionsAndAnswers> {
  ...
  async createEvaluation(data: EvaluationData): Promise<IEvaluation> {
    return await this.evaluationRepo.create({
      ...
      feedbackData: {},
      domainScores: Array.from(data.domainScores.values()).reduce((acc, ds) => {
      ...
      metadata: {
        ...
        domainScores: Object.fromEntries(
          Array.from(data.domainScores.entries()).map(([domain, ds]) => [domain, ds.score])
        ),
  ...
  calculateCompletionTime(program: IProgram): number {
    const startTime = program.metadata?.createdAt ||
