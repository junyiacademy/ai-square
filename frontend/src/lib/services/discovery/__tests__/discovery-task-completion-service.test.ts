...
const createTestTask = (overrides: Partial<ITask> & { interactions?: IInteraction[] } = {}): ITask & { interactions: IInteraction[] } => {
  const base = {
    id: 'task-1',
    programId: 'prog-1',
    status: 'active',
...
describe('DiscoveryTaskCompletionService', () => {
  ...
  describe('hasTaskPassed', () => {
    it('should return true when task has passing interaction', () => {
      const task = createTestTask({
        interactions: [
          ...
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: true } },
        ]
      });
    ...
    it('should return false when no passing interaction exists', () => {
      const task = createTestTask({
        interactions: [
          { timestamp: '2024-01-01', type: 'user_input', content: 'response' },
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: false } },
        ]
      });
  ...
  describe('completeTaskWithEvaluation', () => {
    it('should create evaluation and update task on successful completion', async () => {
      const task = createTestTask({
        metadata: {},
        interactions: [
          { timestamp: '2024-01-01', type: 'user_input', content: 'response 1' },
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: true, xpEarned: 80 }, metadata: { skillsImproved: ['skill1'] } },
          { timestamp: '2024-01-01', type: 'user_input', content: 'response 2' },
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: false, xpEarned: 50 } },
        ]
      });
      ...
      (FeedbackGenerationService.generateComprehensiveFeedback as jest.Mock).mockResolvedValue({
      ...
      const result = await DiscoveryTaskCompletionService.completeTaskWithEvaluation(
        task,
        program,
        userId,
        userLanguage,
    ...
    it('should use fallback feedback on error', async () => {
      const task = createTestTask({
        interactions: [
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: true, xpEarned: 100  } }
        ]
      });

      (FeedbackGenerationService.generateComprehensiveFeedback as jest.Mock).mockRejectedValue(
        new Error('AI service error')
      );
      (FeedbackGenerationService.getFallbackMessage as jest.Mock).mockReturnValue(
      ...
      expect(FeedbackGenerationService.getFallbackMessage).toHaveBeenCalledWith('en');
    });
  });
  ...
  describe('extractSkillsImproved', () => {
    it('should extract unique skills from interactions', () => {
      const task = createTestTask({
        interactions: [
          { timestamp: '2024-01-01', type: 'ai_response', content: { skillsImproved: ['coding', 'debugging']  } },
          { timestamp: '2024-01-01', type: 'ai_response', content: { skillsImproved: ['debugging', 'testing']  } },
  ...
  describe('findBestXPEarned', () => {
    it('should find maximum XP from passed attempts', () => {
      const task = createTestTask({
        interactions: [
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: true, xpEarned: 60  } },
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: true, xpEarned: 85  } },
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: false, xpEarned: 95  } }, // Not passed, shouldn't count
          { timestamp: '2024-01-01', type: 'ai_response', content: { completed: true, xpEarned: 70  } },
        ]
      });
