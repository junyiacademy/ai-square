...
const withEnv = (vars: Record<string, string>, fn: () => Promise<void> | void) => {
...
describe('production-monitor', () => {
  ...
  it('is disabled in non-production (no intervals started)', async () => {
    await withEnv({ NODE_ENV: 'test' }, async () => {
      await (async () => {
        ...
        jest.doMock('../performance-monitor', () => ({ performanceMonitor: { getAllMetrics: jest.fn(() => []) } }))
        jest.doMock('@/lib/cache/distributed-cache-service', () => ({ distributedCacheService: { getStats: jest.fn(async () => ({ hitRate: 0 })) } }))
        const mod = require('../production-monitor') as typeof import('../production-monitor')
    ...
    await withEnv({ NODE_ENV: 'production' }, async () => {
      ...
      await (async () => {
        ...
        jest.doMock('@/lib/cache/distributed-cache-service', () => ({
          distributedCacheService: { getStats: jest.fn(async () => ({ hitRate: 70, localCacheSize: 10 })) }
        }))
        ...
        const call = logSpy.mock.calls.find(c => String(c[0]).includes('Performance Metrics:'))
  ...
  it('triggers alerts on thresholds, applies cooldown, and sends webhook', async () => {
    await withEnv({
      ...
      await (async () => {
        ...
        jest.doMock('@/lib/cache/distributed-cache-service', () => ({
          distributedCacheService: { getStats: jest.fn(async () => ({ hitRate: 50, redisStats: { redisConnected: false }, localCacheSize: 99 })) }
        }))
        ...
        await jest.advanceTimersByTimeAsync(30 * 1000)
        await Promise.resolve()
        const after = errorSpy.mock.calls.filter(c => String(c[0]).includes('🚨 ALERT')).length
        ...
        await Promise.resolve()
        const finalCount = errorSpy.mock.calls.filter(c => String(c[0]).includes('🚨 ALERT')).length
  ...
  it('handles webhook send failures gracefully', async () => {
    await withEnv({ NODE_ENV: 'production', ALERT_WEBHOOK_URL: 'https://example.com/webhook' }, async () => {
      ;(global as any).fetch = jest.fn().mockImplementationOnce(() => Promise.reject('net-down'))
      const errorSpy = jest.spyOn(console, 'error').mockImplementation(() => undefined)
      ...
      await (async () => {
        ...
        jest.doMock('@/lib/cache/distributed-cache-service', () => ({
          distributedCacheService: { getStats: jest.fn(async () => ({ hitRate: 100 })) }
        }))

        const mod = require('../production-monitor') as typeof import('../production-monitor')
        // call private sendAlert via type assertion to trigger webhook path
        ;(mod.productionMonitor as unknown as { sendAlert: (t: string, d: Record<string, unknown>) => void }).sendAlert('test', { a: 1 })
        })
      })()

      // should log webhook failure
      expect(errorSpy).toHaveBeenCalled()
      const logged = errorSpy.mock.calls.some(c => String(c[0]).includes('Failed to send webhook alert'))
