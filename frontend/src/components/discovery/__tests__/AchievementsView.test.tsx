...
describe("AchievementsView", () => {
  const testTranslations: Record<string, Record<string, string>> = {
    discovery: {
      ...
      "achievements.badges.creative_thinker.description":
      ...
      "achievements.badges.problem_solver.description":
        "Solved complex problems",
      "achievements.badges.path_explorer.title": "Path Explorer",
      "achievements.badges.path_explorer.description":
        "Explored different paths",
    },
  };
  ...
  beforeEach(() => {
    ...
    useTranslation.mockReturnValue(
      mockUseTranslation(testTranslations, "discovery"),
    );
  });
  ...
  it("should render without crashing", () => {
    ...
    const element =
      screen.queryByText("Your Achievements") ||
      screen.queryByText(/achievements/i) ||
  ...
  it("should render earned badges correctly", () => {
    ...
    expect(
      getBadgeByCategory("Creative Thinker", "earned"),
    ).toBeInTheDocument();
  ...
  it("should render available badges correctly", () => {
    ...
    expect(
      getBadgeByCategory("AI Collaborator", "available"),
    ).toBeInTheDocument();
    expect(
      getBadgeByCategory("Problem Solver", "available"),
    ).toBeInTheDocument();
    expect(
      getBadgeByCategory("Path Explorer", "available"),
    ).toBeInTheDocument();
  ...
  it("should not show level benefits when level = 1", () => {
    ...
    renderWithProviders(
      <AchievementsView achievements={lowLevelAchievements} />,
    );
  ...
  it("should show empty state when no badges earned", () => {
    ...
    renderWithProviders(
      <AchievementsView achievements={noBadgesAchievements} />,
    );

    expect(screen.getByText("還沒有獲得任何徽章")).toBeInTheDocument();
    expect(
      screen.getByText("完成任務來獲得你的第一個徽章！"),
    ).toBeInTheDocument();
  });
  ...
  it("should render all available badge types", () => {
    ...
    renderWithProviders(
      <AchievementsView achievements={allBadgesAchievements} />,
    );
  ...
  it("should handle high level benefits correctly", () => {
    ...
    renderWithProviders(
      <AchievementsView achievements={highLevelAchievements} />,
    );
  ...
  it("should handle missing badge IDs gracefully", () => {
      ...
      badges: [
        {
          ...
          name: "Nonexistent Badge",
          description: "Badge that does not exist",
          unlockedAt: "2024-01-01",
    ...
    renderWithProviders(
      <AchievementsView achievements={invalidBadgeAchievements} />,
    );
  ...
  it("should render with proper structure and CSS classes", () => {
    const { container } = renderWithProviders(
      <AchievementsView achievements={mockAchievements} />,
    );
