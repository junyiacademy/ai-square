...
jest.mock("@/lib/dynamic-imports", () => ({
  DynamicDomainRadarChart: ({ data }: any) => (
    <div
      data-testid="domain-radar-chart"
...
jest.mock("../CompetencyKnowledgeGraph", () => ({
  __esModule: true,
  default: () => (
    <div data-testid="competency-knowledge-graph">Knowledge Graph</div>
  ),
}));
...
describe("AssessmentResults", () => {
  ...
  it("switches between tabs correctly", async () => {
    ...
    expect(
      screen.getByTestId("competency-knowledge-graph"),
    ).toBeInTheDocument();
  });
  ...
  it("displays summary text correctly", async () => {
    ...
    expect(
      screen.getByText(/You achieved Expert level with 85% overall score/),
    ).toBeInTheDocument();
  });
