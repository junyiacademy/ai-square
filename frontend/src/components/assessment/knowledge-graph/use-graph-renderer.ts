...
function getNodeRadius(node: GraphNode): number {
  switch (node.type) {
    case "domain":
      return node.id === "center" ? NODE_RADII.center : NODE_RADII.domain;
    case "competency":
      return NODE_RADII.competency;
    case "ksa-theme":
      return NODE_RADII.ksaTheme;
    case "ksa-code":
      return NODE_RADII.ksaCode;
    case "ksa-subcode":
      return NODE_RADII.ksaSubcode;
    default:
...
function getTrafficLightColor(mastery: number): string {
  switch (mastery) {
    ...
    default:
      return COLORS.trafficLight.gray;
  }
}

function getNodeFill(node: GraphNode): string {
  if (
    (node.type === "ksa-code" || node.type === "ksa-subcode") &&
  ...
  switch (node.ksaType) {
    case "knowledge":
      return COLORS.ksa.knowledge;
    case "skills":
      return COLORS.ksa.skills;
    case "attitudes":
      return COLORS.ksa.attitudes;
    default:
      return COLORS.ksa.default;
  }
}
...
export function useGraphRenderer(
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3
          ...
          .distance((d) => {
            ...
            if (target.type === "ksa-subcode")
              return FORCE_CONFIG.linkDistance.ksaSubcode;
            if (target.type === "ksa-code")
              return FORCE_CONFIG.linkDistance.ksaCode;
            if (target.type === "ksa-theme")
              return FORCE_CONFIG.linkDistance.ksaTheme;
      ...
      .force(
        ...
        d3.forceManyBody<GraphNode>().strength((d) => {
          if (d.type === "ksa-subcode")
            return FORCE_CONFIG.chargeStrength.ksaSubcode;
          if (d.type === "ksa-code") return FORCE_CONFIG.chargeStrength.ksaCode;
          if (d.type === "ksa-theme")
            return FORCE_CONFIG.chargeStrength.ksaTheme;
      ...
      .force(
        ...
        d3
          .forceRadial<GraphNode>(
            (d) => {
              if (d.type === "ksa-theme")
                return FORCE_CONFIG.radialDistance.ksaTheme;
              if (d.type === "ksa-code")
                return FORCE_CONFIG.radialDistance.ksaCode;
              if (d.type === "ksa-subcode")
                return FORCE_CONFIG.radialDistance.ksaSubcode;
              return 0;
            },
            dimensions.width / 2,
            dimensions.height / 2,
    ...
    const dragBehavior = d3
      ...
      .on("start", (event, d) => {
        if (!event.active)
          simulation.alphaTarget(ANIMATION.alphaTarget).restart();
        d.fx = d.x;
        d.fy = d.y;
      })
      .on("drag", (event, d) => {
    ...
    node
      ...
      .attr("stroke", (d) =>
        selectedNode?.id === d.id
          ? COLORS.stroke.selected
          : COLORS.stroke.normal,
      )
    ...
    node
      .on("mouseover", (event, d) => {
        tooltip
          .transition()
          .duration(ANIMATION.tooltipDelay)
          .style("opacity", 0.9);
      ...
      .on("mouseout", () => {
        tooltip
          .transition()
          .duration(ANIMATION.tooltipFadeout)
          .style("opacity", 0);
      });
    ...
    simulation.on("tick", () => {
      ...
      node.attr(
        "transform",
        (d) =>
          `translate(${(d as GraphNode & { x: number }).x},${(d as GraphNode & { y: number }).y})`,
      );
    });
