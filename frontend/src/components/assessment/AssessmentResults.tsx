...
export default function AssessmentResults({
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "recommendations" | "ksa" | "knowledge-graph"
  >("overview");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        console.log(
          "🔄 Fetching relations tree data for language:",
          i18n.language,
        );
        ...
        setKsaMaps({
          kMap: data.kMap as Record<
            string,
            { summary: string; theme: string; explanation?: string }
          >,
          sMap: data.sMap as Record<
            string,
            { summary: string; theme: string; explanation?: string }
          >,
          aMap: data.aMap as Record<
            string,
            { summary: string; theme: string; explanation?: string }
          >,
        });
  ...
  const radarData: RadarChartData[] = Object.entries(result.domainScores).map(
    ([domain, score]) => ({
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {Object.entries(result.domainScores).map(
                      ([domainKey, score]) => {
                        const domain =
                          domains[domainKey as keyof typeof domains];
                        return (
                          <div
                            key={domainKey}
