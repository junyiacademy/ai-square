...
export default function CompetencyKnowledgeGraph({
  ...
  const calculateKSAMastery = useCallback(() => {
    ...
    const ksaMastery: Record<
      string,
      { correct: number; total: number; questions: string[] }
    > = {};
    ...
    console.log("✅ KSA mastery calculation complete:", {
      ...
      codeDetails: Object.entries(ksaMastery)
        .slice(0, 5)
        .map(([code, data]) => ({
          code,
  ...
  const buildGraphData = useCallback(() => {
    ...
    if (result.ksaAnalysis) {
      console.log(
        "📊 Using KSA analysis from evaluation result:",
        result.ksaAnalysis,
      );
    ...
    parentCodes.forEach((subcodes, parentCode) => {
      ...
      if (!ksaInfo) {
        console.log(
          `⚠️ No KSA info found for ${parentCode}, skipping (unmapped code)`,
        );
    ...
    Object.entries(ksaMastery).forEach(([code, data]) => {
      ...
      if (!ksaInfo) {
        console.log(
          `⚠️ No KSA info found for ${code}, skipping (unmapped code)`,
        );
        return; // Skip processing this code
      }
      ...
      if (isSubcode && parentCode) {
        ...
        const parentNodeExists = nodes.some(
          (n) => n.id === `code-${parentCode}`,
        );
        if (parentNodeExists) {
  ...
  const getTrafficLightColor = (mastery: number): string => {
    switch (mastery) {
      case 0:
        return "#ef4444"; // Red - completely wrong
      case 1:
        return "#f59e0b"; // Yellow - partially correct
      case 2:
        return "#10b981"; // Green - all correct
      default:
  ...
  useEffect(() => {
    console.log(
      "🎯 CompetencyKnowledgeGraph useEffect - Data availability check:",
      {
    ...
    if (!svgRef.current) {
      console.log(
        "⚠️ CompetencyKnowledgeGraph: No SVG ref, graph will not render",
      );
      return;
    }
    ...
    if (!domainsData || !ksaMaps) {
      console.log(
        "⚠️ CompetencyKnowledgeGraph: Missing KSA maps data, using simplified display",
      );
    }
    ...
    const simulation = d3
      ...
      .force(
        "collision",
        d3.forceCollide<GraphNode>().radius((d) => getNodeRadius(d) + 20),
      )
      .force(
        "radial",
        d3
    ...
    node
      ...
      .attr("fill", (d) => {
        ...
        if (
          (d.type === "ksa-code" || d.type === "ksa-subcode") &&
          d.mastery !== undefined
        ) {
    ...
    node
      .append("text")
      .attr("dy", (d) =>
        d.type === "domain" && d.id === "center" ? ".35em" : "-.8em",
      )
      .attr("text-anchor", "middle")
      .style("font-size", (d) => {
        switch (d.type) {
          case "domain":
            return d.id === "center" ? "16px" : "14px";
          case "competency":
            return "13px";
          case "ksa-theme":
            return "14px";
      ...
      .style("font-weight", (d) =>
        d.type === "domain" || d.type === "ksa-theme" ? "bold" : "normal",
      )
      .style("fill", (d) => {
    ...
    simulation.on("tick", () => {
      link
        ...
        .attr(
          "y2",
          (d) => (d.target as GraphNode & { x: number; y: number }).y,
        );

      node.attr(
        "transform",
        (d) =>
          `translate(${(d as GraphNode & { x: number; y: number }).x},${(d as GraphNode & { x: number; y: number }).y})`,
      );
    });
    ...
    if (selectedNode) {
      d3.select(svgRef.current)
        .selectAll("circle")
        .attr("stroke", (d) =>
          (d as GraphNode).id === selectedNode.id ? "#4f46e5" : "#fff",
        )
        .attr("stroke-width", (d) =>
          (d as GraphNode).id === selectedNode.id ? 4 : 2,
        );
    }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4">
      ...
      <div
        className={`${showQuestionReview ? "lg:w-1/2" : "w-full"} transition-all duration-300`}
      >
        <div className="mb-4">
        ...
        <div
          className="bg-gray-50 rounded-lg overflow-hidden touch-none"
          style={{ height: "900px" }}
        >
        ...
        {selectedNode && (
          <div className="mt-4 p-4 bg-white rounded-lg shadow-md">
            ...
            {(selectedNode.type === "ksa-code" ||
              selectedNode.type === "ksa-subcode") &&
              selectedNode.details && (
                <div className="mb-3">
                  ...
                  <div className="text-sm text-gray-600">
                    答對: {selectedNode.details.correct}/
                    {selectedNode.details.total} 題
                  </div>
                  {selectedNode.details.questions &&
                    selectedNode.details.questions.length > 0 && (
