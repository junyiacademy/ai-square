...
describe("Footer", () => {
  it("renders footer with all sections", async () => {
    ...
    // Check description
    expect(
      screen.getByText(/AI Square is a Git-Based learning platform/),
    ).toBeInTheDocument();

    // Check stats - text is combined in single elements
    expect(
      screen.getByText("Languages: English / 繁體中文 / 简体中文"),
    ).toBeInTheDocument();
  ...
  it("renders footer tagline", async () => {
    ...
    expect(
      screen.getByText("Made with ❤️ for AI literacy education"),
    ).toBeInTheDocument();
  });
});
