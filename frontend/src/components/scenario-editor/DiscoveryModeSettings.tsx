    ...
    <div className="bg-white rounded-lg shadow">
      ...
      {isExpanded && (
        <div className="px-4 pb-4 border-t border-gray-100 space-y-4 mt-3">
          ...
          {discoveryData?.careerInfo && (
            <div>
              ...
              <div className="bg-gray-50 rounded-lg p-3 space-y-2">
                ...
                {discoveryData.careerInfo.requiredSkills &&
                  discoveryData.careerInfo.requiredSkills.length > 0 && (
                    <div>
                      <span className="text-xs font-medium text-gray-600 block mb-1">
                        所需技能:{" "}
                      </span>
                      <div className="flex flex-wrap gap-1">
                        {discoveryData.careerInfo.requiredSkills.map(
          ...
          {discoveryData?.skillTree && (
            <div>
              ...
              <div className="space-y-2">
                {discoveryData.skillTree.core &&
                  discoveryData.skillTree.core.length > 0 && (
                    <div className="bg-blue-50 rounded-lg p-3">
                      <span className="text-xs font-medium text-blue-700 block mb-1">
                        Core Skills:
                      </span>
                      <div className="flex flex-wrap gap-1">
                        {discoveryData.skillTree.core.map(
                          (skill: string, i: number) => (
                            <span
                              key={i}
                ...
                {discoveryData.skillTree.advanced &&
                  discoveryData.skillTree.advanced.length > 0 && (
                    <div className="bg-purple-50 rounded-lg p-3">
                      <span className="text-xs font-medium text-purple-700 block mb-1">
                        Advanced Skills:
                      </span>
                      <div className="flex flex-wrap gap-1">
                        {discoveryData.skillTree.advanced.map(
                          (skill: string, i: number) => (
                            <span
                              key={i}
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg p-3 grid grid-cols-3 gap-3">
                {discoveryData.xpRewards.completion !== undefined && (
                  <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-orange-600">
                      {discoveryData.xpRewards.completion} XP
                    </div>
                  </div>
                )}
                {discoveryData.xpRewards.challenge !== undefined && (
                  <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-orange-600">
                      {discoveryData.xpRewards.challenge} XP
                    </div>
                  </div>
                )}
                {discoveryData.xpRewards.innovation !== undefined && (
                  <div className="text-center">
                    ...
                    <div className="text-lg font-bold text-orange-600">
                      {discoveryData.xpRewards.innovation} XP
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Exploration Path */}
          {discoveryData?.explorationPath &&
            discoveryData.explorationPath.length > 0 && (
              <div>
                <label className="text-sm font-medium text-gray-700 block mb-2">
                  Exploration Path - 探索路徑
                </label>
                <div className="space-y-1">
                  {discoveryData.explorationPath.map(
