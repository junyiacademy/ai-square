...
const createMockT = (): TFunction<readonly ["pbl", "common"], undefined> => {
  const mockFn = ((key: string) => key) as TFunction<
    readonly ["pbl", "common"],
    undefined
  >;
  return mockFn;
...
describe("TaskProgressSidebar", () => {
  ...
  it("renders progress sidebar with all tasks", () => {
    ...
    const taskButtons = container.querySelectorAll(
  ...
  it("shows collapsed view when isCollapsed is true", () => {
    ...
    expect(container.querySelectorAll("button[title]").length).toBeGreaterThan(
      0,
    );
  });
  ...
  it("does not show View Report link when collapsed", () => {
    ...
    expect(
      screen.queryByText("pbl:complete.viewReport"),
