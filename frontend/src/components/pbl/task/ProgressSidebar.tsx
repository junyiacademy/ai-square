  ...
  return (
    <div className={`bg-white dark:bg-gray-800 shadow-lg transition-all duration-300 relative ${
      ...
      <div className="h-full flex flex-col">
        <div className={`flex items-center justify-between ${isCollapsed ? 'px-2 py-4' : 'p-4'}`}>
                    ...
                    >
                      ...
                      <div className="ml-4 flex-1">
                        <p className={`text-sm font-medium ${
                          ...
                          {getLocalizedField(task as unknown as Record<string, unknown>, 'title', language)}
                        </p>
              ...
              {scenario.tasks.map((task, index) => {
                ...
                return (
                  <button
                    ...
                    title={`${getLocalizedField(task as unknown as Record<string, unknown>, 'title', language)}${isEvaluated && taskEval?.score !== undefined ? ` - ${t(getQualitativeRating(taskEval.score).i18nKey)}` : ''}`}
                  >
                    {isEvaluated ? (
                      <svg className="h-5 w-5 text-green-600 dark:text-green-500" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
          ...
          {!isCollapsed && Object.keys(taskEvaluations).length > 0 && (
            <div className="mt-6 px-4">
              ...
              >
                ...
                {t('complete.viewReport', 'View Report')}
              </Link>
            </div>
          )}
        </div>
