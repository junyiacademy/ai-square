...
describe("KSARadarChart", () => {
  ...
  it("passes correct data structure to chart", async () => {
    ...
    const knowledgeDataset = chartData.datasets.find(
      (d: any) => d.label === "Knowledge",
    );
  ...
  it("configures chart options correctly", async () => {
    ...
    const chartOptions = JSON.parse(
      radarChart.getAttribute("data-chart-options") || "{}",
    );
  ...
  it("groups scores by category correctly", async () => {
    ...
    // Find skills dataset
    const skillsDataset = chartData.datasets.find(
      (d: any) => d.label === "Skills",
    );

    // Skills should have scores for S1, S2, S3 and 0 for knowledge/attitude indices
    const nonZeroScores = skillsDataset.data.filter(
