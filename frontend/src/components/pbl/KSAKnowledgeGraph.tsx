...
export default function KSAKnowledgeGraph({
  ksaScores,
  title,
  ksaMapping,
}: KSAKnowledgeGraphProps) {
  ...
  useEffect(() => {
    ...
    const simulation = d3
      ...
      .force(
        "collision",
        d3.forceCollide().radius(Math.min(width, height) * 0.08),
      );
    ...
    node
      ...
      .attr("font-weight", (d) =>
        d.id === "center" || d.category === "center" ? "bold" : "normal",
      )
    ...
    function dragended(
      ...
      if (
        d.id !== "center" &&
        !["knowledge", "skills", "attitudes"].includes(d.id)
      ) {
        d.fx = null;
        d.fy = null;
      }
          ...
          {selectedNode && selectedNode.score !== undefined ? (
            <div className="p-6 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 h-full">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    {selectedNode.category === "knowledge" &&
                      t("ksa:knowledgeComponent")}
                    {selectedNode.category === "skills" &&
                      t("ksa:skillsComponent")}
                    {selectedNode.category === "attitudes" &&
                      t("ksa:attitudesComponent")}
                  </p>
                </div>
                <button
              ...
              <div className="flex items-center gap-4 mb-6">
                <div
                  className={`text-5xl font-bold ${getScoreColor(selectedNode.score)}`}
                >
                  {selectedNode.score}%
                </div>
                <div
                  className={`px-4 py-2 rounded-full text-sm font-medium ${getScoreStatus(selectedNode.score).color}`}
                >
                  <span className="mr-1 text-lg">
                    {getScoreStatus(selectedNode.score).icon}
                  </span>
                ...
                {selectedNode.score < 80 && (
                  <div className="mt-4 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-700">
                    <p className="text-sm text-amber-800 dark:text-amber-200 flex items-start gap-2">
                      ...
                      <span>
                        Consider practicing more scenarios that focus on this
                        competency to improve your score.
                      </span>
                    </p>
                  </div>
                )}
                ...
                {selectedNode.score >= 80 && (
                  <div className="mt-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-700">
                    <p className="text-sm text-green-800 dark:text-green-200 flex items-start gap-2">
                      ...
                      <span>
                        Excellent performance! You have demonstrated strong
