...
export function LoginForm({
  ...
  // 檢查是否應該顯示示範帳號（只在 localhost 和 staging 顯示）
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "";
  const isLocalhost =
    appUrl.includes("localhost") || appUrl.includes("127.0.0.1");
  const isStaging =
    appUrl.includes("staging") || appUrl.includes("aisquare-staging");
  const shouldShowDemoAccounts = isLocalhost || isStaging;

  // Placeholder 根據環境決定
  const emailPlaceholder = shouldShowDemoAccounts
    ? "student@example.com"
    : "your-email@example.com";
  const passwordPlaceholder = shouldShowDemoAccounts
    ? "student123"
    : "••••••••";
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 分隔線與測試帳戶（僅在非 staging/production 環境顯示） */}
