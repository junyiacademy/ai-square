  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xl font-bold">
            {language === 'zhTW' ? '能力知識圖譜' : 'Competency Knowledge Graph'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            <div className="grid gap-3">
              {domains.map(([domain, score]) => (
                <div key={domain} className="border rounded-lg p-3 hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-center mb-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xl font-bold">
            {language === 'zhTW' ? '題目回顧與做題紀錄' : 'Question Review & Practice Records'}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
              ...
              >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-3">
                    ...
                    <div className="bg-gray-50 dark:bg-gray-800 rounded p-2">
                      ...
                      <p className="font-bold text-lg">
                        {task.timeSpent ? `${Math.floor(task.timeSpent / 60)}m ${task.timeSpent % 60}s` : '--'}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {task.interactions && task.interactions.length > 0 && (
                  <details className="mt-3">
                    <summary className="cursor-pointer text-sm font-medium text-purple-600 hover:text-purple-700">
                      {language === 'zhTW' ? '查看做題紀錄' : 'View Practice Records'}
                      ({task.interactions.length} {language === 'zhTW' ? '次互動' : 'interactions'})
                    </summary>
                        ...
                        >
                          <div className="flex justify-between items-start mb-1">
                            <span className="font-medium text-xs text-gray-500">
                              {interaction.type === 'user_input'
                                ? (language === 'zhTW' ? '你的回答' : 'Your Answer')
                                : (language === 'zhTW' ? 'AI 回饋' : 'AI Feedback')}
                            </span>
                ...
                {task.status === 'completed' && task.userAnswer && (
                  <div className="mt-3 grid gap-2">
                    ...
                    {task.correctAnswer && task.correctAnswer !== task.userAnswer && (
                      <div className="bg-green-50 dark:bg-green-900/20 p-3 rounded">
                        <span className="text-xs font-medium text-green-700 dark:text-green-300">
                          {language === 'zhTW' ? '正確答案：' : 'Correct Answer:'}
                        </span>
