      ...
      try {
        ...
        try {
          ...
          if (pblResponse.ok) {
            ...
            if (pblData.success && pblData.programs) {
              pblItems = pblData.programs.map((program: Record<string, unknown>) => {
                const currentTask = (program.tasks as Array<{ id?: string; title?: string }>)?.[program.currentTaskIndex as number || 0];
                return {
                  type: 'pbl' as const,
                  timestamp: (program.completedAt as string) || (program.startedAt as string) || (program.program as { startedAt: string })?.startedAt,
                  data: {
                    id: (program.programId as string) || (program.id as string),
                    logId: (program.programId as string) || (program.id as string),
                    scenarioId: program.scenarioId as string,
                    scenarioTitle: program.scenarioTitle as string || 'PBL Scenario',
                    currentTaskId: currentTask?.id,
                    currentTaskTitle: currentTask?.title,
                    status: program.status as 'completed' | 'in_progress' | 'paused',
                    startedAt: (program.startedAt as string) || (program.program as { startedAt: string })?.startedAt,
                    completedAt: program.completedAt as string,
                    duration: (program.totalTimeSeconds as number) || 0,
                    progress: {
                      percentage: Math.round(((program.evaluatedTasks as number) || 0) / ((program.totalTaskCount as number) || 1) * 100),
                      completedTasks: (program.evaluatedTasks as number) || 0,
                      totalTaskCount: (program.totalTaskCount as number) || 0,
                    },
                    totalInteractions: (program.tasks as Array<{ log?: { interactions?: unknown[] } }>)?.reduce((sum, task) => sum + ((task.log?.interactions?.length || 0)), 0) || 0,
                    averageScore: program.overallScore as number,
                    domainScores: program.domainScores as PBLSession['domainScores'],
        ...
        try {
          ...
          if (discoveryResponse.ok) {
            ...
            if (Array.isArray(discoveryScenarios) && discoveryScenarios.length > 0) {
              for (const scenario of discoveryScenarios) {
                ...
                if (programsResponse.ok) {
                  ...
                  const scenarioItems = programs.map((program: Record<string, unknown>) => {
                    ...
                    const currentTask = { id: currentTaskId, title: 'Current Task' };

                    let duration = 0;
                    if (isCompleted && program.completedAt && program.startedAt) {
                    ...
                    return {
                      ...
                      timestamp: (program.completedAt as string) || (program.startedAt as string) || new Date().toISOString(),
                      data: {
                        id: scenario.id,
                        programId: program.id as string,
                        scenarioId: scenario.id,
                        scenarioTitle: scenario.title as string || 'Discovery Path',
                        careerType: (scenario.metadata as { careerType?: string })?.careerType || 'unknown',
                        currentTaskId: currentTask.id as string,
                        currentTaskTitle: currentTask.title,
                        status: program.status as 'completed' | 'active' | 'inactive',
