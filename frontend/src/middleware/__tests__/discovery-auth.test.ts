  ...
  describe("checkDiscoveryAuth", () => {
    ...
    it("should redirect unauthenticated requests to login", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/discovery/scenarios",
      );
      const response = await checkDiscoveryAuth(request);
      ...
      expect(response?.headers.get("location")).toContain(
        "redirect=%2Fdiscovery%2Fscenarios",
      );
    });
    ...
    it("should preserve redirect parameter with full path", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/discovery/scenarios/123/program/456",
      );
      const response = await checkDiscoveryAuth(request);

      expect(response?.headers.get("location")).toContain(
        "redirect=%2Fdiscovery%2Fscenarios%2F123%2Fprogram%2F456",
      );
    });
    ...
    it("should handle paths with query parameters", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/discovery/scenarios?filter=active&sort=date",
      );
      const response = await checkDiscoveryAuth(request);
    ...
    it("should handle paths with hash fragments", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/discovery#section",
      );
      const response = await checkDiscoveryAuth(request);

      expect(response).toBeTruthy();
      expect(response?.headers.get("location")).toContain(
        "redirect=%2Fdiscovery",
      );
    });
  });
});
