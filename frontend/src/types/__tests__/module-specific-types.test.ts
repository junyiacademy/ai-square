...
describe("Module-specific Types", () => {
  ...
  describe("Assessment Types", () => {
    it("should create valid Assessment scenario", () => {
      ...
      expect(assessmentScenario.assessmentData.assessmentType).toBe(
        "diagnostic",
      );
    ...
    it("should create valid Assessment task", () => {
      ...
      expect(
        (assessmentTask.content.context as Record<string, unknown>).questions,
      ).toHaveLength(1);
      expect(
        (assessmentTask.content.context as Record<string, unknown>).timeLimit,
      ).toBe(1800);
    });
    ...
    it("should create valid Assessment evaluation", () => {
      const assessmentEval: IAssessmentEvaluation = {
        ...
        assessmentData: {
          totalQuestions: 20,
          correctAnswers: 15,
          questionResults: [{ questionId: "q1", correct: true, timeSpent: 60 }],
          domainScores: {
            Engaging_with_AI: 80,
            Creating_with_AI: 70,
          },
