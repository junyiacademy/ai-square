...
describe('PBL Evaluate Types', () => {
  describe('Conversation interface', () => {
    it('should define user conversation structure', () => {
      const userConversation: Conversation = {
        type: 'user',
        content: 'I think AI systems should prioritize fairness over efficiency in most cases.',
    ...
    it('should define assistant conversation structure', () => {
      const assistantConversation: Conversation = {
        type: 'assistant',
        content: 'That\'s an interesting perspective. Can you provide a specific example?',
  ...
  describe('EvaluateRequestBody interface', () => {
    it('should define complete evaluation request structure', () => {
      ...
      const evaluateRequest: EvaluateRequestBody = {
        conversations: [
          {
            type: 'user',
            content: 'I believe we should consider all stakeholders when making AI decisions.',
            timestamp: '2024-01-01T12:00:00Z'
          },
          {
            type: 'assistant',
            content: 'Excellent point! Which stakeholders do you think are most important?',
            timestamp: '2024-01-01T12:00:30Z'
          },
          {
            type: 'user',
            content: 'End users, developers, and society at large should all be considered.',
    ...
    it('should handle various KSA codes', () => {
      const requestWithKSACodes: EvaluateRequestBody = {
        conversations: [
          { type: 'user', content: 'Test conversation' }
        ],
        task: { id: 'task-999' } as any,
        focusKSA: [
          'K1.1', 'K2.2', 'K3.1', // Knowledge codes
          'S1.2', 'S2.1', 'S3.3', // Skills codes
          'A1.1', 'A2.3', 'A3.2'  // Attitudes codes
        ]
      };

      expect(requestWithKSACodes.focusKSA).toHaveLength(9);
      expect(requestWithKSACodes.focusKSA?.filter(code => code.startsWith('K'))).toHaveLength(3);
      expect(requestWithKSACodes.focusKSA?.filter(code => code.startsWith('S'))).toHaveLength(3);
      expect(requestWithKSACodes.focusKSA?.filter(code => code.startsWith('A'))).toHaveLength(3);
    });
  });
  ...
  describe('EvaluationResult interface', () => {
    it('should define complete evaluation result structure', () => {
      const evaluationResult: EvaluationResult = {
        score: 85,
        feedback: 'Your analysis demonstrates strong ethical reasoning and comprehensive stakeholder consideration.',
        detailedScores: {
          relevance: 88,
          depth: 82,
          criticalThinking: 90,
        ...
        overallAssessment: 'You have demonstrated strong analytical skills and ethical awareness throughout this task.'
      };
    ...
    it('should define successful evaluation response', () => {
      const successResponse: EvaluateResponse = {
        ...
        evaluation: {
          ...
          overallAssessment: 'Outstanding work demonstrating deep understanding.'
        }
      };
  ...
  describe('Type validation and edge cases', () => {
    it('should handle various conversation types', () => {
      ...
      expect(conversations.filter(c => c.type === 'assistant')).toHaveLength(2);
    });
    ...
    it('should handle multi-language evaluation contexts', () => {
      ...
      expect(multiLanguageRequest.conversations[0].content).toContain('English');
    });
  });
