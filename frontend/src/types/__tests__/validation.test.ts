...
describe("Validation Types", () => {
  ...
  describe("KSAData interface", () => {
    it("should define complete KSA data structure", () => {
      ...
      expect(Object.keys(ksaData.knowledge_codes?.themes || {})).toHaveLength(
        2,
      );
      expect(Object.keys(ksaData.knowledge_codes?.codes || {})).toHaveLength(2);
      expect(ksaData.skills_codes?.codes?.["S1.1"].name).toBe(
        "Critical Analysis",
      );
  ...
  describe("DomainsData interface", () => {
    it("should define domains data structure", () => {
      ...
      expect(domainsData.domains?.["engaging_with_ai"].name).toBe(
        "Engaging with AI",
      );
      expect(domainsData.domains?.["creating_with_ai"].name_zhTW).toBe(
        "與AI創造",
      );
    });
  });
  ...
  describe("PBLTask interface", () => {
    it("should define PBL task structure", () => {
      const pblTask: PBLTask = {
        ...
        description:
          "Identify and analyze stakeholders in the AI healthcare scenario.",
