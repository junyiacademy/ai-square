...
describe("bundle-optimization config", () => {
  ...
  describe("LAZY_ROUTES", () => {
    it("contains expected lazy-loaded routes", async () => {
      expect(LAZY_ROUTES).toEqual(["/pbl/history", "/admin", "/settings"]);
    });
  ...
  describe("PRELOAD_RESOURCES", () => {
    it("contains critical API endpoints", async () => {
      expect(PRELOAD_RESOURCES).toEqual(["/api/relations", "/api/auth/check"]);
    });
  });
  ...
  describe("SPLIT_CHUNKS", () => {
    ...
    it("vendor name function handles scoped packages", async () => {
      const mockModule = {
        context: "/path/to/node_modules/@tanstack/react-query/index.js",
      };
      const name = SPLIT_CHUNKS.vendor.name(mockModule);
  ...
  describe("SW_CONFIG", () => {
    it("has runtime caching for Google Fonts", async () => {
      ...
      expect(fontCache.options.expiration.maxAgeSeconds).toBe(
        60 * 60 * 24 * 365,
      );
    });
    ...
    it("has runtime caching for images", async () => {
      ...
      expect(imageCache.urlPattern).toEqual(
      ...
      expect(imageCache.options.expiration.maxAgeSeconds).toBe(
        60 * 60 * 24 * 30,
      );
    });
  });
