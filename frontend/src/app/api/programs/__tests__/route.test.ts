...
beforeEach(() => {
  ...
  mockFactory.repositoryFactory.getProgramRepository = jest
    .fn()
  ...
  describe("GET", () => {
    ...
    it("should return programs for specific user", async () => {
      const request = new NextRequest(
        "http://localhost/api/programs?userId=test@example.com",
      );
      const response = await GET(request);
    ...
    it("should return programs for specific scenario", async () => {
      const request = new NextRequest(
        "http://localhost/api/programs?scenarioId=scenario-1",
      );
      const response = await GET(request);
    ...
    it("should filter active programs when status=active", async () => {
      const request = new NextRequest(
        "http://localhost/api/programs?userId=test@example.com&status=active",
      );
      const response = await GET(request);

      expect(response.status).toBe(200);
      expect(mockProgramRepo.getActivePrograms).toHaveBeenCalledWith(
        "test@example.com",
      );
    ...
    it("should filter completed programs when status=completed", async () => {
      const request = new NextRequest(
        "http://localhost/api/programs?userId=test@example.com&status=completed",
      );
      const response = await GET(request);

      expect(response.status).toBe(200);
      expect(mockProgramRepo.getCompletedPrograms).toHaveBeenCalledWith(
        "test@example.com",
      );
