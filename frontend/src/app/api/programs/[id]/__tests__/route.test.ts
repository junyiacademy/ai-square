...
describe("/api/programs/[id] Route", () => {
  ...
  describe("GET /api/programs/[id]", () => {
    it("should handle async params correctly in Next.js 15", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/programs/test-program-id",
      );
      ...
      expect(mockTaskRepo.findByProgram).toHaveBeenCalledWith(
        "test-program-id",
      );
    ...
    it("should return 404 when program not found", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/programs/nonexistent",
      );
  ...
  describe("DELETE /api/programs/[id]", () => {
    it("should handle async params and mark program as abandoned", async () => {
      ...
      expect(mockProgramRepo.update).toHaveBeenCalledWith("test-program-id", {
        status: "abandoned",
      });
