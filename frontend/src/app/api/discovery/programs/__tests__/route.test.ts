...
describe('/api/discovery/programs', () => {
  ...
  describe('POST /api/discovery/programs', () => {
    ...
    it('should return 404 when scenario not found', async () => {
      ...
      expect(mockScenarioRepo.findById).toHaveBeenCalledWith('discovery-scenario-123');
    });
    ...
    it('should return 500 when programRepo.create throws', async () => {
      ...
      mockGetUnifiedAuth.mockResolvedValue({ user: { email: 'test@example.com', id: 'user-123', role: 'student' } });
      mockUserRepo.findByEmail.mockResolvedValue(mockUser);
      mockScenarioRepo.findById.mockResolvedValue(mockScenario);
    ...
    it('should create program when requiredSkills is empty or missing', async () => {
      ...
      mockScenarioRepo.findById.mockResolvedValue({ id: 'discovery-scenario-124', mode: 'discovery', title: { en: 'No Data' } });
      const res2 = await POST(new NextRequest('http://localhost:3000/api/discovery/programs', { method: 'POST', body: JSON.stringify({ scenarioId: 'discovery-scenario-124' }) }));
  ...
  describe('GET /api/discovery/programs', () => {
    ...
    it('should return 404 when user not found', async () => {
      (getUnifiedAuth as jest.Mock).mockResolvedValue({ user: { email: 'u@e.com' } });
      mockUserRepo.findByEmail.mockResolvedValue(null);
      const request = new NextRequest('http://localhost:3000/api/discovery/programs');
    ...
    it('should keep stable order when lastActivityAt ties (by input order)', async () => {
      ...
      mockProgramRepo.findByUser.mockResolvedValue([
        { id: 'p1', userId: 'u1', mode: 'discovery', status: 'active', completedTaskCount: 1, totalTaskCount: 2, lastActivityAt: sameTime, discoveryData: {} },
        { id: 'p2', userId: 'u1', mode: 'discovery', status: 'active', completedTaskCount: 1, totalTaskCount: 2, lastActivityAt: sameTime, discoveryData: {} },
      ]);
      const request = new NextRequest('http://localhost:3000/api/discovery/programs');
      const { GET } = require('../route');
      const res = await GET(request);
