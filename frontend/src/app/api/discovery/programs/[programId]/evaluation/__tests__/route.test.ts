...
describe("/api/discovery/programs/[programId]/evaluation", () => {
  ...
  it("returns 401 when unauthenticated and no userEmail provided", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/discovery/programs/p1/evaluation",
    );
  ...
  it("accepts userEmail query param when no session and returns 200 if program matches", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/discovery/programs/p1/evaluation?userEmail=guest@example.com",
    );
    const res = await GET(req, {
      params: Promise.resolve({ programId: "p1" }),
    });
  ...
  it("returns 404 when program not found or access denied", async () => {
    mockGetUnifiedAuth.mockResolvedValue({
      user: { id: "u1", email: "u@test.com", role: "student" },
    } as any);
    ...
    const tasks = [
      {
        ...
        title: { en: "Task 1" },
        metadata: {
          evaluation: { xpEarned: 80, score: 80, skillsImproved: ["analysis"] },
        },
