...
(repositoryFactory.getProgramRepository as jest.Mock) =
  mockGetProgramRepository;
(repositoryFactory.getTaskRepository as jest.Mock) = mockGetTaskRepository;
(repositoryFactory.getEvaluationRepository as jest.Mock) =
  mockGetEvaluationRepository;
(repositoryFactory.getScenarioRepository as jest.Mock) =
  mockGetScenarioRepository;
(VertexAIService as unknown as jest.Mock).mockImplementation(
  () => mockVertexAIService,
);
(TranslationService as unknown as jest.Mock).mockImplementation(
  ...
  describe("POST", () => {
    ...
    const mockCompletedTasks = [
      ...
      },
      {
        ...
        interactions: [{ type: "user_input", context: { timeSpent: 400 } }],
      },
    ];

    const mockAIFeedback = {
      overallAssessment:
        "The learner demonstrated strong problem-solving skills",
    ...
    it("successfully regenerates evaluation for completed program", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/discovery/programs/program123/regenerate",
        {
          method: "POST",
    ...
    it("handles non-English feedback generation", async () => {
      ...
      mockTranslationService.translateFeedback
        .mockResolvedValueOnce(
          "The learner demonstrated excellent problem-solving skills",
        )
    ...
    it("handles AI feedback generation errors gracefully", async () => {
      ...
      (mockScenarioRepo.findById as jest.Mock).mockResolvedValue(mockScenario);
      mockEvaluationRepo.findByProgram.mockResolvedValue([]);
      mockVertexAIService.sendMessage.mockRejectedValue(
