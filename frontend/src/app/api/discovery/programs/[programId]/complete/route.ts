...
export async function POST(
  ...
  try {
    ...
    completedTasks.forEach((task) => {
      const interactions =
        (task as { interactions?: unknown[] }).interactions || [];
      ...
      const successfulResponses = interactions.filter((i: unknown) => {
        ...
        return (
          ...
          (interaction.context as Record<string, unknown>)?.completed === true
        );
      });

      if (successfulResponses.length > 0) {
        const lastSuccess = successfulResponses[
          successfulResponses.length - 1
        ] as Record<string, unknown>;
        const content =
          typeof lastSuccess.content === "string"
            ? JSON.parse(lastSuccess.content)
            : (lastSuccess.content as Record<string, unknown>);

        const xpEarned = (content?.xpEarned as number) || 0;
        const score =
          (content?.score as number) || (content?.completed ? 100 : 0);
    ...
    const timeSpentSeconds = completedTasks.reduce((sum, task) => {
      ...
      const timeSpent = interactions.reduce((taskTime: number, interaction) => {
        ...
        const time =
          ((i.metadata as Record<string, unknown>)?.timeSpent as number) ||
    ...
    const taskEvaluations = completedTasks.map((task) => {
      ...
      const attempts = interactions.filter(
        (i) => (i as Record<string, unknown>).type === "user_input",
      ).length;
      const aiResponses = interactions.filter(
        (i) => (i as Record<string, unknown>).type === "ai_response",
      );
      const passCount = aiResponses.filter(
        (r) =>
          (r as Record<string, unknown>).context &&
          ((r as Record<string, unknown>).context as Record<string, unknown>)
            ?.completed === true,
      ).length;
      ...
      const successfulResponses = aiResponses.filter((r) => {
        ...
        return (
          response.context &&
          (response.context as Record<string, unknown>)?.completed === true
        );
      });
      if (successfulResponses.length > 0) {
        const lastSuccess = successfulResponses[
