...
describe('/api/discovery/scenarios/[id]/start', () => {
  ...
  describe('Parameter Validation', () => {
    ...
    it('should return 400 for invalid UUID format', async () => {
      ...
      const request = new NextRequest(`http://localhost:3000/api/discovery/scenarios/${invalidId}/start`, {
        method: 'POST',
        body: JSON.stringify({ language: 'en' })
      });
  ...
  describe('Program Creation', () => {
    ...
    it('should create program and return complete response', async () => {
      ...
      expect(mockLearningServiceFactory.getService).toHaveBeenCalledWith('discovery');
      expect(mockDiscoveryService.startLearning).toHaveBeenCalledWith(
        'user-123',
        validUuid,
        { language: 'en' }
  ...
  describe('Error Handling', () => {
    ...
    it('should handle user repository errors', async () => {
      ...
      mockUserRepo.findByEmail.mockRejectedValue(new Error('User query failed'));
    ...
    it('should handle learning service errors', async () => {
      ...
      mockDiscoveryService.startLearning.mockRejectedValue(new Error('Service unavailable'));
    ...
    it('should handle task repository errors', async () => {
      ...
      mockTaskRepo.findByProgram.mockRejectedValue(new Error('Task query failed'));
