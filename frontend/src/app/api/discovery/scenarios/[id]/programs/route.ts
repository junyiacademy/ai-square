...
export async function GET(
  ...
  try {
    ...
    const scenarioPrograms = allPrograms.filter(
...
export async function POST(
  ...
  try {
    ...
    const program = await programRepo.create({
      ...
      discoveryData: {
        careerType:
          scenario.sourceMetadata?.careerType || scenario.sourceId || "unknown",
      },
      assessmentData: {},
    ...
    if (taskTemplates.length > 0) {
      ...
      for (let i = 0; i < taskTemplates.length; i++) {
        ...
        console.log(
          `  Creating task ${i + 1}/${taskTemplates.length}: ${template.id}`,
        );
      ...
      const task = await taskRepo.create({
        ...
        description: {
          en: "Explore your chosen career path",
          zh: "探索你選擇的職業道路",
        },
        type: "chat",
        status: "active",
        content: {
          scenarioId: scenario.id,
          instructions: {
            en: "Learn about this career through AI conversation",
            zh: "通過 AI 對話了解這個職業",
          },
          xp: 100,
        },
