...
describe("/api/discovery/scenarios/[id]/programs/[programId]", () => {
  ...
  it("returns 401 when unauthenticated", async () => {
    ...
    const req = new NextRequest(
      "http://localhost/api/discovery/scenarios/s1/programs/p1",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "s1", programId: "p1" }),
    });
  ...
  it("returns program details with tasks summary (200)", async () => {
    ...
    scenarioRepo.findById.mockResolvedValue({
      id: "s1",
      title: { en: "Scenario" },
      sourceMetadata: { careerType: "data_analyst" },
    });

    const req = new NextRequest(
      "http://localhost/api/discovery/scenarios/s1/programs/p1?lang=en",
    );
    const res = await GET(req, {
      params: Promise.resolve({ id: "s1", programId: "p1" }),
    });
