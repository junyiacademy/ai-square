  ...
  return {
    VertexAIService: jest.fn().mockImplementation(() => ({
      sendMessage: jest.fn(async () => ({
        content: '{"feedback":"ok","completed":true,"xpEarned":50}',
      })),
    })),
  };
});
...
jest.mock("@/lib/services/translation-service", () => {
  ...
  (
    MockTranslationService as unknown as { getFeedbackByLanguage?: jest.Mock }
  ).getFeedbackByLanguage = jest
    .fn()
    .mockImplementation(
      (versions: Record<string, string>, lang: string) =>
...
describe("/api/discovery/scenarios/[id]/programs/[programId]/tasks/[taskId]", () => {
  ...
  describe("GET", () => {
    it("returns 401 when unauthenticated", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/discovery/scenarios/s1/programs/p1/tasks/t1",
      );
      const res = await GET(req, {
        params: Promise.resolve({ id: "s1", programId: "p1", taskId: "t1" }),
      });
    ...
    it("returns 200 with task data", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/discovery/scenarios/s1/programs/p1/tasks/t1?lang=en",
      );
      const res = await GET(req, {
        params: Promise.resolve({ id: "s1", programId: "p1", taskId: "t1" }),
      });
    ...
    it("translates evaluation to requested language and stores versions", async () => {
      const {
        TranslationService,
      } = require("@/lib/services/translation-service");
      ...
      evaluationRepo.findById.mockResolvedValue({
        id: "e1",
        score: 0.9,
        feedbackData: { en: "Great job" },
        createdAt: "2024-01-01T00:00:00Z",
      });

      const req = new NextRequest(
        "http://localhost/api/discovery/scenarios/s1/programs/p1/tasks/t1?lang=es",
      );
      const res = await GET(req, {
        params: Promise.resolve({ id: "s1", programId: "p1", taskId: "t1" }),
      });
      ...
      expect(taskRepo.update).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          metadata: expect.objectContaining({
            evaluationFeedbackVersions: expect.objectContaining({
              es: "translated-feedback",
            }),
          }),
        }),
      );
    ...
    it("falls back to existing version when translation or update fails", async () => {
      ...
      jest
        .spyOn(TranslationService.prototype, "translateFeedback")
        .mockImplementationOnce(async () => {
          throw new Error("t-fail");
        });
      // Ensure static fallback picks english
      (
        TranslationService as unknown as { getFeedbackByLanguage: jest.Mock }
      ).getFeedbackByLanguage.mockReturnValueOnce("Great job");
  ...
  describe("PATCH", () => {
    ...
    it("invalid action returns 400", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/discovery/scenarios/s1/programs/p1/tasks/t1",
        { method: "PATCH", body: JSON.stringify({ action: "unknown" }) },
      );
      const res = await PATCH(req, {
        params: Promise.resolve({ id: "s1", programId: "p1", taskId: "t1" }),
      });
    ...
    it("submit with AI error returns success:false payload (200)", async () => {
      ...
      const req = new NextRequest(
        "http://localhost/api/discovery/scenarios/s1/programs/p1/tasks/t1",
        {
          ...
          body: JSON.stringify({
            action: "submit",
            content: { response: "hello", timeSpent: 5 },
          }),
        },
      );
      const res = await PATCH(req, {
    ...
    it("confirm-complete returns 400 when no passed ai_response exists", async () => {
      ...
      taskRepo.getTaskWithInteractions.mockResolvedValue({
        id: "t1",
        programId: "p1",
        status: "pending", // not completed and no passed ai_response
        interactions: [
          {
            type: "user_input",
            content: "answer",
          ...
          {
            type: "ai_response",
            content: JSON.stringify({ completed: false, feedback: "no" }),
            timestamp: new Date().toISOString(),
            metadata: {},
          },
        ],
    ...
    it("regenerate-evaluation returns 403 when not in development", async () => {
      ...
      const req = new NextRequest(
        ...
        {
          method: "PATCH",
          body: JSON.stringify({ action: "regenerate-evaluation" }),
        },
      );
      const res = await PATCH(req, {
        params: Promise.resolve({ id: "s1", programId: "p1", taskId: "t1" }),
