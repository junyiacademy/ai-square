...
export async function GET(request: NextRequest) {
  try {
    ...
    const programRepo = userId
      ? repositoryFactory.getProgramRepository()
      : null;
    ...
    const compute = async () => {
      ...
      if (userId && programRepo) {
        ...
        const discoveryPrograms = programs.filter(
          (p) => p.mode === "discovery",
        );
      ...
      const processedScenarios = scenarios.map((scenario) => {
        ...
        if (userProgress) {
          ...
          if (stats.completedCount > 0) {
            ...
            const activeProgram = progress.activeProgram as Record<
              string,
              unknown
            >;
