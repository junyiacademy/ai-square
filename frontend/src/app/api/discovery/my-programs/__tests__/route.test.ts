...
describe("GET /api/discovery/my-programs", () => {
  const mockedGetUnifiedAuth = getUnifiedAuth as jest.MockedFunction<
  ...
  it("returns cached data when available", async () => {
    ...
    const cachedData = [{ id: "scenario1", title: "Cached Scenario" }];
  ...
  it("handles scenario loading errors gracefully", async () => {
    ...
    mockScenarioRepository.findById.mockRejectedValue(
  ...
  it("calculates latest activity correctly", async () => {
    ...
    expect(
      new Date(data[0].userPrograms.lastActivity).getTime(),
  ...
  it("handles repository errors", async () => {
    ...
    mockedCacheService.get.mockResolvedValue(null);
    mockProgramRepository.findByUser.mockRejectedValue(
