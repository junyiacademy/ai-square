...
const mockDistributedCache = distributedCacheService as jest.Mocked<
...
describe("/api/ksa route", () => {
  ...
  describe("Language Parameter Handling", () => {
    it("should default to English when no language parameter is provided", async () => {
      ...
      expect(data.knowledge_codes.themes.Theme1.explanation).toBe(
        "Theme explanation",
      );
    });
    ...
    it("should return Chinese translations when lang=zhTW is provided", async () => {
      const request = createMockRequest(
        "http://localhost:3000/api/ksa?lang=zhTW",
      );
      ...
      expect(mockJsonYamlLoader.load).toHaveBeenCalledWith(
        "rubrics_data/ksa_codes/ksa_codes_zhTW",
        { preferJson: false },
      );
      expect(data.knowledge_codes.description).toBe("Knowledge description");
      expect(data.knowledge_codes.themes.Theme1.explanation).toBe(
      ...
      expect(data.knowledge_codes.themes.Theme1.codes["K1.1"].summary).toBe(
        "Code summary",
      );
    });
    ...
    it("should fallback to English when translation is not available", async () => {
      const request = createMockRequest(
        "http://localhost:3000/api/ksa?lang=fr",
      );
      ...
      expect(mockJsonYamlLoader.load).toHaveBeenCalledWith(
        "rubrics_data/ksa_codes/ksa_codes_fr",
        { preferJson: false },
      );
