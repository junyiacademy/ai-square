...
// No fallback value to prevent hardcoded secrets
...
export async function POST(request: NextRequest) {
  ...
  try {
    // SECURITY: Require admin key from environment variable
    ...
    // Verify secret key (check both header and body for compatibility)
    const authHeader = request.headers.get("x-admin-key");
    const bodySecret = body.secretKey as string;
    if (authHeader !== adminSecret && bodySecret !== adminSecret) {
    ...
    if (action === "reset-full" || action === "init-full") {
      ...
      const pblScenarioFiles = pblFiles.filter((f) =>
        f.endsWith("_scenario.yaml"),
      );
      ...
      try {
        const assessmentFolders = await fs.readdir(assessmentDir, {
          withFileTypes: true,
        });
              ...
              `,
                [
                  ...
                  JSON.stringify({
                    en:
                      data.description ||
                      "Assessment for AI literacy competencies",
      ...
      try {
        ...
        const careerFiles = discoveryFiles.filter((f) =>
          f.endsWith("_career.yaml"),
        );
              ...
              `,
                [
                  ...
                  JSON.stringify({
                    ...
                    zh:
                      data.description_zh || data.description || "職業探索路徑",
                  }),
                  JSON.stringify({
