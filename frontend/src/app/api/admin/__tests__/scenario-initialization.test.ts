...
describe("Scenario Initialization APIs", () => {
  ...
  describe("Assessment Init API", () => {
    it("should delete all assessment scenarios when clean=true", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3001/api/admin/init-assessment",
        {
          method: "POST",
  ...
  describe("PBL Init API", () => {
    it("should delete all PBL scenarios when clean=true", async () => {
      ...
      // Act
      const request = new NextRequest(
        "http://localhost:3001/api/admin/init-pbl",
        {
          method: "POST",
          body: JSON.stringify({ clean: true }),
        },
  ...
  describe("Discovery Init API", () => {
    it("should delete all discovery scenarios when clean=true", async () => {
      ...
      // Act
      const request = new NextRequest(
        "http://localhost:3001/api/admin/init-discovery",
        {
          method: "POST",
          body: JSON.stringify({ clean: true }),
        },
  ...
  describe("Integration", () => {
    it("should clean and reinitialize all modes", async () => {
      ...
      mockFindByMode.mockImplementation((mode) => {
        switch (mode) {
          case "pbl":
            return Promise.resolve([{ id: "pbl-1", mode: "pbl" }]);
          case "assessment":
            return Promise.resolve([{ id: "assess-1", mode: "assessment" }]);
          case "discovery":
            return Promise.resolve([{ id: "disc-1", mode: "discovery" }]);
          default:
      ...
      const pblRequest = new NextRequest(
        "http://localhost:3001/api/admin/init-pbl",
        {
          method: "POST",
          body: JSON.stringify({ clean: true }),
      ...
      const assessRequest = new NextRequest(
        "http://localhost:3001/api/admin/init-assessment",
        {
          method: "POST",
          body: JSON.stringify({ clean: true }),
      ...
      const discRequest = new NextRequest(
        "http://localhost:3001/api/admin/init-discovery",
        {
          method: "POST",
          body: JSON.stringify({ clean: true }),
