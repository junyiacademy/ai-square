...
export async function POST(request: NextRequest) {
  try {
    ...
    let allAssessmentScenarios =
      (await scenarioRepo.findByMode?.("assessment", body.clean)) || [];

    // If clean flag is set, delete ALL assessment scenarios first
    if (body.clean) {
      console.log(
        `[Init Assessment] Cleaning ${allAssessmentScenarios.length} scenarios`,
      );
      ...
      for (const scenario of allAssessmentScenarios) {
        ...
        try {
          ...
          if (programs.length > 0) {
            console.log(
              `[Init Assessment] Found ${programs.length} programs for scenario ${scenario.id}, deleting them first`,
            );
          ...
          console.error(
            `[Init Assessment] Failed to delete scenario ${scenario.id}:`,
            error,
          );
    ...
    const existingScenarios = allAssessmentScenarios.filter(
      (s) =>
        s.status === "active" &&
        (s.sourceId === "ai_literacy" ||
          s.sourcePath === "assessment_data/ai_literacy"),
    );
    ...
    const assessmentData = {
      ...
      questionBank: data.tasks.map((task: Record<string, unknown>) => ({
        ...
        questions: (task.questions as Array<Record<string, unknown>>).map(
    ...
    if (existingScenarios.length > 0) {
      ...
      const description = {
        en: "Test your understanding of AI concepts and applications",
      };
      ...
      try {
        ...
        const assessmentKeys = keys.filter(
          (key) =>
        ...
        console.log(
          `[Init Assessment] Cleared ${assessmentKeys.length} cache entries`,
        );
      } catch (error) {
