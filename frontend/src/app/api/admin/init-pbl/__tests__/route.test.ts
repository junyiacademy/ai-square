  ...
  describe("POST", () => {
    it("should initialize PBL scenarios from YAML files", async () => {
      ...
      (fs.readdir as jest.Mock)
        ...
        .mockResolvedValueOnce([
          "ai_job_search_en.yaml",
          "ai_job_search_zh.yaml",
        ]) // files in first dir
        .mockResolvedValueOnce([
          "ai_education_en.yaml",
          "ai_education_zh.yaml",
        ]); // files in second dir
      ...
      const mockYamlData = {
        ...
        tasks: [{ id: "task1", title: { en: "Task 1" }, type: "question" }],
      };
    ...
    it("should handle multilingual prerequisites correctly", async () => {
      (fs.readdir as jest.Mock)
        ...
        .mockResolvedValueOnce([
          "semiconductor_adventure_en.yaml",
          "semiconductor_adventure_zhTW.yaml",
        ]);
    ...
    it("should handle existing scenarios when force is false", async () => {
      ...
      mockFindByMode.mockResolvedValue([{ sourceId: "existing-scenario" }]);
    ...
    it("should handle errors gracefully", async () => {
      ...
      // When readdir fails at top level, route returns 500 with error message
