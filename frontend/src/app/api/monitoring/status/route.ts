...
export async function GET(request: NextRequest) {
  try {
    ...
    const basicStatus = {
      ...
      cache: {
        redis:
          (cacheStats as unknown as { redisConnected?: boolean })
      ...
      performance: {
        ...
        averageResponseTime:
          perfReport.length > 0
            ? perfReport.reduce((sum, m) => sum + m.averageResponseTime, 0) /
              perfReport.length
            : 0,
        cacheHitRate:
          perfReport.length > 0
            ? perfReport.reduce((sum, m) => sum + m.cacheHitRate, 0) /
              perfReport.length
            : 0,
        errorRate:
          perfReport.length > 0
            ? perfReport.reduce((sum, m) => sum + m.errorRate, 0) /
              perfReport.length
            : 0,
      },
      health: "healthy",
