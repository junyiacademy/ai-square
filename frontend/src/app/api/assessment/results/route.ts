...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    let assessmentProgram = programs.find(
      (p) => p.scenarioId === body.scenarioId && p.status === "active",
    );
    ...
    const evaluation = await evaluationRepo.create({
      ...
      aiAnalysis: {
        insights: [
          `You achieved ${body.result.level} level with ${body.result.correctAnswers}/${body.result.totalQuestions} correct answers`,
        ],
        strengths: body.result.domainScores
          ? Object.entries(body.result.domainScores)
              .filter(([, score]) => (score as number) >= 70)
              .map(([domain]) => `Strong performance in ${domain}`)
          : [],
        improvements: body.result.domainScores
          ? Object.entries(body.result.domainScores)
              .filter(([, score]) => (score as number) < 70)
...
export async function GET(request: NextRequest) {
  ...
  return cachedGET(
    ...
    async () => {
      ...
      const programs =
        (await programRepo.getCompletedPrograms?.(user.id)) || [];
      const assessmentPrograms = programs.filter(
        (p) =>
      ...
      if (assessmentPrograms.length > 0) {
        ...
        for (const program of assessmentPrograms) {
          ...
          const assessmentEvaluation = programEvaluations.find(
          ...
          if (assessmentEvaluation) {
            const result: AssessmentResult = {
              ...
              language:
                (assessmentEvaluation.metadata?.language as string) || "en",
              scores: {
                overall: assessmentEvaluation.score,
                domains: {
                  engaging_with_ai:
                    assessmentEvaluation.domainScores?.["Engaging_with_AI"] ||
                    0,
                  creating_with_ai:
                    assessmentEvaluation.domainScores?.["Creating_with_AI"] ||
                    0,
                  managing_with_ai:
                    assessmentEvaluation.domainScores?.["Managing_with_AI"] ||
                    0,
                  designing_with_ai:
                    assessmentEvaluation.domainScores?.["Designing_with_AI"] ||
                    0,
                },
              },
              summary: {
                total_questions:
                  (assessmentEvaluation.metadata?.totalQuestions as number) ||
                  0,
                correct_answers:
                  (assessmentEvaluation.metadata?.correctAnswers as number) ||
                  0,
                level:
                  (assessmentEvaluation.metadata?.level as string) ||
                  "beginner",
              },
              answers:
                (assessmentEvaluation.metadata
                  ?.answers as AssessmentResult["answers"]) || [],
            };
