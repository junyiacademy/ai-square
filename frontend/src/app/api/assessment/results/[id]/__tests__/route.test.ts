...
const { repositoryFactory } = jest.requireMock(
...
describe("/api/assessment/results/[id]", () => {
  ...
  it("returns 400 when userId is missing", async () => {
    const request = new NextRequest(
      "http://localhost:3000/api/assessment/results/eval123",
    );
  ...
  it("returns 200 with local file result when Postgres disabled", async () => {
    ...
    jest.doMock("fs/promises", () => ({
      readFile: jest.fn(async () =>
        JSON.stringify({
          results: [{ user_id: userId, assessment_id: evalId, score: 88 }],
        }),
      ),
    }));
    ...
    const request = new NextRequest(
      `http://localhost:3000/api/assessment/results/${evalId}?userId=${encodeURIComponent(userId)}`,
    );
    const response = await LocalGET(request, {
      params: Promise.resolve({ id: evalId }),
    });
    const data = await response.json();
  ...
  it("returns 500 when local file missing result", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/assessment/results/xx?userId=u",
    );
    const response = await LocalGET(request, {
  ...
  it("returns 200 with Postgres evaluation when enabled", async () => {
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/assessment/results/e1?userId=u@e.com",
    );
  ...
  it("returns 500 when Postgres user not found", async () => {
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/assessment/results/e2?userId=nouser@example.com",
    );
  ...
  it("returns 500 when evaluation belongs to another user (Postgres)", async () => {
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/assessment/results/e3?userId=u@e.com",
    );
  ...
  it("returns 200 using userId lookup (non-email) in Postgres path", async () => {
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/assessment/results/e9?userId=u9",
    );
  ...
  it("uses memoized local results (readFile called once for repeated request)", async () => {
    ...
    jest.resetModules();
    const readFileMock = jest.fn(async () =>
      JSON.stringify({
    ...
    const res1 = await LocalGET(req1, {
      params: Promise.resolve({ id: evalId }),
    });
    ...
    const res2 = await LocalGET(req2, {
      params: Promise.resolve({ id: evalId }),
    });
