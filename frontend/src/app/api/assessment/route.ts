...
export async function GET(request: NextRequest) {
  try {
    ...
    const activeScenario =
      scenarios.find((s) => s.status === "active") || scenarios[0];

    console.log(
      `Loading assessment data from database for scenario: ${activeScenario.id}`,
    );
    ...
    try {
      ...
      if (activeScenario.assessmentData?.questions) {
        const questionsData = activeScenario.assessmentData.questions as Record<
          string,
      ...
      const domains = activeScenario.assessmentData?.domains || {
        engaging_with_ai: {
          ...
          description: "Understanding AI interactions",
          questions: 5,
        },
        creating_with_ai: {
          name: "Creating with AI",
          description: "Using AI for creation",
          questions: 5,
        },
        managing_with_ai: {
          name: "Managing AI",
          description: "Managing AI systems",
          questions: 5,
        },
        designing_with_ai: {
          name: "Designing AI",
      ...
      const processedQuestions = rawQuestions.map(
        (q: Record<string, unknown>) => {
          ...
          const questionObj = q.question as
            | Record<string, unknown>
            | string
            | null;
          const question =
            typeof questionObj === "object" && questionObj !== null
              ? (questionObj[lang] as string) ||
                (questionObj["en"] as string) ||
                ""
              : (questionObj as string) || "";

          const options = q.options;

          const explanationObj = q.explanation as
            | Record<string, unknown>
            | string
            | null;
          const explanation =
            typeof explanationObj === "object" && explanationObj !== null
              ? (explanationObj[lang] as string) ||
                (explanationObj["en"] as string) ||
                ""
