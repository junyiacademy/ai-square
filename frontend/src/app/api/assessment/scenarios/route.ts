...
export async function GET(request: NextRequest) {
  try {
    ...
    if (dbScenarios.length > 0) {
      console.log(
        `Found ${dbScenarios.length} assessment scenarios in database`,
      );
      ...
      const formattedScenarios = dbScenarios.map((scenario: IScenario) => ({
        id: scenario.id,
        title:
          scenario.title?.[lang] || scenario.title?.en || "Untitled Assessment",
        description:
          scenario.description?.[lang] ||
          scenario.description?.en ||
          "No description",
        folderName:
          scenario.sourceMetadata?.folderName ||
          scenario.sourceId ||
          scenario.id,
        config: {
          totalQuestions:
            ((scenario.assessmentData as Record<string, unknown>)
              ?.totalQuestions as number) || 12,
      ...
      if (key) {
        // 先嘗試從快取取得
        const cached = (await distributedCacheService.get(key)) as {
          data?: { scenarios?: unknown[] };
        } | null;
        if (
          cached &&
          cached.data?.scenarios?.length &&
          cached.data.scenarios.length > 0
        ) {
        ...
        if (formattedScenarios.length > 0) {
          await distributedCacheService.set(key, result, {
    ...
    console.error(
      "[Assessment API] ERROR: No scenarios in database. Database initialization required!",
    );
