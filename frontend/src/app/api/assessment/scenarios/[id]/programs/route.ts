  ...
  try {
    ...
    if (evaluationIds.length > 0) {
      const rawEvaluations = await Promise.all(
        evaluationIds.map((id: unknown) =>
          evaluationRepo.findById(id as string).catch(() => null),
        ),
      );
    ...
    const enrichedPrograms = userPrograms.map((program) => {
      ...
      const evaluation =
        typeof evaluationId === "string" && evaluationId
...
export async function POST(
  ...
  try {
    ...
    const activeProgram = existingPrograms.find(
      (p: IProgram) => p.scenarioId === id && p.status === "active",
    );

    if (activeProgram) {
      console.log(
        `User ${email} already has an active program for scenario ${id}, returning existing`,
      );
    ...
    try {
      const program = await assessmentService.startLearning(user.id, id, {
        language,
      });
