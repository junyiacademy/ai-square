    ...
    switch (action) {
      ...
      case "submit":
        ...
        const questionsArray = Array.isArray(
          (task.content as Record<string, unknown>)?.questions,
        )
          ? ((task.content as Record<string, unknown>)
              ?.questions as QuestionType[])
          : [];
        const assessmentInteractions: AssessmentInteraction[] = answers.map(
          (answer: {
            questionId: string;
            answer: string;
            timeSpent?: number;
        ...
        // Calculate score
        const correctCount = assessmentInteractions.filter(
          (i) => i.context.isCorrect,
        ).length;
