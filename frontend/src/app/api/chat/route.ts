...
async function getUserMemory(userEmail: string): Promise<UserMemory | null> {
  ...
  try {
    ...
    const shortTermFile = bucket.file(
      `user/${sanitizedEmail}/memory/short_term.json`,
    );
    const longTermFile = bucket.file(
...
async function generateChatTitle(
  ...
  const conversationContext = messages
    .slice(0, 6) // user, assistant, user (if exists)
    .map((msg) => {
...
- Use examples relevant to their context`;
    ...
    const chat = model.startChat({
      history: [
        ...
        },
        {
          ...
          parts: [
            {
              text: "I understand. I'll help guide the user based on their profile and learning progress.",
            },
          ],
        },
      ],
    ...
    const session = await saveMessage(
      userEmail,
      currentSessionId,
      assistantMessage,
    );
    ...
    if (shouldGenerateTitle) {
      ...
      const sanitizedEmail = userEmail.replace("@", "_at_").replace(/\./g, "_");
      const sessionFile = bucket.file(
        `user/${sanitizedEmail}/chat/sessions/${currentSessionId}.json`,
      );
      await sessionFile.save(JSON.stringify(session, null, 2));
