...
describe("POST /api/chat", () => {
              ...
              return {
                ...
                download: jest.fn().mockResolvedValue([
                  JSON.stringify({
                    identity: "student",
                    goals: [],
                    assessmentResult: { overallScore: 50, domainScores: {} },
                    completedPBLs: [],
                    learningStyle: "visual",
                  }),
                ]),
  ...
  it("should auto-generate title for new session (no sessionId) and persist it", async () => {
              ...
              return {
                exists: jest
                  ...
                  .mockImplementation(async () => [
                    Boolean(savedPayloads[filePath]?.length),
                  ]),
                download: jest
                  .fn()
                  .mockImplementation(async () => [
                    savedPayloads[filePath]?.[
                      savedPayloads[filePath].length - 1
                    ] ?? JSON.stringify({}),
                  ]),
                save,
              };
    ...
    const savedSessionPaths = Object.keys(savedPayloads).filter((p) =>
      p.includes("chat/sessions/"),
    );
  ...
  it('should update title when existing session title is "New Chat" and messages reach threshold', async () => {
          ...
          file: jest.fn().mockImplementation((filePath: string) => {
            ...
            if (filePath.includes("chat/sessions/sess-1.json")) {
              return {
                ...
                download: jest
                  .fn()
                  .mockResolvedValue([JSON.stringify(existingSession)]),
                save,
              };
            }
            if (filePath.endsWith("chat/index.json")) {
    ...
    const sessionPath = Object.keys(savedPayloads).find((p) =>
      p.includes("chat/sessions/sess-1.json"),
    ) as string;
    const lastSave =
      savedPayloads[sessionPath][savedPayloads[sessionPath].length - 1];
  ...
  it("should swallow short-term memory save errors and still return 200", async () => {
    ...
    const shortTermSave = jest.fn(async () => {
