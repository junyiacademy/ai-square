  ...
  describe("GET - Get Chat Session", () => {
    const mockSession = {
      ...
      messages: [
        ...
        },
        {
          ...
          role: "assistant",
          content:
            "I can help you with that! What kind of job are you looking for?",
    ...
    it("should return session data for authenticated user", async () => {
      ...
      mockDownload.mockResolvedValue([
        Buffer.from(JSON.stringify(mockSession)),
      ]);

      const request = new NextRequest(
        "http://localhost:3000/api/chat/sessions/session-123",
        {
          headers: {
      ...
      expect(mockFile).toHaveBeenCalledWith(
        "user/user_at_example_com/chat/sessions/session-123.json",
      );
    });
    ...
    it("should return 404 when session not found", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/chat/sessions/nonexistent",
        {
          headers: {
            "x-user-info": JSON.stringify(userInfo),
          },
