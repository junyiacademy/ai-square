...
describe("/api/relations", () => {
  beforeEach(() => {
    ...
    mockJsonYamlLoader.load.mockImplementation((filename: string) => {
      if (filename.includes("ai_lit_domains"))
        return Promise.resolve(mockDomainsData);
  ...
  describe("successful responses", () => {
    it("returns relations data for English", async () => {
      const request = new MockNextRequest(
        "http://localhost/api/relations?lang=en",
      ) as any;
      ...
      expect(data.domains[0].competencies[0].description).toBe(
    ...
    it("returns translated data for Chinese", async () => {
      ...
      mockJsonYamlLoader.load.mockImplementation((filename: string) => {
        if (filename.includes("ai_lit_domains_zhTW"))
          return Promise.resolve(mockChineseDomainsData);
        if (filename.includes("ksa_codes_zhTW"))
          return Promise.resolve(mockChineseKsaData);
        return Promise.resolve({});
      });

      const request = new MockNextRequest(
        "http://localhost/api/relations?lang=zhTW",
      ) as any;
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(data.domains[0].competencies[0].scenarios).toEqual([
        "情境1",
        "情境2",
      ]);
      expect(data.domains[0].competencies[0].context).toBe(
        "關於 AI 素養的內容",
      );
    ...
    it("returns translated data for Spanish", async () => {
      ...
      mockJsonYamlLoader.load.mockImplementation((filename: string) => {
        if (filename.includes("ai_lit_domains_es"))
          return Promise.resolve(mockSpanishDomainsData);
        if (filename.includes("ksa_codes_es"))
          return Promise.resolve(mockKsaData);
        return Promise.resolve({});
      });

      const request = new MockNextRequest(
        "http://localhost/api/relations?lang=es",
      ) as any;
      const response = await GET(request);
      const data = await response.json();

      expect(response.status).toBe(200);
      expect(data.domains[0].overview).toBe(
        "Descripción general de interactuar con IA",
      );
      expect(data.domains[0].competencies[0].description).toBe(
        "Comprensión de las capacidades de IA",
      );
    });

    it("falls back to English for missing translations", async () => {
      const request = new MockNextRequest(
        "http://localhost/api/relations?lang=fr",
      ) as any;
      const response = await GET(request);
      const data = await response.json();
      ...
      expect(response.status).toBe(200);
      expect(data.domains[0].overview).toBe("Overview of engaging with AI");
      expect(data.domains[1].competencies[0].description).toBe(
        "Collaborating creatively with AI",
      );
    });
  ...
  describe("caching behavior", () => {
    ...
    it("fetches and caches data when cache miss", async () => {
      ...
      expect(mockCacheService.get).toHaveBeenCalledWith("relations-en");
      expect(mockJsonYamlLoader.load).toHaveBeenCalledWith(
