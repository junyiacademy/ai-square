...
export async function POST(
  ...
  try {
    ...
    const createEvaluationData = (
      ...
      aiAnalysis:
        (evaluation.conversationInsights as Record<string, unknown>) || {},
      createdAt: new Date().toISOString(),
      pblData: {
        ksaScores:
          (evaluation.ksaScores as Record<string, number>) ||
          (evaluation.domainScores as Record<string, number>) ||
          {},
        rubricsScores:
          (evaluation.rubricsScores as Record<string, number>) || {},
    ...
    const existingEvaluationId = task?.metadata?.evaluationId as
      | string
      | undefined;
    if (existingEvaluationId) {
      ...
      evaluationRecord = await evalRepo.create(
        createEvaluationData(evaluation, existingEval?.metadata),
      );
      ...
      console.log(
        "  domainScores:",
        JSON.stringify(evaluationRecord.domainScores || {}, null, 2),
      );
      ...
      evaluationRecord = await evalRepo.create(
        createEvaluationData(evaluation),
      );
    ...
    const transformedEvaluation = {
      ...
      evaluatedAt:
        evaluationRecord.metadata?.evaluatedAt || evaluationRecord.createdAt,
    };

    return NextResponse.json({
      success: true,
      message: existingEvaluationId
