...
export async function GET(request: NextRequest) {
  ...
  return cachedGET(
    ...
    async () => {
      console.log(
        `Fetching PBL history for user: ${userEmail}, scenario: ${scenarioId || "all"}, language: ${language}`,
      );
      ...
      const completionDataPromises = programs.map(async (program) => {
              ...
              ([dimension, score]: [string, number]) => {
                if (dimension.includes("_")) {
                  domainScores[dimension] =
                    (domainScores[dimension] || 0) + score;
                } else {
        ...
        try {
          const scenarioContent = await contentRepo.getScenarioContent(
            program.scenarioId,
            language,
          );
          scenarioTitle =
            scenarioContent.title[language] ||
            scenarioContent.title["en"] ||
            program.scenarioId;
        } catch {
          console.warn(
            `Failed to load scenario title for ${program.scenarioId}`,
          );
        }
