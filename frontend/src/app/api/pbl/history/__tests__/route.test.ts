...
describe('GET /api/pbl/history', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    (repositoryFactory.getUserRepository as jest.Mock).mockReturnValue(mockRepos.user);
  ...
  it('should return history for authenticated user', async () => {
    mockRepos.user.findByEmail.mockResolvedValue({ id: 'user-1', email: 'test@example.com' });
  ...
  it('should filter by scenarioId', async () => {
    mockRepos.user.findByEmail.mockResolvedValue({ id: 'user-1', email: 'test@example.com' });
    mockRepos.program.findByUser.mockResolvedValue([
      { id: 'prog-1', scenarioId: 'scenario-1', status: 'completed', totalTaskCount: 5, createdAt: '2024-01-01' },
      { id: 'prog-2', scenarioId: 'scenario-2', status: 'completed', totalTaskCount: 3, createdAt: '2024-01-01' }
    ]);
    mockRepos.task.findByProgram.mockResolvedValue([]);
    mockRepos.evaluation.findByProgram.mockResolvedValue([]);
  ...
  it('should calculate scores correctly', async () => {
    ...
    mockRepos.evaluation.findByProgram.mockResolvedValue([
      { id: 'eval-1', score: 80, domainScores: { domain_1: 75, knowledge: 85 } },
      { id: 'eval-2', score: 90, domainScores: { domain_1: 95, knowledge: 90 } }
    ]);
