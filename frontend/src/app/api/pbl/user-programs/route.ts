  ...
  return cachedGET(
    ...
    async () => {
      ...
      for (const scenarioId of uniqueScenarioIds) {
        try {
          const content = await contentRepo.getScenarioContent(
            scenarioId,
      ...
      const programsWithInfo = programs.map((program) => {
        ...
        if (scenarioContent) {
          const titleObj = (scenarioContent as Record<string, unknown>)
            ?.title as Record<string, string> | undefined;
          scenarioTitle =
            titleObj?.[user.preferredLanguage] ||
            titleObj?.["en"] ||
            program.scenarioId;
        }
