...
export async function POST(request: NextRequest) {
  try {
    ...
    const taskTemplate =
      (task.metadata?.taskTemplate as Record<string, unknown>) || {};
    let aiModule =
      taskTemplate.aiModule ||
      taskTemplate.ai_module ||
...
function buildSystemPrompt(
  ...
  const aiModuleData = aiModule as {
    role?: string;
    persona?: string;
  ...
  const initialPrompt =
    aiModuleData.initialPrompt || aiModuleData.initial_prompt || "";
