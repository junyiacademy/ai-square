...
export async function GET(request: NextRequest) {
  ...
  return cachedGET(
    ...
    async () => {
      ...
      try {
        console.log(
          "[COMPLETION DEBUG] Attempting fetch to:",
        ...
        console.log(
          "[COMPLETION DEBUG] Fetch response status:",
          completeRes.status,
        );
      } catch (fetchError) {
      ...
      if (!completeRes.ok || completeRes.status === 404) {
        console.log(
          "[COMPLETION DEBUG] Evaluation not found, creating new one via POST",
        );
        try {
          completeRes = await fetch(completeUrl.toString(), {
            method: "POST",
          ...
          console.log(
            "[COMPLETION DEBUG] POST response status:",
            completeRes.status,
          );
        } catch (postError) {
          ...
          throw new Error(
            `POST fetch failed: ${postError instanceof Error ? postError.message : "Unknown error"}`,
          );
        }
      }
      ...
      const evaluationMetadata = metadata?.evaluationMetadata as
      ...
      console.log("Completion API - program metadata:", {
        ...
        feedbackKeys: Object.keys(
          (evaluationMetadata?.qualitativeFeedback as Record<
            string,
            unknown
          >) || {},
        ),
      ...
      if (!evaluation) {
        console.error(
          "Completion API - No evaluation found after complete API call",
        );
      }
          ...
          return {
            ...
            log: {
              interactions: interactions.map((i: IInteraction) => ({
                ...
                message:
                  (i.metadata as Record<string, unknown>)?.message || i.content,
      ...
      const completionData = {
        ...
        totalTasks:
          ...
          evaluation?.pblData?.totalTasks ||
          tasks.length,
        completedTasks: tasks.filter((t: ITask) => t.status === "completed")
          .length,
        evaluatedTasks:
          evaluation?.metadata?.evaluatedTaskCount ||
          evaluation?.pblData?.evaluatedTasks ||
          0,
        overallScore: evaluation?.score || 0,
        domainScores:
          evaluation?.metadata?.domainScores || evaluation?.domainScores || {},
        ksaScores:
          evaluation?.metadata?.ksaScores ||
          evaluation?.pblData?.ksaScores ||
          {},
        totalTimeSeconds:
          evaluation?.metadata?.totalTimeSeconds ||
        ...
        feedbackGeneratedAt:
          evaluation?.metadata?.qualitativeFeedback?.[language]?.generatedAt,
