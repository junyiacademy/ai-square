...
describe("API Route: src/app/api/pbl/scenarios/[id]", () => {
  ...
  beforeEach(() => {
    ...
    mockDistributedCache =
      require("@/lib/cache/distributed-cache-service").distributedCacheService;
    ...
    mockDistributedCache.getWithRevalidation.mockImplementation(
      async (key: string, handler: () => Promise<any>) => {
  ...
  describe("GET", () => {
    it("should handle successful request", async () => {
      const request = new NextRequest(
        "http://localhost:3000/api/pbl/scenarios/test-id?lang=en",
        {
          method: "GET",
        },
      );
    ...
    it("should handle Chinese language parameter", async () => {
      const request = new NextRequest(
        "http://localhost:3000/api/pbl/scenarios/test-id?lang=zh",
        {
          method: "GET",
        },
      );
    ...
    it("should return multilingual prerequisites for Traditional Chinese", async () => {
      const request = new NextRequest(
        "http://localhost:3000/api/pbl/scenarios/test-id?lang=zhTW",
        {
          method: "GET",
        },
      );
    ...
    it.skip("should handle scenario not found", async () => {
      ...
      const originalGetWithRevalidation =
        mockDistributedCache.getWithRevalidation;

      // Mock to simulate that useDistributedCache is false, which bypasses SWR
      mockDistributedCache.getWithRevalidation.mockRejectedValue(
        new Error("SWR disabled for test"),
      );
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/pbl/scenarios/nonexistent",
        {
          method: "GET",
        },
      );
