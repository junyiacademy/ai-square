...
describe("POST /api/pbl/scenarios/[id]/start", () => {
  ...
  const createRequestWithUser = (
    email: string,
    body: Record<string, unknown> = { language: "en" },
  ) => {
    const request = new NextRequest(
      "http://localhost/api/pbl/scenarios/123/start",
      {
        method: "POST",
        headers: {
  ...
  describe("Using new service layer", () => {
    it("should use PBLLearningService to start learning and return tasks", async () => {
      ...
      (learningServiceFactory.getService as jest.Mock).mockReturnValue(
        mockPBLService,
      );
    ...
    it("should create user if not exists", async () => {
      ...
      const mockUserRepo = {
        ...
        create: jest
          .fn()
          .mockResolvedValue({ id: mockUserId, email: "new@example.com" }),
      };
