...
export async function POST(
  ...
  console.log(
    "🚨 START API CALLED - This should only happen when explicitly requested!",
  );
  ...
  try {
    ...
    const validation = validateProgramStart(
      scenario,
      user as unknown as import("@/types/database").DBUser,
    );
    ...
    const program = await pblService.startLearning(user.id, scenarioId, {
      language,
    });
