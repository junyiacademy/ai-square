...
describe("/api/pbl/scenarios route", () => {
  ...
  const mockScenarios = [
    {
      ...
      description: {
        en: "Use AI to enhance your job search",
        zh: "使用 AI 增強求職",
      },
    ...
    },
    {
      ...
      description: {
        en: "Design educational content with AI",
        zh: "使用 AI 設計教育內容",
      },
      difficulty: "advanced",
      estimatedMinutes: 60,
  ...
  beforeEach(() => {
    ...
    mockHybridTranslationService =
      require("@/lib/services/hybrid-translation-service").HybridTranslationService;
    mockScenarioIndexService =
      require("@/lib/services/scenario-index-service").scenarioIndexService;

    require("@/lib/repositories/base/repository-factory").repositoryFactory.getScenarioRepository.mockReturnValue(
      mockScenarioRepo,
    );
  ...
  describe("GET /api/pbl/scenarios", () => {
    it("should return scenarios from database successfully", async () => {
      const request = new NextRequest(
        "http://localhost/api/pbl/scenarios?lang=en",
      );
    ...
    it("should handle Chinese language parameter", async () => {
      const request = new NextRequest(
        "http://localhost/api/pbl/scenarios?lang=zh",
      );
    ...
    it("should fallback to English when language not available", async () => {
      const request = new NextRequest(
        "http://localhost/api/pbl/scenarios?lang=fr",
      );
      ...
      expect(data.data.scenarios[0].description).toBe(
        "Use AI to enhance your job search",
      );
    });
    ...
    it("should handle targetDomains from different sources", async () => {
      ...
      expect(data.data.scenarios[0].targetDomains).toEqual([
        "creating_with_ai",
        "engaging_with_ai",
      ]);
      expect(data.data.scenarios[0].targetDomain).toEqual([
        "creating_with_ai",
        "engaging_with_ai",
      ]);
      expect(data.data.scenarios[0].domains).toEqual([
        "creating_with_ai",
        "engaging_with_ai",
      ]);
    });
    ...
    it("should handle repository errors gracefully", async () => {
      ...
      expect(console.error).toHaveBeenCalledWith(
        "Error loading scenarios from database:",
        expect.any(Error),
      );
    });
    ...
    it("should handle individual scenario processing errors", async () => {
      ...
      const invalidResult = data.data.scenarios.find(
        (s: any) => s.id === "invalid-scenario",
      );
    ...
    it("should handle scenario index build failure", async () => {
      mockScenarioIndexService.buildIndex.mockRejectedValue(
    ...
    it("should log scenario details when scenarios exist", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        `[PBL API] Repository returned ${mockScenarios.length} raw scenarios`,
      );
      expect(console.log).toHaveBeenCalledWith(
        `[PBL API] Found ${mockScenarios.length} PBL scenarios in database`,
      );
    ...
    it("should log when no scenarios found", async () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        "[PBL API] No scenarios found, checking repository...",
      );
      expect(console.log).toHaveBeenCalledWith(
        "[PBL API] Repository findByMode exists?",
        true,
    ...
    it("should handle malformed URL gracefully", async () => {
      const request = new NextRequest(
        "http://localhost/api/pbl/scenarios?lang=en&invalid=param",
      );
    ...
    it("should handle different emoji mappings", async () => {
      const specialScenarios = [
        {
          id: "1",
          sourceId: "ai-stablecoin-trading",
          mode: "pbl",
          status: "active",
          title: "Stablecoin",
          description: "Trading",
        ...
        {
          id: "2",
          sourceId: "ai-robotics-development",
          mode: "pbl",
          status: "active",
          title: "Robotics",
        ...
        {
          id: "3",
          sourceId: "high-school-climate-change",
          mode: "pbl",
          status: "active",
          title: "Climate",
          description: "Change",
        ...
        {
          id: "4",
          sourceId: "high-school-digital-wellness",
          mode: "pbl",
          status: "active",
          title: "Digital",
        ...
        {
          id: "5",
          sourceId: "high-school-smart-city",
          mode: "pbl",
          status: "active",
          title: "Smart",
          description: "City",
        ...
        {
          id: "6",
          sourceId: "high-school-creative-arts",
          mode: "pbl",
          status: "active",
          title: "Arts",
        ...
        {
          id: "7",
          sourceId: "high-school-health-assistant",
          mode: "pbl",
          status: "active",
          title: "Health",
  ...
  describe("Cache behavior", () => {
    it("should use different cache keys for different languages", async () => {
      ...
      const zhRequest = new NextRequest(
        "http://localhost/api/pbl/scenarios?lang=zh",
      );
