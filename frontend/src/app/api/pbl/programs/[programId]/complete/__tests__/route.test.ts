...
describe("/api/pbl/programs/[programId]/complete", () => {
  ...
  beforeEach(() => {
    ...
    mockRepositoryFactory.getProgramRepository.mockReturnValue(
      mockProgramRepo as unknown as ReturnType<
        typeof repositoryFactory.getProgramRepository
      >,
    );
    mockRepositoryFactory.getTaskRepository.mockReturnValue(
      mockTaskRepo as unknown as ReturnType<
        typeof repositoryFactory.getTaskRepository
      >,
    );
    mockRepositoryFactory.getEvaluationRepository.mockReturnValue(
      mockEvalRepo as unknown as ReturnType<
        typeof repositoryFactory.getEvaluationRepository
      >,
    );
    mockRepositoryFactory.getUserRepository.mockReturnValue(
      mockUserRepo as unknown as ReturnType<
        typeof repositoryFactory.getUserRepository
      >,
    );
  ...
  describe("POST", () => {
    describe("Authentication and Authorization", () => {
      it("should return 401 for unauthenticated requests", async () => {
        ...
        const request = new NextRequest(
          "http://localhost:3000/api/pbl/programs/prog-123/complete",
          {
            method: "POST",
          },
        );
    ...
    describe("New Program Evaluation Creation", () => {
      ...
      it("should create new evaluation with single completed task", async () => {
        ...
        const response = await POST(request, {
          params: Promise.resolve({ programId: "prog-123" }),
        });
      ...
      it("should create evaluation with multiple tasks and mixed scores", async () => {
        const task2 = {
          ...
          id: "task-456",
          score: 90,
          metadata: { evaluationId: "task-eval-456" },
        };
        ...
        mockEvalRepo.findById.mockImplementation((id) => {
          if (id === "task-eval-123")
            return Promise.resolve(mockTaskEvaluation);
      ...
      it("should handle tasks with invalid/missing scores", async () => {
        const taskWithStringScore = {
          ...mockTask,
          id: "task-string",
          metadata: { evaluationId: "eval-string" },
        };
        const taskWithNullScore = {
          ...mockTask,
          id: "task-null",
          metadata: { evaluationId: "eval-null" },
        };
    ...
    describe("Existing Evaluation Updates", () => {
      ...
      it("should update existing evaluation when score changes", async () => {
        ...
        mockTaskRepo.findByProgram.mockResolvedValue(tasks);
        mockEvalRepo.findById.mockImplementation((id) => {
          if (id === "existing-eval-123")
            return Promise.resolve(existingEvaluation);
          if (id === "task-eval-123")
            return Promise.resolve(mockTaskEvaluation);
          return Promise.resolve(null);
        });
      ...
      it("should not update when evaluation is current", async () => {
        ...
        mockEvalRepo.findById.mockImplementation((id) => {
          if (id === "existing-eval-123")
            return Promise.resolve(currentEvaluation);
          if (id === "task-eval-123")
            return Promise.resolve(mockTaskEvaluation);
          return Promise.resolve(null);
        });
    ...
    describe("Error Handling", () => {
      ...
      it("should handle evaluation creation failure gracefully", async () => {
        ...
        mockEvalRepo.create.mockRejectedValue(
  ...
  describe("GET", () => {
    ...
    describe("Evaluation Retrieval", () => {
      it("should return existing valid evaluation", async () => {
        ...
        const request = new NextRequest(
          "http://localhost:3000/api/pbl/programs/prog-123/complete?language=en",
        );
      ...
      it("should indicate when feedback generation is needed", async () => {
        ...
        const request = new NextRequest(
          "http://localhost:3000/api/pbl/programs/prog-123/complete?language=zh",
        );
