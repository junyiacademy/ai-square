...
>;
const mockGetProgramStatus =
  postgresqlLearningService.getProgramStatus as jest.MockedFunction<
    typeof postgresqlLearningService.getProgramStatus
  >;
...
describe("GET /api/learning/programs/[programId]/status", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/learning/programs/prog123/status",
    );
  ...
  it("returns 401 when session has no email", async () => {
    mockGetUnifiedAuth.mockResolvedValue({
      user: { id: "user123", email: undefined as any, role: "student" },
    });
  ...
  it("returns 404 when program not found", async () => {
    ...
    const request = new NextRequest(
      "http://localhost:3000/api/learning/programs/nonexistent/status",
    );
    const response = await GET(request, {
      params: Promise.resolve({ programId: "test-id" }),
    });
  ...
  it("returns 500 for other errors", async () => {
    ...
    mockGetProgramStatus.mockRejectedValue(
