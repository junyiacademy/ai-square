...
import {
  getPromptByMode,
  getSystemPrompt,
} from "@/lib/prompts/scenario-generator";
import type {
  ...
  try {
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "Missing required fields: scenarioId, title, description, mode",
        },
        { status: 400 },
      );
    }
    ...
    if (yaml.includes("```")) {
      warnings.push(
        "Response contains markdown code blocks - cleaned automatically",
      );
    }
