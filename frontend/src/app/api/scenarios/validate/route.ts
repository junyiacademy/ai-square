...
import { validateScenario, getSchemaByMode } from '@/lib/validators/scenario-schema';
import type { ValidateScenarioRequest, ValidateScenarioResponse, ValidationError } from '@/types/prompt-to-course';
  ...
  try {
    ...
    if (validationResult.success && validationResult.data) {
      ...
      if (data.prerequisites.length === 0) {
        info.push({
          path: 'prerequisites',
          message: 'No prerequisites defined (this scenario can be taken by beginners)',
          severity: 'info',
        });
      }
      ...
      if (data.mode === 'discovery') {
        if (!data.discoveryData || Object.keys(data.discoveryData).length === 0) {
          errors.push({
            path: 'discoveryData',
            message: 'Discovery scenario must have discoveryData with career information',
            severity: 'error',
          });
        }
      }
      ...
      if (data.mode === 'assessment') {
        if (!data.assessmentData || Object.keys(data.assessmentData).length === 0) {
          errors.push({
            path: 'assessmentData',
            message: 'Assessment scenario must have assessmentData with domains and question types',
            severity: 'error',
          });
        }
      }
    ...
    return NextResponse.json(
      {
        error: error instanceof Error ? error.message : 'Failed to validate scenario',
      },
      { status: 500 }
    );
  }
