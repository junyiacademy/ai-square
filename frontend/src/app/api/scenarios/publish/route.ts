  ...
  try {
    ...
    if (!githubToken) {
      return NextResponse.json(
        {
          error:
            "GitHub integration not configured. Please set GITHUB_TOKEN environment variable.",
        },
        { status: 503 },
      );
    }
    ...
    if (!scenarioIdPattern.test(scenarioId)) {
      return NextResponse.json(
        {
          error:
            "Invalid scenario ID format. Use only alphanumeric characters, hyphens, and underscores.",
        },
        { status: 400 },
      );
    }
    ...
    const validationResult =
      (await validationResponse.json()) as ValidateScenarioResponse;
...
Co-Authored-By: Claude <noreply@anthropic.com>`,
    ...
    return NextResponse.json(
      {
        error:
          "Failed to publish scenario to GitHub. Please check server logs for details.",
      },
      { status: 500 },
    );
  }
