  ...
  describe("GET - Fetch Evaluations", () => {
    it("should return evaluations by userId", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/evaluations?userId=user-123",
      );
      ...
      expect(mockEvaluationRepo.getUserProgress).toHaveBeenCalledWith(
        "user-123",
      );
    });
    ...
    it("should return evaluations by programId", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/evaluations?programId=prog-456",
      );
      const response = await GET(request);
      const data = await response.json();
    ...
    it("should return evaluations by taskId", async () => {
      ...
      const request = new NextRequest(
        "http://localhost:3000/api/evaluations?taskId=task-789",
      );
      const response = await GET(request);
      const data = await response.json();
