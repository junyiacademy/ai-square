  ...
  beforeEach(() => {
    ...
    localStorageMock.getItem.mockImplementation((key) => {
      ...
      if (key === "assessmentResult")
        return JSON.stringify(mockAssessmentResult);
      ...
      await waitFor(
        () => {
          const element = screen.queryByText(
            "Areas for improvement: managing_with_ai, creating_with_ai",
          );
          if (element) expect(element).toBeInTheDocument();
        },
        { timeout: 1000 },
    ...
    it("should show completion status for completed scenarios", async () => {
      ...
      localStorageMock.getItem.mockImplementation((key) => {
        ...
        if (key === "assessmentResult")
          return JSON.stringify(mockAssessmentResult);
        if (key === "userProfile") return JSON.stringify(mockProfile);
        if (key === "completedScenarios")
          return JSON.stringify(completedScenarios);
        return null;
      });
    ...
    it("should boost relevance for matching keywords", async () => {
      ...
      const educationalScenarios = [
        ...mockScenarios,
        {
          ...
          description: {
            en: "Use AI to create educational content and curriculum",
          },
          difficulty: "intermediate",
          estimatedTime: 40,
