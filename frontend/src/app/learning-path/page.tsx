...
const roleKeywordPatterns: { [key: string]: RegExp } = {
  ...
  professional:
    /\b(business|career|job|work|productivity|manage|professional)\w*/i,
...
function LearningPathContent() {
  ...
  const generatePersonalizedReason = (
    ...
    if (domainScore < 60) {
      reasons.push(
        `Strengthen your ${domainName} skills (current: ${domainScore}%)`,
      );
    } else if (domainScore >= 80) {
      reasons.push(
        `Advance your ${domainName} expertise (current: ${domainScore}%)`,
      );
    } else {
      reasons.push(
        `Improve your ${domainName} competency (current: ${domainScore}%)`,
      );
    }
    ...
    if (userProfile.interests && userProfile.interests.length > 0) {
      ...
      const matchedInterests = userProfile.interests.filter(
        (interest: string) => scenarioText.includes(interest.toLowerCase()),
      );

      if (matchedInterests.length > 0) {
        reasons.push(
          `Aligns with your interests in ${matchedInterests.join(" & ")}`,
        );
      }
    }
      ...
      typeof scenario.title === "string"
        ...
        : typeof scenario.title === "object" && scenario.title
          ? (scenario.title as Record<string, string>).en ||
            (scenario.title as Record<string, string>).zh ||
            ""
          : "";
    if (
      scenarioTitle.toLowerCase().includes("job") &&
      userProfile.identity === "professional"
    ) {
      reasons.push("Directly applicable to your career development");
    } else if (
      scenarioTitle.toLowerCase().includes("educat") &&
      userProfile.identity === "teacher"
    ) {
  ...
  const calculateRelevanceScore = (
    scenario: ScenarioListItem & { matchedKeywords?: string[] },
  ...
  const generateLearningPath = async (result: AssessmentResult) => {
    try {
      ...
      const scenarios =
        scenariosData.data?.scenarios || scenariosData.scenarios || [];
      ...
      Object.entries(result.domainScores).forEach(([domain, score]) => {
        ...
        const domainScenarios = (scenarios as ScenarioWithDomains[]).filter(
          (s) =>
            s.domains?.includes(domain) ||
            s.domains?.includes(domainKey) ||
            s.targetDomain?.includes(domain) ||
            s.targetDomain?.includes(domainKey),
        );
        ...
        if (isWeak) {
          ...
          let beginnerScenarios = domainScenarios.filter(
            (s) =>
          ...
          if (userProfile.identity) {
            beginnerScenarios = beginnerScenarios
              .map((s) => ({
                ...s,
          ...
          beginnerScenarios
            ...
            .forEach(
              (
                ...
                const reason = generatePersonalizedReason(
                  scenario,
                  domain,
                  score,
                  matchedKeywords,
                    ...
                    typeof scenario.description === "string"
                      ...
                      : typeof scenario.description === "object" &&
                        ...
                        ? (scenario.description as Record<string, string>).en ||
                          (scenario.description as Record<string, string>).zh ||
                          ""
                        : "",
          ...
          let advancedScenarios = domainScenarios.filter(
            (s) =>
          ...
          let intermediateScenarios = domainScenarios.filter(
            (s) => s.difficulty === "intermediate",
          );

          // Sort by relevance score if user has identity
          if (userProfile.identity) {
            intermediateScenarios = intermediateScenarios
              .map((s) => ({
                ...s,
                ...calculateRelevanceScore(s),
                relevanceScore: calculateRelevanceScore(s).score,
  ...
  if (filterMode === "weak" && assessmentResult) {
    ...
    filteredPath = filteredPath.filter((item) =>
      weakDomains.includes(item.domain),
    );
  }
  ...
  const totalEstimatedTime = filteredPath.reduce(
            ...
            <div className="flex items-center">
              ...
              <div className="flex-1">
                ...
                <p className="text-yellow-700 dark:text-yellow-300 text-sm mt-1">
                  We can provide more personalized learning paths if you tell us
                  about your identity and interests. Update your profile in the
                  settings to get recommendations tailored to your role.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {domainProgress.map((progress) => (
                <div
                  ...
                  onClick={() =>
                    setSelectedDomain(
                      selectedDomain === progress.domain
                        ? null
                        : progress.domain,
                    )
                  }
                >
                  <div
                    className={`p-6 rounded-xl bg-gradient-to-br ${getDomainColor(progress.domain)} ${
                      selectedDomain === progress.domain
                    ...
                    <div className="space-y-2">
                      <div className="bg-white/20 rounded-full h-3 overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${(progress.completedItems / progress.totalItems) * 100 || 0}%`,
                          }}
                        />
                      </div>
        ...
        {selectedDomain && (
          <div className="mb-6 flex items-center justify-between">
            <p className="text-gray-600 dark:text-gray-400">
              {t("learningPath:learningPath.filteringBy", {
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center mb-2">
                      ...
                      <span
                        className={`ml-3 px-2 py-1 text-xs rounded-full ${getDifficultyColor(item.difficulty)}`}
                      >
                        {t(`common:difficulty.${item.difficulty}`)}
                      </span>
                      {/* Show badge if personalized for user's role */}
                      {userProfile.identity &&
                        (item.reason.includes("Perfect for") ||
