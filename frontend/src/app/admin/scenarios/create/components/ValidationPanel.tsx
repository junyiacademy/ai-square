...
import type { ValidateScenarioResponse, ValidationError } from '@/types/prompt-to-course';
...
export function ValidationPanel({ yaml, mode }: ValidationPanelProps) {
  const [validationResult, setValidationResult] = useState<ValidateScenarioResponse | null>(null);
      ...
      >
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            {item.path !== 'root' && (
              <div className="text-xs font-mono opacity-70 mb-1">{item.path}</div>
            )}
    ...
    <div className="space-y-4">
      ...
      <div className={`p-4 rounded-lg border-2 ${
        ...
        <div className="mt-2 text-sm">
          {validationResult.errors.length} errors, {validationResult.warnings.length} warnings, {validationResult.info.length} info
        </div>
      </div>
