...
describe("FixDemoAccountsPage", () => {
  ...
  it("should render the admin fix demo interface", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Fix Demo Accounts" }),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        "This tool will reset the demo accounts with proper passwords.",
      ),
    ).toBeInTheDocument();
  });
  ...
  it("should show fix demo button", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Fix Demo Accounts" }),
    ).toBeInTheDocument();
  });
  ...
  it("should show demo account information", () => {
    ...
    expect(
      screen.getByText("• Student: student@example.com / student123"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("• Teacher: teacher@example.com / teacher123"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("• Admin: admin@example.com / admin123"),
    ).toBeInTheDocument();
  });
  ...
  it("should show detailed results after successful operation", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("Actions Performed:")).toBeInTheDocument();
      expect(
        screen.getByText(/student@example.com.*created.*student/),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/teacher@example.com.*updated.*teacher/),
      ).toBeInTheDocument();
  ...
  it("should reset error state when starting new operation", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Network error: First error"),
      ).toBeInTheDocument();
    });
    ...
    await waitFor(() => {
      expect(
        screen.queryByText("Network error: First error"),
