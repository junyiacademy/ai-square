...
describe("TermsOfServicePage", () => {
  ...
  it("should display all major sections", () => {
    ...
    expect(
      screen.getByText("6. Privacy and Data Protection"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("7. AI Services and Limitations"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("8. Disclaimers and Limitations"),
    ).toBeInTheDocument();
  ...
  it("should show service features", () => {
    render(<TermsOfServicePage />);
    expect(
      screen.getByText("AI literacy education content and assessments"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Problem-Based Learning (PBL) scenarios"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("AI-powered tutoring and feedback"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Progress tracking and analytics"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Multi-language support (9 languages)"),
    ).toBeInTheDocument();
  });
  ...
  it("should show account types", () => {
    render(<TermsOfServicePage />);
    expect(
      screen.getByText("Student: Access to learning content and assessments"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Teacher: Additional class management features"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Administrator: Platform management capabilities"),
    ).toBeInTheDocument();
  });
  ...
  it("should show acceptable use policy", () => {
    render(<TermsOfServicePage />);
    expect(
      screen.getByText("Use the platform for educational purposes"),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Misuse or attempt to manipulate the AI systems"),
    ).toBeInTheDocument();
  });
  ...
  it("should display contact email", () => {
    ...
    const emailLink = screen.getByRole("link", {
      name: /support@junyiacademy.org/i,
    });
    expect(emailLink).toBeInTheDocument();
    expect(emailLink).toHaveAttribute(
      "href",
      "mailto:support@junyiacademy.org",
    );
  });
