...
jest.mock("@/utils/locale", () => ({
  formatDateWithLocale: jest.fn((date: string) =>
...
describe("ChatPage", () => {
  ...
  describe("Component Rendering", () => {
    it("should render main layout components", async () => {
      ...
      await waitFor(() => {
        expect(screen.getAllByTestId("header").length).toBeGreaterThanOrEqual(
          1,
        );
        expect(
          screen.getAllByTestId("panel-group").length,
    ...
    it("should show loading state initially", async () => {
      ...
      expect(
        screen.getAllByText("Please log in to start chatting")[0],
      ).toBeInTheDocument();
    });
    ...
    it("should handle authentication failure", async () => {
      ...
      await waitFor(() => {
        expect(console.error).toHaveBeenCalledWith(
          "Failed to load user and sessions:",
          expect.any(Error),
        );
      });
    });
  ...
  describe("Message Display", () => {
    ...
    beforeEach(() => {
      mockFetch.mockImplementation((url, options) => {
        ...
        return (
          mockFetch.getMockImplementation()?.(url, options) ||
          Promise.resolve({
            ok: true,
            json: () => Promise.resolve({}),
          })
    ...
    it("should render messages with correct styling classes", async () => {
      ...
      await waitFor(
        () => {
          ...
          if (hello && hiThere) {
            const userMessage =
              hello.closest('[class*="blue"]') || hello.parentElement;
            const assistantMessage =
              hiThere.closest('[class*="white"]') || hiThere.parentElement;
    ...
    it("should display scenario scores", async () => {
      ...
      await waitFor(
        () => {
          ...
          expect(
            score85Elements.length > 0 || score92Elements.length > 0,
          ).toBeTruthy();
        },
        { timeout: 3000 },
      );
  ...
  describe("Streak Calculation", () => {
    it("should calculate consecutive day streak correctly", async () => {
      ...
      mockFetch.mockImplementation((url) => {
        ...
        return (
          mockFetch.getMockImplementation()?.(url) ||
          Promise.resolve({
            ok: true,
            json: () => Promise.resolve({}),
          })
