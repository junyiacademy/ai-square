  ...
  useTranslation: () => ({
    t: (key: string, options?: any) => {
      ...
      const translations: Record<string, string> = {
        ...
        'navigation:historySubtitle': 'Track your AI literacy learning progress',
...
describe('UnifiedHistoryPage', () => {
  ...
  describe('Empty States', () => {
    beforeEach(() => {
      ...
      mockFetch.mockImplementation(() => Promise.resolve({
        ok: true,
        json: () => Promise.resolve({ data: [], success: true, programs: [] }),
      }));
    });
    ...
    it('should show empty state when no history items exist', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('No learning history found')).toBeInTheDocument();
        expect(screen.getByText('Take Assessment')).toBeInTheDocument();
  ...
  describe('PBL Item Display', () => {
    ...
    it('should display PBL sessions with scores correctly', async () => {
        ...
        programs: [
          {
            ...
            overallScore: 88,
            tasks: [
              { id: 'task-1', title: 'Ethics Analysis', log: { interactions: [1, 2, 3] } }
            ],
            domainScores: {
              engaging_with_ai: 85,
              creating_with_ai: 90,
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Program ID: pbl-program-1')).toBeInTheDocument();
        ...
        programs: [
          {
            ...
            totalTaskCount: 3,
            tasks: [
              { id: 'task-1', title: 'Ethics Analysis', log: { interactions: [1, 2] } }
            ],
          },
        ],
      };
    ...
    it('should handle PBL continue study navigation', async () => {
      ...
      const continueButton = screen.getByRole('button', { name: /continue study/i });
      await user.click(continueButton);

      expect(mockPush).toHaveBeenCalledWith('/pbl/scenarios/scenario-1/programs/pbl-program-1/tasks/task-1');
    });
    ...
    it('should handle PBL view report navigation', async () => {
      ...
      await waitFor(() => {
        ...
        expect(viewReportLink.closest('a')).toHaveAttribute('href', '/pbl/programs/pbl-program-1/complete');
      });
    });
  });
  ...
  describe('Discovery Item Display', () => {
    ...
    it('should display Discovery sessions with scores correctly', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Program ID: discovery-program-1')).toBeInTheDocument();
        expect(screen.getByText('Career Type: data_scientist')).toBeInTheDocument();
    ...
    it('should handle Discovery continue study navigation for active sessions', async () => {
      ...
      expect(mockPush).toHaveBeenCalledWith('/discovery/scenarios/discovery-scenario-1/programs/discovery-program-1/tasks/task-3');
    });
  });
        ...
        data: [
          {
            ...
            summary: { total_questions: 20, correct_answers: 18, level: 'expert' },
            duration_seconds: 1200,
            language: 'en',
          },
          {
            ...
            summary: { total_questions: 20, correct_answers: 15, level: 'advanced' },
            duration_seconds: 1200,
            language: 'en',
          },
          {
            ...
            summary: { total_questions: 20, correct_answers: 9, level: 'beginner' },
            duration_seconds: 1200,
            language: 'en',
          },
        ],
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle discovery programs with no scenarios returned', async () => {
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith('Discovery API returned non-OK status:', 404);
      });
  ...
  describe('Action Buttons', () => {
    ...
    it('should display action buttons at the bottom', async () => {
      ...
      await waitFor(() => {
        ...
        expect(takeNewAssessment.closest('a')).toHaveAttribute('href', '/assessment');
        expect(startNewPBL.closest('a')).toHaveAttribute('href', '/pbl');
        expect(startNewDiscovery.closest('a')).toHaveAttribute('href', '/discovery');
      });
    });
