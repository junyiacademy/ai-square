...
export default function AssessmentPage() {
    ...
    const fetchAssessmentData = async () => {
      ...
      try {
        const response = await authenticatedFetch(
          `/api/assessment?lang=${i18n.language}`,
        );
        if (!response.ok) {
  ...
  const calculateAssessmentResult = (
    ...
    const domainPercentages = Object.keys(domainScores).reduce(
      (acc, domain) => {
  ...
  const generateRecommendations = (
    ...
    sortedDomains.slice(0, 2).forEach(([domain]) => {
      ...
      const domainName = getTranslatedField(
        i18n.language,
        domainInfo,
        "name",
      ) as string;
      const domainDescription = getTranslatedField(
        i18n.language,
        domainInfo,
        "description",
      ) as string;
  ...
  const getTranslatedField = (
    ...
    if (langCode !== "en") {
      return (obj[`${fieldName}_${langCode}`] ||
        obj[fieldName] ||
        "") as string;
    }
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                <div className="bg-blue-50 p-6 rounded-lg">
                  ...
                  <ul className="text-blue-700 space-y-2 text-left">
                    <li>
                      •{" "}
                      {t("assessmentInfo.questions", {
                        count: assessmentData.assessment_config.total_questions,
                      })}
                    </li>
                    <li>
                      •{" "}
                      {t("assessmentInfo.timeLimit", {
                        minutes:
                          assessmentData.assessment_config.time_limit_minutes,
                      })}
                    </li>
