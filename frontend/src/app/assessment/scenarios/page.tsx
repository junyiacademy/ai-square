...
export default function AssessmentScenariosPage() {
  ...
  const loadAssessmentScenarios = useCallback(async () => {
    ...
    if (
      ...
      now - scenariosCache[cacheKey].timestamp < CACHE_TTL
    ) {
    ...
    try {
      const res = await authenticatedFetch(
        `/api/assessment/scenarios?lang=${i18n.language}`,
      );
            ...
            >
                ...
                <div className="flex items-start justify-between">
                  ...
                  {scenario.userProgress?.bestScore !== undefined && (
                    <div className="text-sm text-green-600 font-semibold">
                      {t("assessment:best", "Best")}:{" "}
                      {scenario.userProgress.bestScore}%
                    </div>
                  )}
                </div>
              ...
              <CardContent>
                <div className="space-y-2 text-sm">
                  <div className="flex items-center gap-2 text-gray-600">
                    ...
                    <span>
                      ...
                      {t("assessment:questions", "questions")}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-gray-600">
                    <Clock className="h-4 w-4" />
                    <span>
                      {scenario.config.timeLimit}{" "}
                ...
                {scenario.userProgress &&
                  scenario.userProgress.completedPrograms > 0 && (
                    <div className="mt-4 pt-4 border-t text-sm text-gray-600">
                      <div className="flex items-center justify-between">
                        <span>
                          {scenario.userProgress.completedPrograms}{" "}
                          {t("assessment:attempts", "attempts")}
                        </span>
                        {scenario.userProgress.lastAttempt && (
                          <span>
                            {t("assessment:last", "Last")}:{" "}
                            {new Date(
                              scenario.userProgress.lastAttempt,
                            ).toLocaleDateString()}
                          </span>
                ...
                <div className="mt-4 pt-4 border-t">
                  <Button
                    className="w-full"
                    onClick={() =>
                      router.push(`/assessment/scenarios/${scenario.id}`)
                    }
                  >
