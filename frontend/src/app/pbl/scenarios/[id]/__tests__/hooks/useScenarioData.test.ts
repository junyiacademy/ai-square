...
describe('useScenarioData', () => {
  ...
  it('fetches scenario and programs data on mount', async () => {
    ...
    const { result } = renderHook(() => useScenarioData(mockScenarioId, mockLanguage));
    ...
    expect(result.current.scenario?.objectives).toEqual(['Objective 1', 'Objective 2']);
  ...
  it('cleans up on unmount to prevent state updates', async () => {
    ...
    const { unmount } = renderHook(() => useScenarioData(mockScenarioId, mockLanguage));

    // Unmount before promises resolve
    unmount();

    // Resolve promises after unmount
    resolveScenario!({ success: true, data: { id: 'test', learningObjectives: [] } });
