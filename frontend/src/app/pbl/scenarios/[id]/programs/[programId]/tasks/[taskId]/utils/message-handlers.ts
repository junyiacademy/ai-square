...
export async function createProgramIfNeeded(
  ...
  const createRes = await authenticatedFetch(`/api/pbl/scenarios/${params.scenarioId}/start`, {
    method: 'POST',
    headers: {
...
export async function saveInteraction(params: SaveInteractionParams): Promise<void> {
  ...
  if (!params.taskId.match(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)) {
    console.log('Skipping interaction save - no valid task ID yet');
    return;
  }

  const saveRes = await authenticatedFetch(`/api/pbl/tasks/${params.taskId}/interactions`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
...
export async function getAIResponse(params: GetAIResponseParams): Promise<string> {
  const aiRes = await authenticatedFetch(`/api/pbl/chat?lang=${params.language}`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Accept-Language': params.language
    ...
    body: JSON.stringify({
      ...
      context: {
        ...
        taskTitle: getLocalizedField(params.currentTask as unknown as Record<string, unknown>, 'title', params.language),
        taskDescription: getLocalizedField(params.currentTask as unknown as Record<string, unknown>, 'description', params.language),
        instructions: Array.isArray(params.currentTask.instructions) ? params.currentTask.instructions : [],
        expectedOutcome: getLocalizedField(params.currentTask as unknown as Record<string, unknown>, 'expectedOutcome', params.language),
...
export async function navigateToNextTask(params: NavigateToNextTaskParams): Promise<void> {
  try {
    const tasksRes = await authenticatedFetch(`/api/pbl/programs/${params.programId}/tasks`);
    if (tasksRes.ok) {
      const tasks = await tasksRes.json();
      const sortedTasks = tasks.sort((a: { taskIndex: number }, b: { taskIndex: number }) => a.taskIndex - b.taskIndex);
      const currentIndex = sortedTasks.findIndex((t: { id: string }) => t.id === params.currentTask.id);
      ...
      if (currentIndex !== -1 && currentIndex < sortedTasks.length - 1) {
        ...
        params.router.push(`/pbl/scenarios/${params.scenarioId}/programs/${params.programId}/tasks/${nextTaskId}`);
      } else {
        console.log('All tasks completed or task not found, going to complete page');
        params.router.push(`/pbl/scenarios/${params.scenarioId}/programs/${params.programId}/complete`);
      }
    } else {
