...
import { Program, Scenario, Task, DomainType } from "@/types/pbl";
...
export default function ProgramLearningPage({
                ...
                try {
                  ...
                  if (taskRes.ok) {
                    ...
                    if (taskData) {
                      ...
                      // eslint-disable-next-line @typescript-eslint/no-unused-vars
                      const _taskTemplate =
                        taskData.content?.context?.taskTemplate || {};
                      // eslint-disable-next-line @typescript-eslint/no-unused-vars
                      const _originalTaskData =
      ...
      if (!programId.startsWith("temp_")) {
        try {
          ...
          if (tasksRes.ok) {
            ...
            const evalPromises = sortedTasks.map(
              async (task: { id: string }) => {
                try {
                  ...
                  console.error(
                    `Error loading evaluation for task ${task.id}:`,
                    err,
                  );
                }
                return null;
  ...
  const loadTaskHistory = async (taskToLoad?: Task) => {
    ...
    try {
      ...
      if (res.ok) {
          ...
          if (data.data?.evaluationId) {
            ...
            if (evalRes.ok) {
              ...
              if (evalData.data?.evaluation) {
                ...
                const currentUserMessageCount = loadedConversations.filter(
                  (c: ConversationEntry) => c.type === "user",
                ).length;
        ...
        if (res.status === 401) {
          console.log(
            "Authentication required for task history - user may need to log in",
          );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (
        ...
        saveUserRes = await authenticatedFetch(
          `/api/pbl/tasks/${taskIdToUse}/interactions`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
      ...
      if (!saveUserRes.ok) {
        ...
        if ("text" in saveUserRes) {
          const errorText = await saveUserRes
            .text()
            .catch(() => "Unknown error");
          let errorData;
          try {
      ...
      const aiRes = await authenticatedFetch(
        `/api/pbl/chat?lang=${i18n.language}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          body: JSON.stringify({
            ...
            context: {
              ...
              instructions: Array.isArray(currentTask.instructions)
      ...
      await authenticatedFetch(
        `/api/pbl/tasks/${currentTask.id}/interactions`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleCompleteTask = async () => {
    if (!currentTask || !program) {
      console.log("handleCompleteTask: Missing currentTask or program", {
        currentTask,
        program,
      });
      return;
    ...
    try {
      ...
      if (tasksRes.ok) {
        ...
        const currentIndex = sortedTasks.findIndex(
          (t: { id: string }) => t.id === currentTask.id,
        );
        ...
        if (currentIndex !== -1 && currentIndex < sortedTasks.length - 1) {
          ...
          router.push(
            `/pbl/scenarios/${scenarioId}/programs/${programId}/tasks/${nextTaskId}`,
          );
        } else {
          ...
          router.push(
            `/pbl/scenarios/${scenarioId}/programs/${programId}/complete`,
          );
        }
      } else {
        ...
        <div className="hidden md:flex w-full h-full">
          ...
          >
            <div className="h-full flex flex-col">
              <div
                className={`flex items-center justify-between ${isProgressCollapsed ? "px-2 py-4" : "p-4"}`}
              >
                <h3
              ...
              {/* Vertical Progress */}
              <div
                className={`flex-1 relative ${isProgressCollapsed ? "px-2" : "px-4"}`}
              >
                {!isProgressCollapsed && (
                  <>
                    ...
                    <div className="space-y-6 relative">
                      {scenario.tasks.map((task, index) => {
                        ...
                        const isCurrent =
                          currentTask && currentTask.id === actualTaskId;
                    ...
                    <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <StarRating
                            score={evaluation.ksaScores.skills}
                            size="sm"
                          />
                        </div>
                        <div className="flex items-center justify-between">
                ...
                <div className="space-y-4">
                  ...
                  {currentTask.expectedOutcome && (
                    <div>
                      ...
                      <p className="text-gray-600 dark:text-gray-400">
                        {i18n.language === "zhTW"
                          ? currentTask.expectedOutcome_zhTW ||
                            currentTask.expectedOutcome
                          : currentTask.expectedOutcome}
                      </p>
                    </div>
