...
interface DiscoveryCompletionData {
  ...
  qualitativeFeedbackVersions?: Record<
    string,
    {
      overallAssessment: string;
      careerAlignment: string;
...
export default function DiscoveryCompletePage() {
  ...
  const [completionData, setCompletionData] =
    useState<DiscoveryCompletionData | null>(null);
    ...
    async (targetLang: string) => {
      ...
      try {
        ...
        const response = await authenticatedFetch(
          `/api/discovery/programs/${params.programId}/translate-feedback`,
          {
            method: "POST",
            headers: {
    ...
    async (regenerate = false) => {
      try {
        ...
        const completeResponse = await authenticatedFetch(
          `/api/discovery/programs/${params.programId}/complete`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const programResponse = await authenticatedFetch(
          `/api/discovery/programs/${params.programId}`,
        );
        ...
        setCompletionData({
          ...
          overallScore:
            evalData.evaluation.overallScore || evalData.evaluation.score || 0,
          totalXP: evalData.evaluation.totalXP || programData.totalXP || 0,
          totalTasks:
            evalData.evaluation.totalTasks || programData.totalTasks || 0,
          completedTasks:
            evalData.evaluation.completedTasks ||
            programData.completedTasks ||
            0,
          ...
          qualitativeFeedbackVersions:
            evalData.evaluation.qualitativeFeedbackVersions || {},
        });
      } catch (err) {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      // Call regenerate API
      const regenerateResponse = await authenticatedFetch(
        `/api/discovery/programs/${params.programId}/regenerate`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          ...
          <p className="text-red-600 mb-4">
            {error || t("discovery:complete.noData")}
          </p>
          <Link
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-xl text-gray-600">
            {t("discovery:complete.completedProgram", {
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <div className="text-sm text-gray-600">
              {t("discovery:complete.totalXP")}
            </div>
          </div>

          <div className="bg-white rounded-lg shadow p-6 text-center">
            <div
              className={`text-3xl font-bold mb-2 ${getScoreColor(completionData.overallScore)}`}
            >
              {completionData.overallScore}
            </div>
            <div className="text-sm text-gray-600">
              {t("discovery:complete.overallScore")}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            ...
            <div className="text-sm text-gray-600">
              {t("discovery:complete.tasksCompleted")}
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6 text-center">
            <div className="flex justify-center items-center text-gray-700 mb-2">
              ...
              <span className="text-xl font-semibold">
                {completionData.daysUsed !== undefined &&
                completionData.daysUsed !== null
                  ? completionData.daysUsed === 0
                    ? 1
                    : completionData.daysUsed
                  : "N/A"}
              </span>
            </div>
            <div className="text-sm text-gray-600">
              {t("discovery:complete.daysUsed")}
            </div>
          </div>
        </div>

        {/* Achievements Section */}
        {completionData.achievementsUnlocked &&
          completionData.achievementsUnlocked.length > 0 && (
        ...
        {/* Skill Improvements */}
        {completionData.skillImprovements &&
          completionData.skillImprovements.length > 0 && (
            <div className="bg-white rounded-lg shadow p-6 mb-8">
              <h2 className="text-xl font-semibold mb-4 flex items-center">
                ...
                {completionData.skillImprovements.map((skill) => (
                  ...
                  >
                    ...
                    <div className="flex justify-between text-xs text-gray-600 mt-1">
                      <span>
                        {t("discovery:complete.previousLevel")}:{" "}
                        {skill.previousLevel}%
                      </span>
                      <span>
                        {t("discovery:complete.currentLevel")}:{" "}
                        {skill.currentLevel}%
                      </span>
                    </div>
                  </div>
                  ...
                  <tr key={task.taskId}>
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div className="flex items-center">
                        ...
                        <span className="ml-2">
                          {t(`discovery:taskTypes.${task.taskType}`)}
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-4 whitespace-nowrap text-sm text-center">
                      <span
                        className={`font-semibold ${getScoreColor(task.score)}`}
                      >
                        {task.score}
                      </span>
                    </td>
        ...
        {completionData.qualitativeFeedback && (
          <div className="bg-white rounded-lg shadow p-6 mb-8">
            ...
            <div
              className={`space-y-4 ${translatingFeedback ? "opacity-50" : ""}`}
            >
              <div>
              ...
              <div>
                ...
                <div className="text-gray-700 prose prose-sm max-w-none">
                  <ReactMarkdown>
                    {completionData.qualitativeFeedback.careerAlignment}
                  </ReactMarkdown>
                </div>
              </div>
              ...
              {completionData.qualitativeFeedback.strengths.length > 0 && (
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {completionData.qualitativeFeedback.strengths.map(
                      (strength, idx) => (
              ...
              {completionData.qualitativeFeedback.growthAreas.length > 0 && (
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {completionData.qualitativeFeedback.growthAreas.map(
                      (area, idx) => (
                        <li key={idx} className="text-gray-700">
                          <span className="prose prose-sm max-w-none inline">
              ...
              {completionData.qualitativeFeedback.nextSteps.length > 0 && (
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    {completionData.qualitativeFeedback.nextSteps.map(
                      (step, idx) => (
                        <li key={idx} className="text-gray-700">
                          <span className="prose prose-sm max-w-none inline">
