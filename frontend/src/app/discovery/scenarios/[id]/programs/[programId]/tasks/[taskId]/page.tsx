...
export default function TaskDetailPage({
    ...
    const fetchTaskData = async () => {
      try {
        ...
        const res = await authenticatedFetch(
          `/api/discovery/scenarios/${scenarioId}/programs/${programId}/tasks/${taskId}?lang=${currentLanguage}`,
          {
            credentials: "include",
            headers: {
              "x-session-token":
                localStorage.getItem("ai_square_session") || "",
            },
          },
        );
        ...
        if (data.status === "pending") {
          await authenticatedFetch(
            `/api/discovery/scenarios/${scenarioId}/programs/${programId}/tasks/${taskId}`,
            {
              method: "PATCH",
              credentials: "include",
              headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const updatedTaskRes = await authenticatedFetch(
        `/api/discovery/scenarios/${scenarioId}/programs/${programId}/tasks/${taskId}`,
        {
          credentials: "include",
          headers: {
      ...
      if (result.evaluation && taskData) {
        setTaskData({
          ...
          evaluation: {
            ...
            feedback:
              result.evaluation.feedback || result.evaluation.feedbackText,
      ...
      <DiscoveryPageLayout>
        <div className="max-w-4xl mx-auto px-4 py-16 text-center">
          ...
          <button
            onClick={() =>
              router.push(
                `/discovery/scenarios/${scenarioId}/programs/${programId}`,
              )
            }
  ...
  const passedInteractions = taskData.interactions.filter(
    (i) =>
      i.type === "ai_response" &&
      (i.content as Record<string, unknown>)?.completed === true,
  );
  const hasPassedBefore = passedInteractions.length > 0;
  const passCount = passedInteractions.length;
  const bestScore = hasPassedBefore
    ? Math.max(
        ...passedInteractions.map(
          (i) =>
            ((i.content as Record<string, unknown>)?.xpEarned as number) || 0,
        ),
      )
    : 0;
  const latestPassScore = hasPassedBefore
    ? ((
        passedInteractions[passedInteractions.length - 1]?.content as Record<
          string,
          unknown
        >
    ...
    <DiscoveryPageLayout>
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
          ...
          {/* Completion Criteria */}
          {taskData.content.completionCriteria &&
            taskData.content.completionCriteria.length > 0 && (
                ...
                {(() => {
                  ...
                  taskData.interactions
                    .filter(
                      (i) =>
                        i.type === "ai_response" &&
                        (i.content as Record<string, unknown>)?.skillsImproved,
                    )
                    .forEach((i) => {
                      const skills = (i.content as Record<string, unknown>)
                      ...
                      <span className="inline-flex items-center space-x-1">
                        {Array.from({ length: passCount }, (_, i) => (
                          <button
                            key={i}
                            onClick={() => {
                              const passedElement = document.getElementById(
                                `passed-interaction-${i}`,
                              );
                              passedElement?.scrollIntoView({
                                behavior: "smooth",
                                block: "center",
                              });
                            }}
                ...
                {taskData.interactions.map((interaction, index) => {
                  ...
                  return (
                    <div
                      key={index}
                      id={
                        passedInteractionIndex >= 0
                          ? `passed-interaction-${passedInteractionIndex}`
                          : undefined
                      }
                      className={`
                      rounded-lg p-4
                          ...
                          {interaction.type === "user_input" ? (
                            ...
                            <>
                              ...
                              >
                                ...
                                (interaction.content as Record<string, unknown>)
                                  .completed ? (
        ...
        {/* Hints Section */}
        {showHints &&
          taskData.content.hints &&
          taskData.content.hints.length > 0 && (
