...
export function createMockRepository<T>(defaultData: T[] = []) {
  return {
    ...
    findActive: jest.fn().mockResolvedValue(defaultData.filter((item: unknown) => (item as Record<string, unknown>).status === 'active')),
...
export function createMockRepositoryFactory() {
  return {
    ...
    getEvaluationRepository: jest.fn().mockReturnValue(mockEvaluationRepository),
    getUserRepository: jest.fn().mockReturnValue(mockUserRepository),
    getAchievementRepository: jest.fn().mockReturnValue(mockAchievementRepository),
  };
}
