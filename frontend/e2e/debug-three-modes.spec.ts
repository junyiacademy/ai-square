...
test.describe("Three Modes Debug - 嚴格檢查", () => {
  ...
  test("PBL Mode 完整流程檢查 - Program → Task → Completion", async ({
    page,
  }) => {
    ...
    // 2. 必須能看到 PBL 標題
    await expect(page.locator('h1:has-text("PBL")')).toBeVisible({
      timeout: 10000,
    });
    ...
    const scenarioCards = page.locator(
      '.grid div[class*="bg-white"], .grid div[class*="bg-gray-800"]',
    );
    ...
    const loadingElements = page.locator(
      '.animate-pulse, .loading, :has-text("Loading")',
    );
    ...
    const startButtons = page.locator(
      'button:has-text("Start New Program"), button:has-text("Continue"), button:has-text("Start"), button:has-text("開始"), button:has-text("Begin")',
    );
    ...
    const taskElements = page.locator(
      'h1, h2, .task-title, [data-testid*="task"]',
    );
    await expect(taskElements.first()).toBeVisible({ timeout: 10000 });
    console.log("✅ Task 載入成功，找到任務元素");

    // 8. 尋找互動元素並進行任務操作
    const interactionElements = page.locator(
      'textarea, input[type="text"], button[type="submit"]',
    );
    ...
    if (interactionCount > 0) {
      ...
      if (await textArea.isVisible({ timeout: 5000 })) {
        await textArea.fill(
          "這是我在 PBL 任務中的測試回答。我理解了問題的要求，並提供了相關的解決方案。",
        );
        console.log("✅ 成功填寫任務回答");

        // 尋找提交按鈕
        const submitBtn = page.locator(
          'button:has-text("Submit"), button:has-text("提交"), button:has-text("Continue"), button:has-text("繼續")',
        );
        if (await submitBtn.first().isVisible({ timeout: 5000 })) {
          ...
          // 檢查是否有評估錯誤
          const hasEvaluationError = errors.some((error) =>
            error.includes("Evaluation API error"),
          );
          ...
          const feedbackElements = page.locator(
            ".ai-response, .feedback, .next-task, .completion, .evaluation-result",
          );
    ...
    if (completionCount > 0) {
      ...
      if (await completeBtn.first().isVisible({ timeout: 5000 })) {
        ...
        // 檢查是否到達最終完成頁面
        const finalCompletion = page.locator(
          '.program-completed, .congratulations, h1:has-text("Congratulations")',
        );
    ...
    if (failed401s.length > 0) {
      console.log(
        `⚠️  PBL 流程中有 ${failed401s.length} 個 401 錯誤 (可能是 API 鑑權相關):`,
      );
      failed401s
        .slice(0, 3)
        .forEach((url, i) => console.log(`  ${i + 1}. ${url}`));
    }

    console.log(
      "✅ PBL Mode 完整流程檢查完成 - Program → Task → Completion 已驗證",
    );
  });

  test("Discovery Mode 完整流程檢查 - Program → Task → Completion", async ({
    page,
  }) => {
    ...
    const hasDiscoveryContent = await page
      .locator("h1, h2, h3")
      .filter({ hasText: /Discovery|Career|職業/ })
      .count();
    if (hasDiscoveryContent === 0) {
      console.log("⚠️  沒有找到 Discovery 相關標題");
    } else {
      console.log(
        `✅ Discovery 頁面載入成功 (${hasDiscoveryContent} 個相關元素)`,
      );
    }

    // 尋找職業卡片或項目
    const careerItems = page.locator(
      'article, .career-card, [data-testid*="career"], .grid > div',
    );
    ...
    if (itemCount > 0) {
      ...
      const detailElements = page.locator(
        "h1, h2, .career-details, .start-exploration",
      );
      ...
      if (detailCount > 0) {
        ...
        const startBtn = page.locator(
          'button:has-text("Start"), button:has-text("開始"), button:has-text("Explore"), button:has-text("探索")',
        );
        if (await startBtn.first().isVisible({ timeout: 5000 })) {
          ...
          if (urlContainsProgram) {
            ...
            // 檢查任務元素
            const taskElements = page.locator(
              ".task, .exploration-step, .career-activity, h2, h3",
            );
            const taskCount = await taskElements.count();

            if (taskCount > 0) {
              console.log(
                `✅ Discovery Task 載入成功，找到 ${taskCount} 個任務元素`,
              );

              // 尋找互動元素
              const interactionElements = page.locator(
                "input, textarea, button, select, .clickable",
              );
              const interactionCount = await interactionElements.count();
            ...
            console.log(
              "⚠️  Discovery Program 可能未成功創建或使用不同的 URL 結構",
            );
          }
        } else {
  ...
  test("Assessment Mode 完整流程檢查 - Program → Task → Completion", async ({
    page,
  }) => {
    ...
    // 檢查評估內容
    const hasAssessmentContent = await page
      .locator("h1, h2, h3")
      .filter({ hasText: /Assessment|評估|Evaluate/ })
      .count();
    ...
    const assessmentItems = page.locator(
      'article, .assessment-card, [data-testid*="assessment"], .grid > div',
    );
    ...
    if (itemCount > 0) {
      ...
      // 尋找開始按鈕
      const startBtn = page.locator(
        'button:has-text("Start"), button:has-text("Begin"), button:has-text("開始")',
      );
      if (await startBtn.first().isVisible({ timeout: 5000 })) {
        ...
        const urlContainsProgram =
          page.url().includes("/program/") ||
          page.url().includes("/assessment/");
        if (urlContainsProgram) {
          console.log("✅ Assessment Program 創建成功");

          // 檢查評估任務元素
          const questionElements = page.locator(
            '.question, .assessment-item, .quiz-question, h2:has-text("Question"), h3',
          );
          const questionCount = await questionElements.count();

          if (questionCount > 0) {
            console.log(
              `✅ Assessment Task 載入成功，找到 ${questionCount} 個問題元素`,
            );

            // 尋找答題元素
            const answerElements = page.locator(
            ...
            if (answerCount > 0) {
              ...
              // 尋找提交或下一題按鈕
              const submitBtn = page.locator(
                'button:has-text("Submit"), button:has-text("Next"), button:has-text("提交"), button:has-text("下一題")',
              );
              if (await submitBtn.first().isVisible({ timeout: 5000 })) {
                await submitBtn.first().click();
                await page.waitForTimeout(3000);
                ...
                // 檢查是否有分數或反饋
                const feedbackElements = page.locator(
                  ".score, .feedback, .result, .assessment-result",
                );
                const feedbackCount = await feedbackElements.count();
                if (feedbackCount > 0) {
                  console.log(
                    `✅ 收到評估反饋，找到 ${feedbackCount} 個反饋元素`,
                  );
                }

                // 檢查是否有完成頁面
                const completionElements = page.locator(
                  '.assessment-complete, .quiz-complete, h1:has-text("Complete"), h2:has-text("完成")',
                );
                const completionCount = await completionElements.count();
                if (completionCount > 0) {
          ...
          console.log(
            "⚠️  Assessment Program 可能未成功創建或使用不同的 URL 結構",
          );
        }
      } else {
