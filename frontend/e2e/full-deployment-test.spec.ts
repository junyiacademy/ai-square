...
const environments = [
  ...
  {
    name: "Production",
    url: "https://ai-square-production-731209836128.asia-east1.run.app",
  },
];
  ...
  test.describe(`${env.name} Environment Tests`, () => {
    ...
    users.forEach((user) => {
      test(`Login test for ${user.role}`, async ({ page }) => {
        ...
        console.log(
          `${env.name} - ${user.role}: Found ${scenarioCount} PBL scenarios`,
        );
    ...
    test("Complete user journey test", async ({ page }) => {
      ...
      if (count > 0) {
        ...
        const startButton = page.locator(
          'button:has-text("Start"), button:has-text("開始")',
        );
      ...
      const langSelector = page.locator(
        'select[aria-label*="language"], select[aria-label*="語言"]',
      );
      ...
      const userMenu = page.locator(
        'button:has-text("student@example.com"), [aria-label*="user menu"]',
      );
      if (await userMenu.isVisible()) {
        await userMenu.click();
        await page.click(
          'button:has-text("Sign out"), button:has-text("登出")',
        );
        await expect(page).toHaveURL(/\/(login|$)/);
      }
    });
    ...
    test("API health check", async ({ request }) => {
      ...
      for (const endpoint of endpoints) {
        ...
        console.log(
          `${env.name} ${endpoint}: ${json.scenarios?.length || 0} scenarios`,
        );
      }
    });
  });
...
test("Cross-environment consistency", async ({ request }) => {
  ...
  // 比較 scenario 數量
  const stagingPBL = await request.get(
    `${environments[0].url}/api/pbl/scenarios?lang=en`,
  );
  const prodPBL = await request.get(
    `${environments[1].url}/api/pbl/scenarios?lang=en`,
  );
