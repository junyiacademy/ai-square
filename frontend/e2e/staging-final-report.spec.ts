...
test.describe("Staging Environment Final Test Report", () => {
  ...
  test("Complete staging validation", async ({ page, request }) => {
    ...
    console.log(
      `   ✅ Database: ${healthData.checks.database.status ? "Connected" : "Disconnected"}`,
    );
    console.log(`   ✅ Environment: ${healthData.environment}`);
    console.log(
      `   ⚠️  Redis: ${healthData.checks.redis.status ? "Connected" : "Not available"}`,
    );
    ...
    // PBL
    const pblResponse = await request.get(
      `${STAGING_URL}/api/pbl/scenarios?lang=en`,
    );
    const pblData = await pblResponse.json();
    if (pblData.success) {
      console.log(
        `   ✅ PBL Scenarios: ${pblData.data.scenarios.length} loaded`,
      );
    } else {
    ...
    const discoveryResponse = await request.get(
      `${STAGING_URL}/api/discovery/scenarios?lang=en`,
    );
    ...
    if (discoveryData.error) {
      ...
      console.log(
        `   ✅ Discovery Scenarios: ${discoveryData.length || 0} loaded`,
      );
    }

    // Assessment
    const assessmentResponse = await request.get(
      `${STAGING_URL}/api/assessment/scenarios?lang=en`,
    );
    ...
    if (assessmentData.error) {
      ...
      console.log(
        `   ✅ Assessment Scenarios: ${assessmentData.length || 0} loaded`,
      );
    }
    ...
    if (loginResponse) {
      ...
      console.log(
        `   ❌ Login: Failed - ${loginData.error || loginData.message || "Unknown error"}`,
      );
    } else {
    ...
    console.log(
      "   ✅ Tables created: users, scenarios, programs, tasks, evaluations",
    );

    // 7. Known Issues Summary
    console.log("\n7. Known Issues:");
    console.log(
      "   ❌ Scenario initialization: Password authentication errors",
    );
