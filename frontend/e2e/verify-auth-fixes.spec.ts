...
test.describe("Authentication Fixes Verification - Staging", () => {
  ...
  test("should handle the specific PBL start endpoint that was causing 401 errors", async ({
    page,
  }) => {
    ...
    const response = await page.request.post(
      `${STAGING_URL}/api/pbl/scenarios/0b45586d-7baf-4d5a-a7f9-dfb9684b6d58/start`,
      {
        data: {},
      },
    );
    ...
    if (response.status() === 200) {
      ...
      console.log(
        "⚠️ Endpoint returned error, but properly formatted (not crashing):",
      );
      ...
      console.log(
        "✅ SUCCESS: Error handling is working properly (no more 401 crashes)",
      );
    }
    ...
    const authCheckResponse = await page.request.get(
      `${STAGING_URL}/api/auth/check`,
    );
  ...
  test("should handle unauthenticated requests gracefully (no crashes)", async ({
    page,
  }) => {
    ...
    console.log(
      "✅ SUCCESS: Unauthenticated requests handled gracefully with proper JSON errors",
    );
  });
});
