...
test('PBL scenario shows previous programs', async ({ page }) => {
  ...
  await page.goto('http://localhost:3000/pbl/scenarios/a5e6c365-832a-4c8e-babb-9f39ab462c1b');

  // Wait for the page to load
  await page.waitForLoadState('networkidle');

  // Check if programs section exists
  const programsSection = await page.locator('text=/Your Programs|Latest Program/i').first();
  ...
  if (hasProgramsSection) {
    ...
    const programItems = await page.locator('[class*="bg-white dark:bg-gray-800"]').count();
    console.log('Number of program items:', programItems);

    // Look for "Continue" or "View Results" buttons
    const continueButtons = await page.locator('button:has-text("Continue"), button:has-text("View Results")').count();
