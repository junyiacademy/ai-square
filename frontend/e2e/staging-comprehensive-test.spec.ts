...
test.describe("Staging Environment Comprehensive Test", () => {
  ...
  test("2. Homepage Loading and Navigation", async ({ page }) => {
    ...
    const langSelector = page
      .locator('select[aria-label*="language"], select[aria-label*="語言"]')
      .first();
  ...
  test("3. Login Functionality Test", async ({ page, context }) => {
    ...
    const submitButton = page
      .locator(
        'button[type="submit"], button:has-text("Sign in"), button:has-text("Login"), button:has-text("登入")',
      )
      .first();
    ...
    if (response) {
      ...
      if (loginData.success) {
        ...
        const hasAuthCookie = cookies.some(
          (c) => c.name.includes("accessToken") || c.name.includes("auth"),
        );
  ...
  test("4. PBL Scenarios Page", async ({ page }) => {
    ...
    if (!hasError) {
      ...
      const scenarioCards = await page
        .locator(
          '[data-testid="scenario-card"], .scenario-card, article, .grid > div',
        )
        .count();
      const emptyState =
        (await page.locator("text=/no scenarios|empty|沒有場景/i").count()) > 0;
      ...
      await page.screenshot({
        path: "staging-pbl-scenarios.png",
        fullPage: true,
      });
  ...
  test("5. Discovery Scenarios Page", async ({ page }) => {
    ...
    if (!hasError) {
      ...
      await page.screenshot({
        path: "staging-discovery-scenarios.png",
        fullPage: true,
      });
    } else {
  ...
  test("6. Assessment Scenarios Page", async ({ page }) => {
    ...
    if (!hasError) {
      const scenarioCards = await page
        .locator(
          '[data-testid="assessment-card"], .assessment-card, article, .grid > div',
        )
        .count();
      ...
      await page.screenshot({
        path: "staging-assessment-scenarios.png",
        fullPage: true,
      });
    } else {
  ...
  test("7. API Endpoints Direct Test", async ({ request }) => {
    ...
    const endpoints = [
      ...
      { name: "Discovery Scenarios", url: "/api/discovery/scenarios?lang=en" },
      {
        ...
        url: "/api/assessment/scenarios?lang=en",
      },
  ...
  test("8. Mobile Responsive Test", async ({ page }) => {
    ...
    console.log(
      `   - Mobile menu button: ${mobileMenuButton > 0 ? "Visible" : "Not found"}`,
    );
  ...
  test("9. Language Switching Test", async ({ page }) => {
    ...
    if (await langSelector.isVisible()) {
      ...
      const hasChineseText =
        (await page.locator("text=/學習|開始|登入/").count()) > 0;
      console.log(
        `   - Chinese text visible: ${hasChineseText ? "Yes" : "No"}`,
      );
