...
async function checkRequestsHaveCredentials(page: any) {
  ...
  await page.route("**/api/**", async (route: any, request: any) => {
    ...
    // Log for debugging
    console.log(
      `API Request: ${url}, Has Cookie: ${headers.cookie ? "Yes" : "No"}`,
    );
...
test.describe("Authentication Header Sync", () => {
  ...
  test("header should persist login state after page refresh", async ({
    page,
  }) => {
    // First login
  ...
  test("all authenticated API requests should include credentials", async ({
    page,
  }) => {
    ...
    const requestsWithoutCredentials = apiRequests.filter(
      (r) => !r.hasCredentials,
    );
    expect(requestsWithoutCredentials).toHaveLength(0);
  });

  test("401 error should not occur when accessing protected routes after login", async ({
    page,
  }) => {
