...
const PRODUCTION_URL =
  "https://ai-square-frontend-731209836128.asia-east1.run.app";
  ...
  test.describe(`${env.name} - Comprehensive E2E Tests`, () => {
    ...
    test.describe("PBL Module", () => {
      test("1.1 Browse PBL scenarios", async ({ page }) => {
        ...
        await page
          .waitForSelector('[data-testid="scenario-card"]', {
            timeout: 10000,
            state: "visible",
          })
          .catch(() => {
            // Fallback selector
            return page.waitForSelector(".grid .rounded-lg", {
              timeout: 10000,
            });
          });
        ...
        console.log(
          `✅ ${env.name} - PBL: Found ${scenarios.length} scenarios`,
        );
      });
      ...
      test("1.2 View PBL scenario details", async ({ page }) => {
        ...
        const firstScenario = await page.waitForSelector(
        ...
        await page.waitForURL(/\/pbl\/scenarios\/[a-f0-9-]+/, {
          timeout: 10000,
        });
      ...
      test("1.3 Start PBL program (requires login)", async ({ page }) => {
        ...
        const startButton = await page
          .waitForSelector(
            'button:has-text("開始"), button:has-text("Start")',
            {
              timeout: 10000,
            },
          )
        ...
        if (startButton) {
          ...
          console.log(
            `⚠️ ${env.name} - PBL: Start button not found (may need different selector)`,
          );
        }
      });
      ...
      test("1.4 Check PBL API response", async ({ page }) => {
        const response = await page.request.get(
          `${env.url}/api/pbl/scenarios?lang=zh`,
        );
        ...
        console.log(
          `✅ ${env.name} - PBL API: ${data.data.scenarios.length} scenarios`,
        );
      });
      ...
      test("1.5 Verify PBL database records", async ({ page }) => {
        ...
        const response = await page.request.get(
          `${env.url}/api/pbl/scenarios?lang=en`,
        );
        const data = await response.json();
    ...
    test.describe("Discovery Module", () => {
      test("2.1 Browse Discovery scenarios", async ({ page }) => {
        ...
        console.log(
          `✅ ${env.name} - Discovery: Found ${scenarios.length} scenarios`,
        );
      });
      ...
      test("2.2 Test Discovery category filters", async ({ page }) => {
        ...
        for (const category of categories) {
          ...
          const button = await page
            .waitForSelector(`button:has-text("${category.name}")`, {
              timeout: 5000,
            })
            .catch(() => null);
          ...
          if (button) {
            ...
            console.log(
              `✅ ${env.name} - Discovery: ${category.name} = ${filtered.length} scenarios`,
            );
          }
        }
      });
      ...
      test("2.3 View Discovery career details", async ({ page }) => {
        ...
        // Verify navigation
        await page.waitForURL(/\/discovery\/scenarios\/[a-f0-9-]+/, {
          timeout: 10000,
        });
      ...
      test("2.4 Check Discovery API response", async ({ page }) => {
        const response = await page.request.get(
          `${env.url}/api/discovery/scenarios?lang=zh`,
        );
        expect(response.ok()).toBeTruthy();
        ...
        data.data.scenarios.forEach((s: any) => {
          const cat =
            s.discoveryData?.category ||
            s.discovery_data?.category ||
            "unknown";
      ...
      test("2.5 Verify Discovery database records", async ({ page }) => {
        const response = await page.request.get(
          `${env.url}/api/discovery/scenarios?lang=en`,
        );
        const data = await response.json();
    ...
    test.describe("Assessment Module", () => {
      ...
      test("3.2 Start Assessment (requires login)", async ({ page }) => {
        ...
        const startButton = await page
          .waitForSelector(
            'button:has-text("開始評估"), button:has-text("Start Assessment")',
            {
              timeout: 5000,
            },
          )
      ...
      test("3.3 Check Assessment API response", async ({ page }) => {
        const response = await page.request.get(
          `${env.url}/api/assessment/scenarios?lang=zh`,
        );
        expect(response.ok()).toBeTruthy();
        ...
        expect(data.data.scenarios.length).toBeGreaterThan(0);
        console.log(
          `✅ ${env.name} - Assessment API: ${data.data.scenarios.length} scenarios`,
        );
      });

      test("3.4 Verify Assessment database records", async ({ page }) => {
        const response = await page.request.get(
          `${env.url}/api/assessment/scenarios?lang=en`,
        );
        const data = await response.json();
      ...
      test("3.5 Test Assessment question flow", async ({ page }) => {
        ...
        if (data.data?.scenarios?.[0]?.assessmentData) {
          expect(data.data.scenarios[0].assessmentData).toHaveProperty(
            "questionBank",
          );
          console.log(
            `✅ ${env.name} - Assessment: Question structure validated`,
          );
        } else {
    ...
    test.describe("Integration Tests", () => {
      ...
      test("Database health check", async ({ page }) => {
        ...
        if (response.ok()) {
          ...
          console.log(
            `✅ ${env.name} - Database: Connected (${data.checks.database.responseTime}ms)`,
          );
        } else {
          console.log(
            `⚠️ ${env.name} - Health endpoint returned ${response.status()}`,
          );
        }
      });
    });
