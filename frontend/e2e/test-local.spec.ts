...
test.describe("Local Environment Testing", () => {
  ...
  test("2. Check PBL API", async ({ page }) => {
    const response = await page.request.get(
      `${LOCAL_URL}/api/pbl/scenarios?lang=zh`,
    );
    const data = await response.json();
    console.log(
      "✅ Local PBL API:",
  ...
  test("3. Check Discovery API", async ({ page }) => {
    const response = await page.request.get(
      `${LOCAL_URL}/api/discovery/scenarios?lang=zh`,
    );
    const data = await response.json();
    console.log(
      "✅ Local Discovery API:",
  ...
  test("4. Check Assessment API", async ({ page }) => {
    const response = await page.request.get(
      `${LOCAL_URL}/api/assessment/scenarios?lang=zh`,
    );
    const data = await response.json();
    console.log(
      "✅ Local Assessment API:",
      data.data?.scenarios?.length || 0,
      "scenarios",
    );
  });
  ...
  test("6. Browse Discovery page", async ({ page }) => {
    ...
    const filterButtons = await page.$$(
      'button:has-text("創意"), button:has-text("技術")',
    );
