...
test.describe('PBL Instructions Display', () => {
  test('should display instructions on PBL task page', async ({ page }) => {
    ...
    const startButton = page.locator('button').filter({ hasText: /start|開始/i }).first();
    ...
    const instructionsSection = page.locator('[data-testid="instructions"], .instructions, h3:has-text("Instructions")').first();
    ...
    if (await instructionsSection.count() > 0) {
      ...
      const instructionText = 'Welcome explorer! This journey will take you through five stations';
