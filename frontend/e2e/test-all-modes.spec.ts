...
test.describe("AI Square Learning Modes Test", () => {
  ...
  test("PBL Mode: Start scenario → Complete task → View completion", async ({
    page,
  }) => {
    ...
    const firstScenario = page
      .locator('article, .scenario-card, [data-testid*="scenario"]')
      .first();
    if (await firstScenario.isVisible()) {
      ...
      const startButton = page
        .locator('button:has-text("Start"), button:has-text("開始")')
        .first();
      if (await startButton.isVisible()) {
        ...
        const taskContent = page
          .locator('textarea, input[type="text"], .task-content')
          .first();
        if (await taskContent.isVisible()) {
          ...
          if (await page.locator("textarea").isVisible()) {
            await page
              .locator("textarea")
              .fill("This is my solution using AI to solve the challenge.");

            const submitButton = page
              .locator('button:has-text("Submit"), button:has-text("提交")')
              .first();
  ...
  test("Discovery Mode: Browse careers → Start exploration → Complete milestone", async ({
    page,
  }) => {
    ...
    const careerCard = page
      .locator('[data-testid*="career"], .career-card, article')
      .first();
    if (await careerCard.isVisible()) {
      ...
      // 3. Start exploration
      const startButton = page
        .locator('button:has-text("Start"), button:has-text("Explore")')
        .first();
      if (await startButton.isVisible()) {
        await startButton.click();
        ...
        if (await activityInput.isVisible()) {
          ...
          const submitButton = page
            .locator('button:has-text("Submit"), button:has-text("Complete")')
            .first();
          if (await submitButton.isVisible()) {
            await submitButton.click();
          }
  ...
  test("Assessment Mode: Start assessment → Answer questions → View results", async ({
    page,
  }) => {
    // 1. Navigate to Assessment
    await page.goto("http://localhost:3000/assessment/scenarios");
    await page.waitForSelector("text=/Assessment|Evaluate/i", {
      timeout: 10000,
    });

    // 2. Start an assessment
    const assessmentCard = page
      .locator(".assessment-card, article, .grid > div")
      .first();
    if (await assessmentCard.isVisible()) {
      ...
      const startButton = page
        .locator(
          'button:has-text("Start Assessment"), button:has-text("Begin")',
        )
        .first();
      if (await startButton.isVisible()) {
        await startButton.click();
        ...
        const option = page
          .locator(
            'input[type="radio"], button[role="radio"], .option, .choice',
          )
          .first();
        if (await option.isVisible()) {
          ...
          const nextButton = page
            .locator('button:has-text("Next"), button:has-text("Submit")')
            .first();
        ...
        const resultsVisible = await page
          .locator("text=/Results|Score|Complete/i")
          .isVisible();
        if (resultsVisible) {
