...
test.describe("Assessment Completion End-to-End Test", () => {
  ...
  test("should complete full assessment flow from login to completion page", async ({
    page,
  }) => {
    ...
    try {
      ...
      if ((await scenarioElement.count()) === 0) {
        console.log(
          "Direct scenario selector not found, looking for assessment cards...",
        );

        // Try to find assessment cards
        const cards = page.locator(
          '[class*="card"], [role="button"], a[href*="assessment"]',
        );
      ...
      if ((await startButton.count()) > 0) {
        ...
        const createProgramResponse = await page.request.post(
        ...
        if (programId) {
          await page.goto(
            `${baseUrl}/assessment/scenarios/${scenarioId}/program/${programId}`,
          );
          await page.waitForLoadState("networkidle");
        }
      }
      ...
      const completeButton = page
        .locator("button")
        .filter({ hasText: /complete|finish|submit/i })
        .first();
      ...
      if ((await completeButton.count()) > 0) {
        ...
        console.log(
          "⚠️ No completion or next buttons found, trying to navigate to completion directly",
        );
      }
      ...
      if (programIdMatch) {
        ...
        if (completionResponse.ok()) {
          ...
          if (pageContent && pageContent.includes("complet")) {
            console.log(
              "✅ Completion page contains completion-related content",
            );
          }
        } else {
      ...
      const relevantAPIs = apiResponses.filter(
        (api) =>
      ...
      if (relevantAPIs.length > 0) {
        ...
        relevantAPIs.forEach((api) => {
          ...
          console.log(
            `  ${statusIcon} ${api.method} ${api.url} - ${api.status}`,
          );
        });
      }
