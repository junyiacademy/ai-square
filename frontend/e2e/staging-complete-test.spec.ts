...
test.describe('Staging Environment Complete Test', () => {
  ...
  test('1. Homepage loads correctly', async ({ page }) => {
    ...
    await expect(page.locator('select[aria-label*="語言"], select[aria-label*="language"]')).toBeVisible();
  ...
  test('4. Demo account login test', async ({ page }) => {
    ...
    if (hasError) {
      const errorText = await page.locator('.text-red-500, [role="alert"]').first().textContent();
  ...
  test('5. PBL Scenarios page', async ({ page }) => {
    ...
    if (!hasError) {
      ...
      const scenarioCards = await page.locator('[data-testid="scenario-card"], .scenario-card, article').count();
      const emptyState = await page.locator('text=/no scenarios|empty|沒有/i').count() > 0;

      console.log(`✅ PBL page loaded - Scenarios: ${scenarioCards}, Empty state: ${emptyState}`);
    } else {
  ...
  test('9. Mobile responsive test', async ({ page }) => {
    ...
    const mobileMenuButton = await page.locator('button[aria-label*="menu"], button[aria-label*="navigation"]').count() > 0;

    console.log(`✅ Mobile responsive: Menu button ${mobileMenuButton ? 'visible' : 'not found'}`);
  });
  ...
  test('10. Static assets loading', async ({ page }) => {
    ...
    // Check if logo loads
    const logoLoaded = await page.locator('img[alt*="logo"], img[alt*="Logo"]').first().evaluate(img => {
    ...
    console.log(`✅ Static assets: Logo ${logoLoaded ? 'loaded' : 'failed'}, CSS ${hasStyles ? 'loaded' : 'failed'}`);
  });
});
