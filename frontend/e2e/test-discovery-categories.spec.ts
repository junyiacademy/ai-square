...
test.describe("Discovery Category Filters - Staging", () => {
  ...
  test("should show correct counts for each category filter", async ({
    page,
  }) => {
    ...
    await page.waitForTimeout(500);
    const artsScenarios = await page
      .locator('[data-testid="scenario-card"]')
      .count();
    ...
    await page.waitForTimeout(500);
    const techScenarios = await page
      .locator('[data-testid="scenario-card"]')
      .count();
    ...
    await page.waitForTimeout(500);
    const businessScenarios = await page
      .locator('[data-testid="scenario-card"]')
      .count();
    ...
    await page.waitForTimeout(500);
    const scienceScenarios = await page
      .locator('[data-testid="scenario-card"]')
      .count();
    ...
    const totalByCategory =
      artsScenarios + techScenarios + businessScenarios + scienceScenarios;
  ...
  test("should be able to select a scenario from filtered results", async ({
    page,
  }) => {
    // Select a filter
    await page.click('button:has-text("創意")');
