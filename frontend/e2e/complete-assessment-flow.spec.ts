...
test.describe("Complete Assessment Flow - 完整評估流程", () => {
  ...
  test("完整 Assessment 做題流程 - 做題 → 提交 → 拿到成績", async ({
    page,
  }) => {
    ...
    await page.waitForSelector(
      '[data-testid*="scenario"], .assessment-card, .scenario-card',
      { timeout: 10000 },
    );

    // 尋找開始按鈕
    const startButtons = page.locator(
      'button:has-text("開始評估"), button:has-text("Start Assessment"), a:has-text("Start")',
    );
    ...
    if (buttonCount === 0) {
      ...
      const cards = page.locator(
        '.assessment-card, .scenario-card, [data-testid*="scenario"]',
      );
    ...
    if (
      ...
      while (questionCount < maxQuestions) {
        ...
        for (const selector of questionSelectors) {
          ...
          if (
            (await questionElement.count()) > 0 &&
            (await questionElement.isVisible())
          ) {
        ...
        if (!answerSelected) {
          ...
          const textInput = page.locator(
            'input[type="text"], textarea, .answer-input',
          );
    ...
    const hasCompletedFlow =
      ...
      (await page.locator(':has-text("完成"), :has-text("Complete")').count()) >
        0;
