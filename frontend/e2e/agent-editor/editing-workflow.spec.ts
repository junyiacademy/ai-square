...
test.describe("Agent Editor - Complete Editing Workflow", () => {
  ...
  test("should create new scenario from scratch", async ({ page }) => {
    ...
    await expect(
      page.getByText("Level 1: Scenario Level - 場景層級"),
    ).toBeVisible();
  ...
  test("should expand and edit task details", async ({ page }) => {
    ...
    await expect(
      page.getByText("Level 3: Task Detail - 任務詳細設定"),
    ).toBeVisible();
  ...
  test("should show change indicator when modifications made", async ({
    page,
  }) => {
    // Setup: Create new scenario
    await page.getByRole("button", { name: /PBL/i }).first().click();
  ...
  test("should handle keyboard shortcuts in inline editing", async ({
    page,
  }) => {
    // Setup: Create new scenario
    await page.getByRole("button", { name: /PBL/i }).first().click();
