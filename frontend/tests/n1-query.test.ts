...
describe('N+1 Query Detection', () => {
    ...
    it('should avoid N+1 with batch loading', async () => {
      ...
      const mockScenarioRepo = {
        findByIds: jest.fn((ids: string[]) => {
          queryCounter.increment(`SELECT * FROM scenarios WHERE id IN (${ids.join(',')})`);
          return Promise.resolve(ids.map(id => scenarioMap.get(id)).filter(Boolean));
        })
      };
  ...
  describe('Task-Evaluation Relationships', () => {
    it('should detect N+1 when loading evaluations for tasks', async () => {
      ...
      const mockEvaluationRepo = {
        findByTaskId: jest.fn((taskId: string) => {
          queryCounter.increment(`SELECT * FROM evaluations WHERE task_id = '${taskId}'`);
          return Promise.resolve(evaluations.filter(e => e.taskId === taskId));
        })
      };
      ...
      const tasksWithEvaluations = await Promise.all(
        tasks.map(async task => {
          const taskEvaluations = await mockEvaluationRepo.findByTaskId(task.id);
    ...
    it('should batch load evaluations to avoid N+1', async () => {
      ...
      const evaluationsByTask = allEvaluations.reduce((acc, evaluation) => {
  ...
  return {
    ...
    scenario: {
      findMany: jest.fn(({ where }: any) => {
        if (where?.id?.in) {
          // Good: Batch query
          queryCounter.increment(`SELECT with IN clause: ${where.id.in.length} ids`);
        } else {
