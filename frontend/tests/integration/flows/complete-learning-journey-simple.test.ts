...
describe.skip("Simple Learning Journey", () => {
  ...
  it("should fetch learning scenarios", async () => {
    ...
    const assessScenarios =
      assessmentData?.data?.scenarios || assessmentData?.scenarios || [];
    ...
    const discScenarios =
      discoveryData?.data?.scenarios || discoveryData?.scenarios || [];
  ...
  it("should handle API errors gracefully", async () => {
    ...
    const response = await fetch(
      `${baseUrl}/api/pbl/scenarios/non-existent-id`,
    );
