...
describe.skip("Assessment Learning Flow", () => {
  ...
  it("should query Assessment scenarios from database", async () => {
    ...
    console.log(
      `Found ${result.rows.length} active Assessment scenarios in DB`,
    );
    ...
    if (result.rows.length > 0) {
      ...
      console.log(
        `First assessment has ${result.rows[0].question_count} questions`,
      );
    }
  ...
  it("should create and query Assessment program", async () => {
    ...
    try {
      ...
      await client.query(
        ...
        [
          userId,
          `assessment-test-${Date.now()}@example.com`,
      ...
      const program = await client.query<{
        ...
        mode: string;
        status: string;
      }>(`SELECT id, mode, status FROM programs WHERE id = $1`, [programId]);
