...
describe.skip("Cache Consistency", () => {
  ...
  describe.skip("Multi-Layer Cache Architecture", () => {
    it("should implement 3-layer cache with fallback", async () => {
      ...
      const missHeader = getHeaderValue(
        firstResponse.headers as Record<string, string>,
        "x-cache",
      );
      ...
      const hitHeader = getHeaderValue(
        secondResponse.headers as Record<string, string>,
        "x-cache",
      );
    ...
    it("should handle Redis failure with fallback cache", async () => {
      ...
      expect(
        response.body?.data?.scenarios || response.body.scenarios,
      ).toBeDefined();
  ...
  describe.skip("Cache Invalidation", () => {
    it("should invalidate cache on data mutation", async () => {
      ...
      // 1. Warm up cache
      const initialResponse = await apiHelper
        ...
        .catch(
          () =>
            ({ status: 500, headers: {}, body: null }) as unknown as {
              status: number;
              headers: Record<string, string>;
              body: unknown;
            },
        );
      const initialCacheHeader = getHeaderValue(
        initialResponse.headers as Record<string, string>,
        "x-cache",
      );
      expect(["MISS", "SWR", "HIT", undefined]).toContain(initialCacheHeader);

      // 2. Verify cache hit
      const cachedResponse = await apiHelper
        .authenticatedRequest(
          "get",
          `/api/pbl/scenarios/${scenarioId}`,
          userToken,
      ...
      expect(
        getHeaderValue(userProgramsResponse.headers, "x-cache"),
      ).toBeUndefined();
    });
  ...
  describe.skip("Cache Performance", () => {
    it("should improve response times significantly", async () => {
      ...
      console.log(
        `Improvement: ${((1 - warmStats.p50 / coldStats.p50) * 100).toFixed(1)}%`,
      );
    });
    ...
    it("should handle cache stampede prevention", async () => {
      ...
      console.log(
        `Stampede prevention: ${misses} misses, ${hits} hits out of ${concurrentRequests} requests`,
      );
    });
  });
    ...
    it("should maintain separate cache for each language", async () => {
      ...
      for (const lang of languages) {
        ...
        const header = getHeaderValue(
          cachedResponse.headers as Record<string, string>,
          "x-cache",
        );
