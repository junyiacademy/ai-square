...
async function testEnvironment(envKey) {
  ...
  try {
    ...
    if (isHealthy) {
      ...
      console.log(
        `${colors.red}❌ Health check failed (Status: ${healthResponse.statusCode})${colors.reset}`,
      );
    }
  } catch (error) {
    results.tests.healthCheck = { passed: false, error: error.message };
    console.log(
      `${colors.red}❌ Health check error: ${error.message}${colors.reset}`,
    );
  }
  ...
  try {
    ...
    const loginSuccess =
      loginResponse.statusCode === 200 && loginResult.success;
    ...
    if (loginSuccess) {
      ...
      console.log(
        `${colors.green}✅ Database connection verified (login successful)${colors.reset}`,
      );
    } else {
      console.log(
        `${colors.red}❌ Database connection issue: ${loginResult.error || "Login failed"}${colors.reset}`,
      );
    }
  } catch (error) {
    results.tests.databaseConnection = { passed: false, error: error.message };
    console.log(
      `${colors.red}❌ Database connection error: ${error.message}${colors.reset}`,
    );
  }
  ...
  try {
    ...
    if (pblSuccess) {
      console.log(
        `${colors.green}✅ PBL API working (${pblData.data?.scenarios?.length || 0} scenarios)${colors.reset}`,
      );
    } else {
    ...
    console.log(
      `${colors.red}❌ PBL API error: ${error.message}${colors.reset}`,
    );
  }
  ...
  try {
    ...
    const discoverySuccess =
      discoveryResponse.statusCode === 200 && discoveryData.success;
    ...
    if (discoverySuccess) {
      ...
      scenarios.forEach((s) => {
        const cat =
          s.discoveryData?.category || s.discovery_data?.category || "unknown";
      ...
      console.log(
        `${colors.green}✅ Discovery API working (${scenarios.length} scenarios)${colors.reset}`,
      );
      ...
      for (const [cat, count] of Object.entries(expectedCategories)) {
        if (categories[cat] !== count) {
          categoryMatch = false;
          console.log(
            `   ${colors.yellow}⚠ ${cat}: expected ${count}, got ${categories[cat] || 0}${colors.reset}`,
          );
        }
      }
      ...
      if (categoryMatch) {
        console.log(
          `   ${colors.green}✅ All category counts match expected values${colors.reset}`,
        );
      }
    } else {
      results.tests.discoveryScenarios = {
    ...
    console.log(
      `${colors.red}❌ Discovery API error: ${error.message}${colors.reset}`,
    );
  }
  ...
  try {
    ...
    const assessmentSuccess =
      assessmentResponse.statusCode === 200 && assessmentData.success;

    results.tests.assessmentScenarios = {
      passed: assessmentSuccess,
      statusCode: assessmentResponse.statusCode,
      scenarioCount: assessmentSuccess
        ? assessmentData.data?.scenarios?.length || 0
        : 0,
    };

    if (assessmentSuccess) {
      console.log(
        `${colors.green}✅ Assessment API working (${assessmentData.data?.scenarios?.length || 0} scenarios)${colors.reset}`,
      );
    } else {
    ...
    console.log(
      `${colors.red}❌ Assessment API error: ${error.message}${colors.reset}`,
    );
  }
  ...
  try {
    ...
    if (imageSuccess) {
      console.log(
        `${colors.green}✅ Static assets serving correctly${colors.reset}`,
      );
    } else {
      console.log(
        `${colors.red}❌ Static assets issue (Status: ${imageResponse.statusCode})${colors.reset}`,
      );
    }
  } catch (error) {
    results.tests.staticAssets = { passed: false, error: error.message };
    console.log(
      `${colors.red}❌ Static assets error: ${error.message}${colors.reset}`,
    );
  }
...
async function main() {
  ...
  if (envToTest && environments[envToTest]) {
    ...
    console.log(
      `${colors.red}Invalid environment: ${envToTest}${colors.reset}`,
    );
  ...
  for (const [envKey, results] of Object.entries(testResults.environments)) {
    ...
    console.log(
      `${results.name}: ${status} (${results.summary.passed}/${results.summary.total} tests passed)`,
    );
    ...
    if (results.summary.failed > 0) {
      ...
      for (const [testName, test] of Object.entries(results.tests)) {
        if (!test.passed) {
          console.log(
            `    - ${testName}: ${test.error || `Status ${test.statusCode}`}`,
          );
        }
      }
    }
