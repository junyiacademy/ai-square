...
async function testIdGeneration() {
  ...
  try {
    ...
    const taskResult = await pool.query(`
      ...
      RETURNING id
    `, [
      programId, scenarioId, 'pbl', 0, 'question', 'pending',
      '{"en": "Test Task"}', '{"en": "Test Instructions"}',
      '{}', '{}', '[]'
    ]);
    ...
    const batchResult = await pool.query(`
      ...
      RETURNING id
    `, [
      programId, scenarioId, 'pbl', 'question', 'pending',
      '{"en": "Batch Task"}', '{"en": "Batch Instructions"}',
      '{}', '{}', '[]'
    ]);

    console.log(`✅ Created ${batchResult.rows.length} batch tasks with unique IDs\n`);
