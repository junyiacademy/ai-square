...
async function runDemo() {
  ...
  await distributedCacheService.set(
    "demo:key",
    { ts: Date.now() },
    { ttl: 2000 },
  );
  ...
  const swr1 = await distributedCacheService.getWithRevalidation(
    swrKey,
    fetcher,
    {
      ttl: 500,
  ...
  console.log(
    "SWR second fetch (stale should be returned, bg revalidate triggered)...",
  );
  const swr2 = await distributedCacheService.getWithRevalidation(
    swrKey,
    fetcher,
    {
      ttl: 500,
  ...
  const swr3 = await distributedCacheService.getWithRevalidation(
    swrKey,
    fetcher,
    {
      ttl: 500,
