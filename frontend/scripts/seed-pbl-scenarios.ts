...
async function loadPBLScenarios() {
  ...
  try {
    ...
    await pool.query(
      "DELETE FROM scenarios WHERE mode = 'pbl' AND (title->>'en' LIKE '%test%' OR title->>'en' LIKE '%Test%')",
    );
    const cleanupResult = await pool.query(
      "SELECT COUNT(*) FROM scenarios WHERE mode = 'pbl'",
    );
    ...
    for (const scenarioFolder of scenarioFolders) {
      ...
      for (const lang of LANGUAGES) {
        ...
        title[lang] =
          data.scenario_info?.title ||
          enData.scenario_info?.title ||
          `PBL Scenario: ${scenarioFolder}`;
        description[lang] =
          data.scenario_info?.description ||
          enData.scenario_info?.description ||
          "Problem-based learning scenario";
        objectives[lang] =
          data.scenario_info?.learning_objectives ||
          enData.scenario_info?.learning_objectives ||
          [];
      }

      // 準備任務模板
      const taskTemplates =
        enData.tasks?.map((task: any, index: number) => ({
          id: task.id || `task_${index + 1}`,
          title: LANGUAGES.reduce(
            (acc, lang) => {
              const data = multilingualData[lang];
              acc[lang] =
                data.tasks?.[index]?.title || task.title || `Task ${index + 1}`;
              return acc;
            },
