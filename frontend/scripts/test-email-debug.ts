...
async function testEmailDebug() {
  ...
  console.log('🔑 Password (raw):', passRaw ? `"${passRaw}" (${passRaw.length} chars)` : 'NOT SET');
  console.log('🔑 Password (clean):', passClean ? `${passClean.length} chars` : 'NOT SET');
  console.log('');
  ...
  console.log('4. Check if the account is a regular Gmail or Google Workspace account');
}
