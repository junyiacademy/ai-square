...
class SchemaDriftDetector {
  ...
  async connectRemote(env: "staging" | "production"): Promise<void> {
    if (env === "staging") {
      ...
      console.log(
        chalk.yellow(
          "⚠️  Note: Direct staging connection requires Cloud SQL Proxy",
        ),
      );
      console.log(
        chalk.yellow(
          "   Run: cloud_sql_proxy -instances=PROJECT:REGION:INSTANCE=tcp:5434",
        ),
      );
  ...
  compareSchemas(
    ...
    for (const table of allTables) {
      ...
      const localFieldMap = new Map(localFields.map((f) => [f.column_name, f]));
  ...
  generateMigration(diffs: SchemaDiff[]): string {
    ...
    for (const diff of diffs) {
      if (
        diff.table === "evaluations" &&
        diff.missingInRemote.includes("evaluation_subtype")
      ) {
        migrations.push(`-- 🚨 CRITICAL: Fix evaluation_subtype issue`);
        migrations.push(
          `ALTER TABLE evaluations ADD COLUMN IF NOT EXISTS evaluation_subtype TEXT;`,
        );
        migrations.push("");
      }
      ...
      for (const field of diff.missingInRemote) {
        ...
        migrations.push(
          `-- ALTER TABLE ${diff.table} ADD COLUMN ${field} <TYPE>;`,
        );
        migrations.push("");
      }
      ...
      for (const field of diff.missingInLocal) {
        ...
        migrations.push(
          `-- Consider adding to Prisma schema: ${diff.table}.${field}`,
        );
        migrations.push("");
      }
    }
  ...
  async checkEvaluationSubtype(): Promise<void> {
    ...
    console.log(
      `Local DB has evaluation_subtype: ${localCheck.rows.length > 0 ? chalk.green("YES") : chalk.red("NO")}`,
    );
    console.log(
      `Prisma schema has evaluationSubtype: ${hasPrismaField ? chalk.green("YES") : chalk.red("NO")}`,
    );
    console.log(
      `TypeScript has evaluation_subtype: ${hasTypeField ? chalk.green("YES") : chalk.red("NO")}`,
    );
    ...
    if (localCheck.rows.length > 0 && !hasPrismaField) {
      ...
      console.log(
        chalk.yellow("Fix: Add to Prisma schema or remove from database"),
      );
    }
    ...
    if (hasTypeField && !hasPrismaField) {
      ...
      console.log(
        chalk.yellow("Fix: Add to Prisma schema or remove from TypeScript"),
      );
    }
  }
    ...
    try {
      ...
      if (compareWithRemote) {
        ...
        if (diffs.length === 0) {
          ...
          console.log(
            chalk.red(`❌ Found ${diffs.length} table(s) with differences:\n`),
          );
          ...
          for (const diff of diffs) {
            ...
            if (diff.missingInRemote.length > 0) {
              console.log(
                chalk.red(
                  `  Missing in remote: ${diff.missingInRemote.join(", ")}`,
                ),
              );
            }
            ...
            if (diff.missingInLocal.length > 0) {
              console.log(
                chalk.blue(
                  `  Missing in local: ${diff.missingInLocal.join(", ")}`,
                ),
              );
            }
            ...
            if (diff.typeMismatches.length > 0) {
              ...
              for (const mismatch of diff.typeMismatches) {
                console.log(
                  `    ${mismatch.field}: ${mismatch.local} (local) vs ${mismatch.remote} (remote)`,
                );
              }
            }
...
if (require.main === module) {
  ...
  if (compareWithRemote) {
    console.log(
      chalk.yellow("Note: Remote comparison requires Cloud SQL Proxy setup"),
    );
  }

  detector
    .detect(compareWithRemote)
    .then(() => {
      process.exit(0);
    })
    .catch(() => {
