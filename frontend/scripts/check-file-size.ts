...
function hasMultipleConcerns(content: string, filePath: string): boolean {
  const concernPatterns = {
    ...
    apiHandling: /(?:export\s+async\s+function\s+(?:GET|POST|PUT|DELETE|PATCH))/,
  ...
  return problematicCombinations.some(([concern1, concern2]) =>
    detectedConcerns.includes(concern1) && detectedConcerns.includes(concern2)
  );
}
 ...
 */
function isFileExempt(filePath: string, content: string): { isExempt: boolean; reason?: string } {
  ...
  for (const pattern of EXEMPT_PATTERNS) {
    if (pattern.test(fileName)) {
      return { isExempt: true, reason: 'Type definition or configuration file' };
    }
  }

  // Check for exemption documentation in file
  const exemptionMatch = content.match(/FILE SIZE EXEMPTION[\s\S]*?Justification:([\s\S]*?)(?:\*\/|Reviewed:)/i);
  if (exemptionMatch) {
    return { isExempt: true, reason: exemptionMatch[1].trim().substring(0, 100) };
  }
 ...
 */
function analyzeFileMetrics(filePath: string, content: string, lineCount: number): FileMetrics {
  return {
    lines: lineCount,
...
function generateSuggestions(
  ...
  if (metrics.complexity > 50) {
    suggestions.push(`HIGH COMPLEXITY (${metrics.complexity}): Break down complex logic into smaller functions`);
  ...
  if (metrics.hasMultipleConcerns) {
    suggestions.push('MIXED CONCERNS: Separate different responsibilities into focused files');
    suggestions.push('Follow Single Responsibility Principle - one file, one purpose');
  }

  // Duplication suggestions
  if (metrics.duplicationScore > 0.3) {
    suggestions.push(`CODE DUPLICATION (${(metrics.duplicationScore * 100).toFixed(0)}%): Extract repeated patterns into utilities`);
    suggestions.push('Apply DRY principle to reduce redundancy');
  }

  // Structure suggestions
  if (!metrics.hasClearSections && metrics.lines > 200) {
    suggestions.push('Add section comments to improve navigation (e.g., // === Helper Functions ===)');
  ...
  switch (type) {
    case 'component':
      if (metrics.exportCount > 5) {
        suggestions.push('Too many exports - extract child components into separate files');
      }
    ...
    case 'api':
      if (metrics.hasMultipleConcerns) {
        ...
        suggestions.push('CRITICAL: Use Repository Pattern for database access');
      }
    ...
    case 'service':
      if (metrics.lines > 600) {
        suggestions.push('Split into multiple focused services by feature/domain');
      }
    ...
    case 'test':
      if (metrics.lines > 1000) {
        suggestions.push('Consider splitting test file by feature or test suite');
      }
      suggestions.push('Use describe blocks to organize related tests');
      break;

    default:
      suggestions.push('Consider splitting into multiple smaller files by concern');
...
function determineSeverity(
  ...
  const criteria: EnforcementCriteria = {
    ...
    multipleResponsibilities: metrics.hasMultipleConcerns || metrics.exportCount > 10,
  };

  // Only ERROR if meets ALL enforcement criteria
  if (criteria.size && criteria.complexity && criteria.multipleResponsibilities) {
...
function scanDirectory(dir: string, violations: FileViolation[] = []): FileViolation[] {
  ...
  for (const entry of entries) {
    ...
    const { isExempt, reason: exemptionReason } = isFileExempt(fullPath, content);
    const { severity, enforcementCriteria } = determineSeverity(metrics, limit, isExempt);

    // Report if exceeds soft limit OR has quality issues
    if (lines > limit || metrics.hasMultipleConcerns || metrics.complexity > 50) {
 ...
 */
function formatViolations(violations: FileViolation[], showSuggestions: boolean, verboseMode: boolean): string {
  ...
  if (errors.length > 0) {
    ...
    output += '   Files meet ALL enforcement criteria: 2x size + high complexity + multiple concerns\n\n';
    errors.forEach(v => {
      output += `  ${v.file}\n`;
  ...
  if (warnings.length > 0) {
    ...
    output += '   Files exceed soft limits but may be justified if well-structured\n\n';
    warnings.forEach(v => {
      output += `  ${v.file}\n`;
      output += `    Lines: ${v.lines} / Soft Limit: ${v.limit} (${v.type})\n`;
...
function main() {
  ...
  console.log('   Checking: Complexity, Cohesion, AI-Readability, Token Efficiency\n');
  ...
  if (violations.length > 0) {
    ...
    if (errors.length > 0) {
      ...
      errors.forEach(e => {
        console.log(`  • ${e.file}`);
        console.log(`    Criteria: ${e.lines} lines (2x limit: ${e.limit * 2}), ` +
