...
export function Editor({ file, content, onChange, isLoading }: EditorProps) {
  ...
  const loadFileContent = async (filePath: string) => {
    try {
      const response = await fetch(
        `/api/content?path=${encodeURIComponent(filePath)}`,
      );
      ...
      const windowWithSetOriginalContent = window as Window & {
  ...
  return (
    <div className="flex flex-col h-full">
      <Tabs
        value={activeTab}
        onValueChange={(v) => setActiveTab(v as "code" | "visual" | "preview")}
      ...
      >
        ...
        <TabsContent
          value="visual"
          className="flex-1 m-0 p-8 overflow-y-auto bg-gradient-to-br from-gray-50 to-white"
        >
          <div className="max-w-4xl mx-auto text-center py-16">
            ...
            <p className="text-sm text-gray-500">
              Coming soon with drag-and-drop components
            </p>
          </div>
        </TabsContent>
