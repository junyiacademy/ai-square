...
describe("yaml-order utils", () => {
  describe("sortPBLScenario", () => {
    it("should sort scenario fields in correct order", () => {
      ...
      expect(scenarioKeys.indexOf("id")).toBeLessThan(
        scenarioKeys.indexOf("title"),
      );
      expect(scenarioKeys.indexOf("title")).toBeLessThan(
        scenarioKeys.indexOf("description"),
      );
      expect(scenarioKeys.indexOf("description")).toBeLessThan(
        scenarioKeys.indexOf("difficulty"),
      );
      expect(scenarioKeys.indexOf("difficulty")).toBeLessThan(
        scenarioKeys.indexOf("estimated_duration"),
      );
      expect(scenarioKeys.indexOf("estimated_duration")).toBeLessThan(
        scenarioKeys.indexOf("target_domains"),
      );
      expect(scenarioKeys.indexOf("target_domains")).toBeLessThan(
        scenarioKeys.indexOf("prerequisites"),
      );
      expect(scenarioKeys.indexOf("prerequisites")).toBeLessThan(
        scenarioKeys.indexOf("learning_objectives"),
      );

      // Check ksa_mapping field order
      const ksaKeys = Object.keys(result.ksa_mapping!);
      expect(ksaKeys.indexOf("knowledge")).toBeLessThan(
        ksaKeys.indexOf("skills"),
      );
      expect(ksaKeys.indexOf("skills")).toBeLessThan(
        ksaKeys.indexOf("attitudes"),
      );
      ...
      expect(taskKeys.indexOf("title")).toBeLessThan(
        taskKeys.indexOf("description"),
      );
      expect(taskKeys.indexOf("description")).toBeLessThan(
        taskKeys.indexOf("category"),
      );
      expect(taskKeys.indexOf("category")).toBeLessThan(
        taskKeys.indexOf("instructions"),
      );
      expect(taskKeys.indexOf("instructions")).toBeLessThan(
        taskKeys.indexOf("expected_outcome"),
      );
    });
    ...
    it("should preserve all field values", () => {
      const input = {
        scenario_info: {
          ...
          target_domains: [
            "creating_with_ai" as const,
            "managing_with_ai" as const,
          ],
      ...
      expect(result.scenario_info.target_domains).toEqual([
        "creating_with_ai",
        "managing_with_ai",
      ]);
