  ...
  describe("formatKSACodesForPrompt", () => {
    it("should format KSA codes into a readable prompt format", () => {
      ...
      expect(result).toContain(
        "KSA (Knowledge, Skills, Attitudes) Competency Codes Reference:",
      );
    ...
    it("should handle empty KSA codes gracefully", () => {
      ...
      const {
        formatKSACodesForPrompt: emptyFormatter,
      } = require("../ksa-codes-loader");
    ...
    it("should include usage guidelines", () => {
      ...
      const knowledgeLine = lines.find((line) =>
        line.includes("Knowledge (K):"),
      );
      const skillsLine = lines.find((line) => line.includes("Skills (S):"));
      const attitudesLine = lines.find((line) =>
        line.includes("Attitudes (A):"),
      );
