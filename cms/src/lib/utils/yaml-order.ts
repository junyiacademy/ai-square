...
export function sortObjectByKeyOrder<T extends Record<string, unknown>>(
  obj: T,
  keyOrder: string[],
): T {
...
export function sortPBLScenario<T extends Record<string, unknown>>(data: T): T {
  ...
  // Sort root level
  const sorted = sortObjectByKeyOrder(data, YAML_KEY_ORDER.root) as Record<
    string,
    unknown
  >;
  ...
  if (sorted.scenario_info) {
    sorted.scenario_info = sortObjectByKeyOrder(
      sorted.scenario_info,
      YAML_KEY_ORDER.scenario_info,
    );
  }

  // Sort ksa_mapping
  if (sorted.ksa_mapping) {
    sorted.ksa_mapping = sortObjectByKeyOrder(
      sorted.ksa_mapping,
      YAML_KEY_ORDER.ksa_mapping,
    );
  }
