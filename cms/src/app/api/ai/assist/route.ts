  ...
  try {
    ...
    try {
        ...
        if (parsed.scenario_info && action === "translate") {
          ...
          for (const field of baseFields) {
            ...
            if (missingTranslations.length > 0) {
              validation.errors.push(
                `Missing ${field} translations: ${missingTranslations.join(", ")}`,
              );
            }
          }
          ...
          for (const field of arrayFields) {
            if (parsed.scenario_info[field]) {
              const missingTranslations = languages.filter(
                (lang) =>
                  !parsed.scenario_info[`${field}_${lang}`] ||
      ...
      if (typeof parsed !== "object" || parsed === null) {
        validation.valid = false;
        validation.errors.push(
          "Invalid YAML structure: result is not an object",
        );
      }
