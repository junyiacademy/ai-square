...
describe("/api/content", () => {
  ...
  beforeEach(() => {
    ...
    (
      GitHubService as jest.MockedClass<typeof GitHubService>
    ).mockImplementation(() => mockGitHubService);
  });
  ...
  describe("GET", () => {
    it("should return file content successfully", async () => {
      ...
      const request = new Request(
        "http://localhost:3000/api/content?path=test.yaml",
      );
    ...
    it("should handle file read errors", async () => {
      ...
      const request = new Request(
        "http://localhost:3000/api/content?path=nonexistent.yaml",
      );
      const response = await GET(request);
      const data = await response.json();
