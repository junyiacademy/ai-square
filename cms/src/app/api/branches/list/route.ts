...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `Found ${allPRs.length} total PRs, ${prs.length} open PRs, ${uniquePRs.length} unique open PRs`,
    );
    uniquePRs.forEach((pr) => {
      console.log(
        `PR #${pr.number}: ${pr.title} (state: ${pr.state}, merged: ${pr.merged_at ? "YES" : "NO"}, branch: ${pr.head?.ref || "unknown"})`,
      );
    });
    ...
    const branchDetails: BranchDetails[] = await Promise.all(
      uniquePRs.map(async (pr) => {
        try {
          ...
          return {
            name: branchName,
            creatorEmail: pr.user?.login
              ? `${pr.user.login}@github`
              : "unknown",
